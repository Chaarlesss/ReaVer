...
let options =
  ...
  ; ( "-cudd_print_limit"
    , Arg.Set_int Env.cudd_print_limit
    , "up to which BDD size formulas are printed" )
  ; ( "-env_num_factor"
    , Arg.Set_int num_factor
    , "increase number of constraint variables in BDDs" )
  ; ( "-inputformat"
    , Arg.Set_string inputformat
    , "specifies the input format (for command line and stdin)" )
  ...
  ; ( "-debug"
    , Arg.String (fun l -> Log.globallevel := Log.string2level l)
    , "debug level (ERROR|WARN|INFO|DEBUG)" )
  ; ( "-debug_force"
    , Arg.String (fun l -> Log.globallevel_weaken := Log.string2level l)
    , "debug level (ERROR|WARN|INFO|DEBUG)" )
  ]
;;

let print_usage () = Arg.usage options usage
let parse () = Arg.parse options (fun x -> inputfile := x) usage
