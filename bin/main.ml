...
let logger =
  { Log.fmt = Format.std_formatter; Log.module_name = "Main"; Log.level = Log.Info }
;;
...
let result_to_text result =
  if result
  then "PROPERTY TRUE (final unreachable)"
...
let display_dfprog_info env dfprog =
  ...
  Log.info
    logger
    ("variables(bool/num): state=("
     ^ string_of_int bs_vars
     ^ "/"
     ^ string_of_int ns_vars
     ^ "), input=("
  ...
  Log.debug_o
    logger
    (BddapronUtil.print_equations ~cont:true env.Env.env env.Env.cond)
    "continuous equation system: "
    dfprog.Program.d_cont_equs;
  Log.debug2_o logger Bddapron.Env.print "environment: " env.Env.env;
  Log.debug2_o logger (Bddapron.Cond.print env.Env.env) "conditions: " env.Env.cond;
  Log.debug2_o logger (fun f e -> Bddapron.Env.print_order e f) "env-order: " env.Env.env;
  Log.info_o
    logger
  ...
  Log.debug_o logger (Env.print_zero_defs env) "zero_defs: " dfprog.Program.d_zero_defs;
  Log.debug_o
    logger
    (BddapronUtil.print_boolexpr env.Env.env env.Env.cond)
    "careset = "
    env.Env.cond.Bdd.Cond.careset;
  Log.debug3_o logger Format.pp_print_int "bool_size: " !Env.bool_size
;;

let print_header_footer footer =
  match !Log.print_format with
  | Log.Xml ->
    Format.pp_print_string logger.Log.fmt ("<" ^ (if footer then "/" else "") ^ "reaver>");
...
let build_env_dfprog decl translate =
  let make_env num_factor =
    Env.make ~num_factor decl.Program.typdef decl.Program.state decl.Program.input
  in
  ...
  while not !succeeded do
    ...
    := try
       ...
       | Bdd.Env.Bddindex ->
         ...
         Log.warn_o logger Format.pp_print_int "restarting with num_factor=" !num_factor;
...
let parse inputfile inputformat input =
  ...
  then
    if inputfile <> ""
    then NbacTrans.parse ~is_file:true inputfile
    else NbacTrans.parse ~is_file:false input
  else if Filename.check_suffix inputfile ".nbac"
  then NbacTrans.parse inputfile
  else raise (Arg.Bad ("input file format of '" ^ inputfile ^ "' not recognized"))
;;
...
let main () =
  ...
  Log.info_o logger Format.pp_print_string "parsing inputfile " !Option.inputfile;
  let decl, translate, default_df2cf, default_strat =
    parse !Option.inputfile !Option.inputformat !Option.input
  in
  ...
  let strategy = Verif.str_to_strategy env (Util.string_remove_whitespaces str_strat) in
...
let _ =
  ...
  (try
     ...
     Log.info_o logger Format.pp_print_string "ReaVer, version " version;
     main ()
   with
   | Option.InvalidArgs ->
     ...
     Option.print_usage ()
   | Option.HelpAndExit -> ()
   | Parse.Lex_error -> Log.error logger "aborted."
   | Exit -> Log.error logger "aborted."
   | Arg.Bad s -> Log.error logger s
   | NbacExpr.NbacParseError s -> Log.error logger s
   | Df2cf.InvalidDf2cf s -> Log.error logger ("invalid preprocessing method: " ^ s)
   | Verif.InvalidStrategy s -> Log.error logger ("invalid verification strategy: " ^ s)
   ...
   | BddapronUtil.NotSupported s -> Log.error logger ("not supported: " ^ s)
   | Domain.NotSupported s -> Log.error logger ("not supported: " ^ s)
   | Bdd.Env.Bddindex ->
     Log.error
       logger
       "insufficient environment size: try with increasing values for option \
        \"-env_num_factor <n>\""
   | Sys_error s -> Log.error logger s);
