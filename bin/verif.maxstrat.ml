...
let strats =
  (* identifier * short description * long description list *)
  [ (********** PARTITIONING ***********************************************)
    ( "pIF"
    , ("initial, final and other states", "partition by initial, final and other states")
    )
  ; ( "pE"
    , ( "enumeration of Boolean states"
      , "enumerate (non initial/non final) Boolean states, options: v=<vars>...restrict \
         enumeration to variables" ) )
  ; ( "pM"
    , ( "manual partitioning"
      , "manual partitioning, options: e=<exprs>...partitioning predicates" ) )
  ; ( "pMD"
    , ( "discrete numerical modes (Boolean-defined)"
      , "partition by discrete numerical modes (Boolean-defined), "
        ^ "options: v=<vars>...restrict to given numerical variables, "
        ^ "i=<forget>...bi=ignore Boolean inputs (default), bic=ignore Boolean inputs \
           and numerical constraints" ) )
  ; ( "pMHB"
    , ( "continuous modes (Boolean-defined)"
      , "partition by continuous modes (Boolean-defined), "
        ^ "options: v=<vars>...restrict to given numerical variables, "
        ^ "i=<forget>...bi=ignore Boolean inputs (default), bic=ignore Boolean inputs \
           and numerical constraints" ) )
  ; ( "pMHN"
    , ( "continuous modes (numerically defined)"
  ...
  ; ( "pQ"
    , ( "enumeration of hybrid translation variables"
      , "enumerate hybrid translation state variables" ) )
  ; "pS", ("convex staying conditions", "split into convex staying conditions")
  ; "tR", ("relational abstractions", "relational abstractions")
  ; ( "pB"
    , ( "boolean backward bisimulation"
      , "refine partition by boolean backward bisimulation" ) )
  ; (********** PREPROCESSING ***********************************************)
    "rT", ("refine transition functions to arcs", "refine transition functions to arcs")
  ; "rB", ("remove boolean inputs", "remove boolean inputs")
  ; "rS", ("split non-convex guards", "split non-convex guards")
  ; ( "rAB"
  ...
  ; ( "rAS"
    , ( "split non-convex numerical guards in accelerable self-loops"
      , "split non-convex numerical guards in accelerable self-loops" ) )
  ; "rAF", ("flatten accelerable self-loops", "flatten accelerable self-loops")
  ; ( "rAD"
    , ( "decouple accelerable from non-accelerable or Boolean self-loops"
      , "decouple accelerable from non-accelerable or Boolean self-loops" ) )
  ; ( "rAI"
    , ("inputization for decoupled self-loops", "inputization for decoupled self-loops") )
  ; (********** ANALYSIS ***********************************************)
    (********** discrete ***********************************************)
    "aB", ("boolean analysis", "boolean analysis, options: b...backward")
  ; ( "aS"
    , ( "standard analysis"
      , "standard analysis, options: d=<domain>, b...backward, ws=<widening start>, \
         wd=<descending iterations>" ) )
  ; ( "aA"
    , ( "analysis with abstract acceleration"
      , "analysis with abstract acceleration, options:d=<domain>, b...backward, \
         ws=<widening start>, wd=<descending iterations>, aws=<widening start for \
         accelerable transitions>" ) )
  ; ( "aM"
    , ( "numerical max-strategy iteration"
      , "numerical max-strategy iteration, options: d=<template domain>, i=<improvement \
         algorithm>...e=enumerate strategies, s=use SMT (default)" ) )
  ; (*   ("aL",("logico-numerical max-strategy iteration",
           "logico-numerical max-strategy iteration, options: d=<template domain>")); *)
    ( "aLP"
    , ( "logico-numerical max-strategy iteration with power domain"
      , "logico-numerical max-strategy iteration with power domain, options: d=<template \
         domain>, i=<improvement algorithm>...m=micro-iterations, n=no micro-iterations, \
         s=symbolic template rows (default), t=try imp. strats" ) )
  ; (********** ANALYSIS ***********************************************)
    (********** hybrid ***********************************************)
    ( "aH"
    , ( "hybrid analysis with time elapse"
      , "hybrid analysis with time elapse, options: d=<domain>, ws=<widening start>, \
         wd=<descending iterations>" ) )
  ]
;;
...
let domain_descs =
  [ ( "P"
    , ( "convex polyhedra"
      , "convex polyhedra, options: l...without strict inequalities, p...power domain" ) )
  ; "O", ("octagons", "octagons, options: p...power domain")
  ; "I", ("intervals", "intervals, options: p...power domain")
  ; ( "TE"
    , ( "template polyhedra (emulation)"
      , "template polyhedra (emulation), options: t=<template expressions>, p...power \
         domain" ) )
  ; ( "FdpI"
    , ( "finitely disjunctive partitioned interval domain"
      , "finitely disjunctive partitioned interval domain" ) )
  ]
;;
...
let parse_domain env dom =
  let domstr, options =
    try ParseParams.parse_param dom with
  ...
  let domstr =
    ...
    ^ (if domstr = "P"
       then
         ParseParams.get_option options defaults "l" (function
           | "" -> "l"
           | x -> x)
       else "")
    ^ ParseParams.get_option options defaults "p" (function
      | "" -> "p"
      | x -> x)
  in
  match domstr with
  | "Psc" ->
    let _ = DomainStd.StrictPolProd.makeinit_doman () in
    (module DomainStd.StrictPolProd : Domain.T)
  | "Psp" ->
    let _ = DomainStd.StrictPolPow.makeinit_doman () in
    (module DomainStd.StrictPolPow : Domain.T)
  | "Plc" ->
    let _ = DomainStd.LoosePolProd.makeinit_doman () in
    (module DomainStd.LoosePolProd : Domain.T)
  | "Plp" ->
    let _ = DomainStd.LoosePolPow.makeinit_doman () in
    (module DomainStd.LoosePolPow : Domain.T)
  | "Oc" ->
    let _ = DomainStd.OctProd.makeinit_doman () in
    (module DomainStd.OctProd : Domain.T)
  | "Op" ->
    let _ = DomainStd.OctPow.makeinit_doman () in
    (module DomainStd.OctPow : Domain.T)
  | "Ic" ->
    let _ = DomainStd.BoxProd.makeinit_doman () in
    (module DomainStd.BoxProd : Domain.T)
  | "Ip" ->
    let _ = DomainStd.BoxPow.makeinit_doman () in
    (module DomainStd.BoxPow : Domain.T)
  | "TEc" ->
    let templates =
      ParseParams.get_option options defaults "t" (fun x ->
        Template.template_of_strlist env (ParseParams.comma_str_to_strlist x))
    in
...
let parse_template_domain env dom =
  ...
  | "TEp" ->
    let _ = Template.EmuPow.makeinit_doman templates in
...
let build_call_pE env options =
  ...
  let vars =
    ParseParams.get_option options defaults "v" ParseParams.comma_str_to_strlist
  in
...
let build_call_pM env options =
  ...
  let exprlist =
    ParseParams.get_option options defaults "e" ParseParams.comma_str_to_strlist
  in
...
let build_call_pMHN env options =
  let defaults = Util.list2mappe [ "d", "FdpI"; "ws", "0"; "wd", "1" ] in
  ...
  VerifUtil.Trans
    ( TransHybrid.modes_num (An.analyze (AnalysisStd.make_hyb_param ws wd))
...
let build_call_rAB env options =
  ...
  let decouplemode =
    ParseParams.get_option options defaults "d" (function
...
let parse_aS env options =
  let defaults = Util.list2mappe [ "b", "f"; "d", "P:s,c"; "ws", "2"; "wd", "2" ] in
  let dir =
    ParseParams.get_option options defaults "b" (function
      | "f" -> `Forward
      | _ -> `Backward)
  in
  let ws = ParseParams.get_option options defaults "ws" int_of_string in
  ...
  VerifUtil.Analysis
    ...
    , get_short_stratdesc "aS" ^ " " ^ dir_to_string dir ^ " with " ^ get_domain_desc dom
    )
;;
...
let parse_aB options =
  ...
  in
  VerifUtil.Analysis
    ( AnalysisStd.Bool.analyze (AnalysisStd.make_bool_param dir)
    , get_short_stratdesc "aB" ^ " " ^ dir_to_string dir )
;;

let parse_aA env options =
  let defaults =
    Util.list2mappe [ "b", "f"; "d", "P:s,c"; "ws", "2"; "aws", "7"; "wd", "2" ]
  in
  let dir =
    ParseParams.get_option options defaults "b" (function
      | "f" -> `Forward
  ...
  VerifUtil.Analysis
    ...
    , get_short_stratdesc "aA" ^ " " ^ dir_to_string dir ^ " with " ^ get_domain_desc dom
    )
;;

let parse_aH env options =
  let defaults = Util.list2mappe [ "d", "P:s,c"; "ws", "2"; "wd", "2" ] in
  let ws = ParseParams.get_option options defaults "ws" int_of_string in
  let wd = ParseParams.get_option options defaults "wd" int_of_string in
  let dom = ParseParams.get_option options defaults "d" (fun x -> x) in
  let module Dom = (val parse_domain env dom : Domain.T) in
  ...
  VerifUtil.Analysis
    ...
    , get_short_stratdesc "aH" ^ " " ^ " with " ^ get_domain_desc dom )
;;

let parse_aM env options =
  let defaults = Util.list2mappe [ "d", "TE"; "i", "s" ] in
  let dom = ParseParams.get_option options defaults "d" (fun x -> x) in
  let imp_algorithm =
    ParseParams.get_option options defaults "i" (function
      | "s" -> true
      | _ -> false)
  in
  ...
  VerifUtil.Analysis
    (An.analyze imp_algorithm, get_short_stratdesc "aM" ^ " with " ^ get_domain_desc dom)
;;
...
let parse_aLP env options =
  ...
  let imp_algorithm =
    ParseParams.get_option options defaults "i" (function
      | "s" -> Stratiter2.SymbRows
      | "m" -> Stratiter2.MicroIter
      | "g" -> Stratiter2.NoMicroGenIter
      | "n" -> Stratiter2.NoMicroIter
      | _ -> Stratiter2.TryImpStrat)
  in
  let module Dom = (val parse_template_domain env dom : Template.TEMPLATE_T) in
  let module An = Stratiter2.PowLognum (Dom) in
  VerifUtil.Analysis
    (An.analyze imp_algorithm, get_short_stratdesc "aLP" ^ " with " ^ get_domain_desc dom)
;;
...
let str_to_strategy env str =
  ...
  if Util.list_is_empty ss then raise (InvalidStrategy "empty strategy. nothing to do");
  List.map
    (fun s ->
       let p =
         try ParseParams.parse_param s with
         | Exit | Parse.Lex_error | Parsing.Parse_error -> raise (InvalidStrategy s)
       in
       match p with
       (********** PARTITIONING ***********************************************)
       | "pIF", _ -> VerifUtil.Trans (TransGen.initfinal, get_short_stratdesc "pIF")
       | "pE", options ->
         VerifUtil.Trans (build_call_pE env options, get_short_stratdesc "pE")
       | "pM", options ->
         VerifUtil.Trans (build_call_pM env options, get_short_stratdesc "pM")
       | "pMD", options ->
         VerifUtil.Trans (build_call_pMD env options, get_short_stratdesc "pMD")
       | "pMHB", options ->
         VerifUtil.Trans (build_call_pMHB env options, get_short_stratdesc "pMHB")
       | "pMHN", options -> build_call_pMHN env options
       | "pQ", _ ->
         VerifUtil.Trans (TransGen.enumerate env.Env.cont_q_vars, get_short_stratdesc "pQ")
       | "pS", _ -> VerifUtil.Trans (TransHybrid.convex_staycond, get_short_stratdesc "pS")
       | "tR", _ -> VerifUtil.Trans (RelAbstr.transform, get_short_stratdesc "tR")
       | "pB", _ -> VerifUtil.Trans (TransGen.boolbacksim, get_short_stratdesc "pB")
       (********** PREPROCESSING ***********************************************)
       | "rT", _ -> VerifUtil.Trans (TransGen.refine_by_destloc, get_short_stratdesc "rT")
       | "rAB", options ->
         VerifUtil.Trans (build_call_rAB env options, get_short_stratdesc "rAB")
       | "rAS", _ -> VerifUtil.Trans (TransAcc.split_arcs, get_short_stratdesc "rAS")
       | "rAF", _ -> VerifUtil.Trans (TransAcc.flatten_loops, get_short_stratdesc "rAF")
       | "rAD", _ -> VerifUtil.Trans (TransAcc.decouple_equs, get_short_stratdesc "rAD")
       | "rAI", _ -> VerifUtil.Trans (TransAcc.inputize, get_short_stratdesc "rAI")
       | "rB", _ -> VerifUtil.Trans (TransGen.remove_bool_inputs, get_short_stratdesc "rB")
       | "rS", _ -> VerifUtil.Trans (TransGen.split_arcs, get_short_stratdesc "rS")
       (********** ANALYSIS ***********************************************)
       (********** discrete ***********************************************)
...
let display_cfg_size cfg =
  ...
  Log.info
    logger
    ("CFG (" ^ string_of_int cntv ^ " location(s), " ^ string_of_int cnth ^ " arc(s)")
;;
...
let run env strategy cfprog =
  let rec do_run strats cfprog get_anres =
    ...
    | VerifUtil.Analysis (analysis, description) :: tailstrats ->
      ...
      let result, refine_loc, print_result, get_anres = analysis env cfprog in
      Log.info_o
        logger
        Format.pp_print_bool
        ("analysis '" ^ description ^ "' returned ")
        result;
      ...
      if
        ...
        ||
        match tailstrats with
        | [] -> true
        | _ -> false
      then (
        if !print_overall then VerifUtil.print_overall_reach env cfprog get_anres;
        result, get_anres, cfprog)
      else (
        (* Env.cudd_reorder env; *)
        let cfprog = VerifUtil.refine env cfprog refine_loc in
        display_cfg_size cfprog.Program.c_cfg;
        Log.debug_o logger (Cfg.print_short env) "Refined CFG: " cfprog.Program.c_cfg;
        do_run tailstrats cfprog get_anres)
  in
