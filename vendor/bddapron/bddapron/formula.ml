...
module O = struct
  module Expr0 = struct
    module Bool = struct
      let descend env cond (condition : 'a Expr0.Bool.t) =
      ...
      let to_lconjunction env cond (condition : 'a Expr0.Bool.t)
      ...
      let forget man env cond (condition : 'a Expr0.Bool.t) lvar : 'a Expr0.Bool.t =
        ...
        let ores =
          Bdd.Decompose.descend
            ...
            ~cube_of_down:(fun (careset, condition, abs) ->
              Cudd.Bdd.cube_of_bdd condition)
            ...
            ~terminal:(fun ~depth ~newcube ~cube ~down ->
              ...
              let nabs =
                Apron.Abstract0.forget_array apron elt.Bddleaf.leaf tadim false
              in
                ...
                Array.fold_left
                  (fun res tcons0 ->
                     ...
                     let bdd =
                       ...
                       | `Bool b ->
                         (if b then Cudd.Bdd.dtrue else Cudd.Bdd.dfalse) env.cudd
                       | `Cond condition ->
            ...
            ~down:
              ...
              , let dim = Apron.Environment.dimension env.ext.eapron in
                ...
                ; Bddleaf.leaf =
                    Apron.Abstract0.top apron dim.Apron.Dim.intd dim.Apron.Dim.reald
                } )
        in
        let res =
          match ores with
  ...
  module Expr1 = struct
    module Bool = struct
      let to_lconjunction cond condition =
        ...
        List.map
          (fun (e1, e2) -> Bdd.Env.make_value env e1, Bdd.Env.make_value env e2)
          lconjunction0
      ;;

      let forget man cond condition lvar =
        let env = condition.Bdd.Env.env in
        let res0 = Expr0.Bool.forget man env cond condition.Bdd.Env.val0 lvar in
