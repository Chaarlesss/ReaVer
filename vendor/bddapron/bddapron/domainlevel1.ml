...
module type Level0 = sig
  ...
  val print
    :  ?print_apron:((int -> string) -> Format.formatter -> 'b Apron.Abstract0.t -> unit)
    -> 'a Env.t
    -> Format.formatter
    -> 'b t
    -> unit
  ...
  val of_bddapron
    :  ('a, 'b) man
    -> 'a Env.t
    -> ('a Expr0.Bool.t * 'b Apron.Abstract0.t) list
    -> 'b t
...
module Make (Level0 : Level0) :
  Level1 with type ('a, 'b) man = ('a, 'b) Level0.man and type 'b t0 = 'b Level0.t =
struct
  ...
  let of_apron man env abs1 =
    ...
    if not (Apron.Environment.equal eapron abs1.Apron.Abstract1.env)
    then
      failwith
        "Bddapron.domainlevel1.of_apron: the APRON environment of the APRON abstract \
         value is different from the numerical part of the BDDAPRON environment";
    make_value env (Level0.of_apron man env abs1.Apron.Abstract1.abstract0)
  ;;
    ...
    let lboolabs0 =
      List.map
        (fun (boolexpr1, abs1) ->
           ...
           if not (Env.is_eq env env1)
           then
             failwith
               "Bddapron.domainlevel1.of_bddapron: the BDDAPRON environment of one \
                Boolean expression is different from the expected environment";
           if not (Apron.Environment.equal eapron env1.ext.eapron)
           then
             failwith
               "Bddapron.domainlevel1.of_apron: the APRON environment of one APRON \
                abstract value is different from the numerical part of the expected \
                BDDAPRON environment";
           Expr1.Bool.to_expr0 boolexpr1, abs1.Apron.Abstract1.abstract0)
        lboolabs1
    in
    make_value env (Level0.of_bddapron man env lboolabs0)
  ;;
  ...
  let to_bddapron man t =
    ...
    List.map
      (fun (bdd, abs0) ->
         ...
         , { Apron.Abstract1.env = eapron; Apron.Abstract1.abstract0 = abs0 } ))
      list0
  ;;

  let meet man t1 t2 = Env.mapbinop (Level0.meet man) t1 t2
  let join man t1 t2 = Env.mapbinop (Level0.join man) t1 t2
  let widening man t1 t2 = Env.mapbinop (Level0.widening man) t1 t2
    ...
    if
      not
        (Apron.Environment.equal
           (Env.apron t1.Env.env)
           tlincons1.Apron.Lincons1.array_env)
    then
      failwith "Bddapron.DomainLevel1.widening_threshold: incompatible APRON environments";
    Env.make_value
      t1.Env.env
      (Level0.widening_threshold
         man
         t1.Env.val0
         t2.Env.val0
         tlincons1.Apron.Lincons1.lincons0_array)
  ;;

  let meet_condition man cond t condition =
    let condition0 = Bdd.Domain1.O.check_value Expr0.O.Bool.permute condition t.env in
    make_value t.env (Level0.meet_condition man t.env cond t.val0 condition0)
  ;;

  let meet_condition2 man t condition2 =
    let condition0 =
      Bdd.Domain1.O.check_value Expr0.O.Bool.permute condition2.val1 t.env
    in
    make_value t.env (Level0.meet_condition man t.env condition2.cond t.val0 condition0)
  ;;

  let assign_lexpr ?relational ?nodependency man cond t lvar lexpr odest =
  ...
  let assign_listexpr2 ?relational ?nodependency man t lvar listexpr2 odest =
    let lexpr0 = Bdd.Domain1.O.check_value Expr0.O.permute_list listexpr2.val1 t.env in
    let odest0 = Env.check_ovalue t.env odest in
    make_value
      t.env
      (Level0.assign_lexpr
  ...
  let substitute_lexpr man cond t lvar lexpr odest =
    let lexpr0 = Bdd.Domain1.O.check_lvalue Expr0.O.permute lexpr t.env in
    let odest0 = Env.check_ovalue t.env odest in
    make_value t.env (Level0.substitute_lexpr man t.env cond t.val0 lvar lexpr0 odest0)
  ;;

  let substitute_listexpr2 man t lvar listexpr2 odest =
    let lexpr0 = Bdd.Domain1.O.check_value Expr0.O.permute_list listexpr2.val1 t.env in
    let odest0 = Env.check_ovalue t.env odest in
    make_value
      t.env
      (Level0.substitute_lexpr man t.env listexpr2.cond t.val0 lvar lexpr0 odest0)
  ;;

  let forget_list man t lvar = make_value t.env (Level0.forget_list man t.env t.val0 lvar)
  ...
  let change_environment man t nenv =
    ...
    then t
      ...
      make_value nenv (Level0.apply_change ~bottom man t.val0 change))
  ;;
  ...
  let rename man t lvarvar =
    ...
    make_value nenv (Level0.apply_permutation man t.val0 perm)
  ;;
end
