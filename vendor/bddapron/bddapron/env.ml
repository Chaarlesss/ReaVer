...
let copy_ext ~copy_aext ext = { ext with aext = copy_aext ext.aext }

module O = struct
  type ('a, 'b, 'c, 'd) t = ('a, 'b, 'c, 'd) t0
    constraint 'b = [> 'a typ ] constraint 'c = [> 'a typdef ]

  let make ~symbol ~copy_aext ?bddindex0 ?bddsize ?relational cudd aext =
    let env =
      Bdd.Env.O.make
        ~symbol
  ...
  let print_ext print_aext fmt ext =
    Format.fprintf
      fmt
      "{@[<v>eapron = %a;@ aext = %a@]}"
      (fun fmt x -> Apron.Environment.print fmt x)
      ext.eapron
      print_aext
      ext.aext
  ;;
  ...
  let print print_typ print_typdef print_aext fmt env =
    Bdd.Env.O.print print_typ print_typdef (print_ext print_aext) fmt env
  ;;
end
...
let make ~symbol ?bddindex0 ?bddsize ?relational cudd =
  O.make ~symbol ~copy_aext:(fun () -> ()) ?bddindex0 ?bddsize ?relational cudd ()
;;
...
let vars env =
  ...
  let add ap_var set =
    PSette.add (env.symbol.unmarshal (Apron.Var.to_string ap_var)) set
  in
...
let add_vars_with env ?booking_factor ?packing lvartyp : int array option =
  ...
  let integer, real =
    List.fold_left
      (fun ((integer, real) as acc) (var, typ) ->
  ...
  then
    env.ext.eapron
    <- Apron.Environment.add env.ext.eapron (Array.of_list integer) (Array.of_list real);
  operm
;;
...
let remove_vars_with env lvar : int array option =
  let arith =
    List.fold_left
      (fun acc var ->
         ...
         | `Int | `Real -> Apron.Var.of_string (env.symbol.marshal var) :: acc
         | _ -> acc)
      []
      lvar
  in
  ...
  if arith <> []
  then env.ext.eapron <- Apron.Environment.remove env.ext.eapron (Array.of_list arith);
  operm
;;

let rename_vars_with env lvarvar : int array option * Apron.Dim.perm option =
  let lvar1, lvar2 =
    List.fold_left
      (fun ((lvar1, lvar2) as acc) (var1, var2) ->
...
let lce env1 env2 =
  ...
  if not (env == env1 || env == env2)
  then env.ext.eapron <- Apron.Environment.lce env1.ext.eapron env2.ext.eapron;
  env
;;
...
let compute_change env1 env2 =
  ...
  let capron = Apron.Environment.dimchange2 env1.ext.eapron env2.ext.eapron in
  { cbdd; capron }
;;
...
let string_of_aprondim (env : ('a, 'b, 'c, 'd) O.t) =
  Print.string_of_print (fun fmt dim -> env.symbol.print fmt (var_of_aprondim env dim))
;;
