...
let texpr_support (cond : ('a, 'b) Cond.O.t) (texpr : 'a Expr0.t array) =
  Array.fold_left
    (fun res expr ->
       let supp =
         Cudd.Bdd.support_inter cond.supp (Expr0.O.support_cond cond.Bdd.Cond.cudd expr)
       in
...
let split_lvar symbol (lvar : 'a list) (lexpr : 'a Expr0.t list)
...
let split_texpr (texpr : 'a Expr0.t array)
  ...
  Array.iter
    (fun expr ->
       match expr with
       | #Bdd.Expr0.t as e -> lbexpr := e :: !lbexpr
       | `Apron e -> laexpr := e :: !laexpr)
    texpr;
  !lbexpr, Array.of_list !laexpr
;;

let split_lvarlexpr symbol (lvar : 'a list) (lexpr : 'a Expr0.t list)
  : 'a list * Cudd.Man.v Bdd.Expr0.t list * Apron.Var.t array * 'a ApronexprDD.t array
  =
  let lbvar = ref [] in
  let lavar = ref [] in
  let lbexpr = ref [] in
...
let cofactors
  ...
  then (
    ...
    Cudd.Mtbddc.cofactor t bdd, Cudd.Mtbddc.cofactor t (Cudd.Bdd.dnot bdd))
  else (
...
let rec descend_mtbdd
  ...
  =
  if ApronDD.is_bottom man t
  then t
  else (
    let supp = texpr_support cond texpr in
    let res =
      if Cudd.Bdd.is_cst supp
      then f t texpr
      else (
