...
module O = struct
  ...
  let bottom man env =
    ApronDD.bottom ~cudd:env.cudd man (Apron.Environment.dimension env.ext.eapron)
  ;;

  let top man env =
    ApronDD.top ~cudd:env.cudd man (Apron.Environment.dimension env.ext.eapron)
  ;;

  let of_apron man env abs0 = ApronDD.cst ~cudd:env.cudd man abs0
  ...
  let to_bddapron (man : ('a, 'b) man) (x : 'b t) =
    ...
    Array.fold_left
      (fun res ((bdd, abs) as pair) ->
         if Apron.Abstract0.is_bottom man.apron abs then res else pair :: res)
      []
      tab
  ;;
  ...
  let of_bddapron man env (lbddabs0 : ('a Expr0.Bool.t * 'b Apron.Abstract0.t) list) =
    ...
    List.fold_left
      (fun res (bdd, abs0) ->
         join man res (Cudd.Mtbddc.ite bdd (of_apron man env abs0) bot))
      bot
      lbddabs0
  ;;
  ...
  let meet_idcondb
    ...
    if PMappe.mem idcond env.idcondvar
    then (
      ...
      (fun t texpr ->
         ...
         let res =
           ApronDD.asssub_texpr_array
             ~asssub_bdd:(fun bdd ->
               Bdd.Domain0.O.assign_lexpr ?relational ?nodependency env bdd lbvar lbexpr)
             ApronDD.Assign
             env.symbol
             man
             eapron
  ...
  let forget_list (man : ('a, 'b) man) env (t : 'b t) (lvar : 'a list) =
    ...
    then t
      ...
      then ApronDD.forget_array man t tadim
      else if tadim = [||]
      then ApronDD.exist man ~supp t
      else (
        let op2 = ApronDD.make_op_join man in
        let op1 =
          Cudd.User.make_op1 (fun tu ->
  ...
  let forall_bool_list (man : ('a, 'b) man) env (t : 'b t) (lvar : 'a list) =
    ...
    then t
      ...
      then
        failwith
          "Bddapron.mt.forall_bool_list: numerical variables in universal quantification"
      else if Cudd.Bdd.is_true supp
      then t
      else ApronDD.forall man ~supp t)
  ;;

  let apply_change ~bottom man t change =
    if change.cbdd.intro = None && change.cbdd.remove = None
    then ApronDD.apply_dimchange2 man t change.capron false
    else if change.capron.Apron.Dim.add = None && change.capron.Apron.Dim.remove = None
    then (
      let bdd = change.cbdd in
      let t =
        match bdd.intro with
        | None -> t
      ...
      let mtbdd =
        Cudd.Mapleaf.expansivemapleaf1
          ~default:bottom
          ...
          (fun guard absu ->
             ...
             let nabs =
               Apron.Abstract0.apply_dimchange2 man.apron abs change.capron false
             in
  ...
  let apply_permutation man (t : 'b t) (operm, oapronperm) =
    let res =
      ...
      | None -> t
      | Some apronperm -> ApronDD.permute_dimensions man t apronperm
    in
    let res =
      match operm with
      | None -> res
      | Some perm -> Cudd.Mtbddc.permute res perm
    in
    res
  ;;
end
