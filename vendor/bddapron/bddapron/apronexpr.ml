...
let mpqf_of_coeff coeff =
  ...
  | Apron.Coeff.Interval _ ->
    raise
      (Invalid_argument
         "unable to convert an APRON interval coefficient into a Bddapron.Apronexpr \
          scalar coefficient")
;;
...
let reduce_list (list : (Mpqf.t * 'a) list) : Mpqf.t * (Mpqf.t * 'a) list =
  ...
  Mpz.abs gcd gcd;
  List.iter (fun coeff -> Mpz.gcd gcd gcd coeff) (List.tl ncoeffs);
  let factor = Mpqf.of_mpz2 lcm gcd in
  let nlist =
    List.map2
      (fun coeff (_, something) ->
...
module Lin = struct
  ...
  let is_dependent_on_integer_only typ_of_var (e : 'a t) =
    List.for_all (fun (coeff, var) -> typ_of_var var = `Int) e.lterm
  ;;
  ...
  let support man (e : 'a t) : 'a PSette.t =
    List.fold_left
      (fun res (_, var) -> PSette.add var res)
  ...
  let substitute_by_var man e (substitution : ('a, 'a) PMappe.t) =
    let ne =
      ...
      ; lterm =
          List.map
            (fun ((c, v) as x) ->
               try c, PMappe.find v substitution with
               | Not_found -> x)
            e.lterm
      }
    in
    ...
    then
      List.iter
        (fun (coeff, var) ->
           ...
           assert (sgn <> 0);
           if sgn > 0 then if not !first then Format.pp_print_string fmt "+";
           if not !first then Format.fprintf fmt "@,";
           if Mpqf.cmp_int coeff (-1) = 0
           then Format.pp_print_string fmt "-"
           else if Mpqf.cmp_int coeff 1 <> 0
           then Mpqf.print fmt coeff;
           man.print fmt var;
           first := false)
        expr.lterm;
    let sgn = Mpqf.sgn expr.cst in
    if !first || sgn < 0
    then Mpqf.print fmt expr.cst
    else if sgn > 0
    then fprintf fmt "+%a" Mpqf.print expr.cst;
    fprintf fmt "@]"
  ;;
  ...
  let var (x : 'a) = { cst = Mpqf.of_int 0; lterm = [ Mpqf.of_int 1, x ] }
  let cst (x : Mpqf.t) = { cst = x; lterm = [] }
  let zero = cst (Mpqf.of_int 0)
  let one = cst (Mpqf.of_int 1)

  let rec add_lterm man l1 l2 =
    match l1, l2 with
    | l, [] | [], l -> l
    | (c1, v1) :: r1, (c2, v2) :: r2 ->
      let cmp = man.compare v1 v2 in
      if cmp < 0
      then (c1, v1) :: add_lterm man r1 l2
      else if cmp > 0
      then (c2, v2) :: add_lterm man l1 r2
      else (
        let c = Mpqf.add c1 c2 in
        if Mpqf.sgn c <> 0 then (c, v1) :: add_lterm man r1 r2 else add_lterm man r1 r2)
  ;;

  let scale f e =
    if Mpqf.sgn f = 0
    then { cst = Mpqf.of_int 0; lterm = [] }
    else
      { cst = Mpqf.mul f e.cst; lterm = List.map (fun (c, v) -> Mpqf.mul f c, v) e.lterm }
  ;;

  let negate_lterm l = List.map (fun (c, v) -> Mpqf.neg c, v) l
  let negate e = { cst = Mpqf.neg e.cst; lterm = negate_lterm e.lterm }

  let add man e1 e2 =
    { cst = Mpqf.add e1.cst e2.cst; lterm = add_lterm man e1.lterm e2.lterm }
  ;;

  let rec sub_lterm man l1 l2 =
    match l1, l2 with
    | l, [] -> l
    | [], l -> negate_lterm l
    | (c1, v1) :: r1, (c2, v2) :: r2 ->
      let cmp = man.compare v1 v2 in
      if cmp < 0
      then (c1, v1) :: sub_lterm man r1 l2
      else if cmp > 0
      then (Mpqf.neg c2, v2) :: sub_lterm man l1 r2
      else (
        let c = Mpqf.sub c1 c2 in
        if Mpqf.sgn c <> 0 then (c, v1) :: sub_lterm man r1 r2 else sub_lterm man r1 r2)
  ;;

  let sub man e1 e2 =
    { cst = Mpqf.sub e1.cst e2.cst; lterm = sub_lterm man e1.lterm e2.lterm }
  ;;

  let normalize_as_constraint e =
    if e.lterm = []
    then raise (Constant (Mpqf.sgn e.cst))
    else (
      let factor, lterm = reduce_list e.lterm in
      { cst = Mpqf.mul factor e.cst; lterm })
  ;;
  ...
  let to_linexpr1 man (env : Apron.Environment.t) e : Apron.Linexpr1.t =
    ...
    let list =
      List.map
        (fun (mpqf, name) ->
           Apron.Coeff.s_of_mpqf mpqf, Apron.Var.of_string (man.marshal name))
        e.lterm
    in
    Apron.Linexpr1.set_list
      res
      list
      (if Mpqf.sgn e.cst = 0 then None else Some (Apron.Coeff.s_of_mpqf e.cst));
    res
  ;;
  ...
  let of_linexpr0 man env e =
    of_linexpr1 man { Apron.Linexpr1.linexpr0 = e; Apron.Linexpr1.env }
  ;;
end
...
module Poly = struct
  ...
  let is_dependent_on_integer_only typ_of_var (e : 'a t) =
    List.for_all
      (fun (c, mon) -> List.for_all (fun (var, exp) -> typ_of_var var = `Int) mon)
      e
  ;;

  let print_varexp man fmt (var, exp) =
    man.print fmt var;
    if exp <> 1 then fprintf fmt "^%i" exp;
    ()
  ;;

  let print_monomial man fmt lvarexp =
    Print.list ~first:"" ~sep:"." ~last:"" (print_varexp man) fmt lvarexp
  ;;
  ...
  let compare_varexp man (v1, n1) (v2, n2) =
    ...
    if res <> 0 then res else n1 - n2
  ;;

  let rec compare_monomial man m1 m2 =
    match m1, m2 with
    | v1 :: r1, v2 :: r2 ->
      let res = compare_varexp man v1 v2 in
      if res <> 0 then res else compare_monomial man r1 r2
    | v1 :: r1, [] -> 1
    | [], v1 :: r1 -> -1
    | [], [] -> 0
  ;;
  ...
  let normalize_monomial man m =
    let nm = List.filter (fun (v, n) -> if n < 0 then raise Exit else n > 0) m in
  ...
  let substitute_by_var man (e : 'a t) (substitution : ('a, 'a) PMappe.t) : 'b t =
    let rename_varexp ((var, exp) as varexp) : 'a varexp =
      try PMappe.find var substitution, exp with
      | Not_found -> varexp
    in
    ...
    in
    let rename_term (c, mon) : 'a term = c, rename_monomial mon in
  ...
  let support man (e : 'a t) : 'a PSette.t =
    List.fold_left
      (fun res (_, monomial) ->
         List.fold_left (fun res (var, _) -> PSette.add var res) res monomial)
      (PSette.empty man.compare)
      e
  ;;
  ...
  let negate e = List.map (fun (c, m) -> Mpqf.neg c, m) e

  let rec add man l1 l2 =
    match l1, l2 with
    | l, [] | [], l -> l
    | (c1, m1) :: r1, (c2, m2) :: r2 ->
      let cmp = compare_monomial man m1 m2 in
      if cmp < 0
      then (c1, m1) :: add man r1 l2
      else if cmp > 0
      then (c2, m2) :: add man l1 r2
      else (
        let c = Mpqf.add c1 c2 in
        if Mpqf.sgn c <> 0 then (c, m1) :: add man r1 r2 else add man r1 r2)
  ;;

  let rec sub man l1 l2 =
    match l1, l2 with
    | l, [] -> l
    | [], l -> negate l
    | (c1, m1) :: r1, (c2, m2) :: r2 ->
      let cmp = compare_monomial man m1 m2 in
      if cmp < 0
      then (c1, m1) :: sub man r1 l2
      else if cmp > 0
      then (Mpqf.neg c2, m2) :: sub man l1 r2
      else (
        let c = Mpqf.sub c1 c2 in
        if Mpqf.sgn c <> 0 then (c, m1) :: sub man r1 r2 else sub man r1 r2)
  ;;

  let rec mul_monomial man m1 m2 =
    match m1, m2 with
    | [], l | l, [] -> l
    | (v1, n1) :: r1, (v2, n2) :: r2 ->
      let cmp = man.compare v1 v2 in
      if cmp < 0
      then (v1, n1) :: mul_monomial man r1 m2
      else if cmp > 0
      then (v2, n2) :: mul_monomial man m1 r2
  ...
  let scale man ((coeff, mon) : Mpqf.t * 'a monomial) (l : 'a t) =
    ...
    then []
      ...
      let res = List.map (fun (c, m) -> Mpqf.mul coeff c, mul_monomial man mon m) l in
      normalize man res)
  ;;

  let mul man (l1 : 'a t) (l2 : 'a t) =
    List.fold_left (fun res cm -> add man res (scale man cm l2)) [] l1
  ;;
  ...
  let normalize_as_constraint l =
    let cst, l =
      match l with
      | [] -> raise (Constant 0)
      | (c, []) :: r -> if r = [] then raise (Constant (Mpqf.sgn c)) else c, r
      | _ as l -> Mpqf.of_int 0, l
    in
    let factor, nl = reduce_list l in
    if Mpqf.sgn cst = 0 then nl else (Mpqf.mul factor cst, []) :: nl
  ;;
end
...
module Tree = struct
  ...
  type binop = Apron.Texpr1.binop =
    ...
    | Div
    | Mod
    | Pow (** Destination type for rounding *)
  ...
  let add ?(typ = Real) ?(round = Rnd) e1 e2 =
    if is_zero e1 then e2 else if is_zero e2 then e1 else Binop (Add, e1, e2, typ, round)
  ;;
  ...
  let mul ?(typ = Real) ?(round = Rnd) e1 e2 =
    ...
    then e2
    else if equal_int e1 1
    then e2
    else if equal_int e2 1
    then e1
    else if equal_int e1 (-1)
    then negate e2
    else if equal_int e2 (-1)
  ...
  let substitute_by_var e (substitution : ('a, 'a) PMappe.t) =
    let rec parcours = function
      | Cst _ as x -> x
      | Var var as x ->
        (try Var (PMappe.find var substitution) with
         | Not_found -> x)
  ...
  let rec of_expr man = function
    ...
    | Apron.Texpr1.Var var -> Var (man.unmarshal (Apron.Var.to_string var))
    | Apron.Texpr1.Unop (op, e, t, r) -> Unop (op, of_expr man e, t, r)
  ...
  let rec to_expr man = function
    ...
    | Var var -> Apron.Texpr1.Var (Apron.Var.of_string (man.marshal var))
    | Unop (op, e, t, r) -> Apron.Texpr1.Unop (op, to_expr man e, t, r)
    | Binop (op, e1, e2, t, r) ->
    ...
    let rec compare x y =
      ...
      | Binop (op1, ea1, eb1, t1, r1), Binop (op2, ea2, eb2, t2, r2) ->
        ...
        then res
          ...
          if res <> 0 then res else compare eb1 eb2)
      | Binop _, _ -> 1
    in
...
let rec lin_of_poly man (p : 'a Poly.t) : 'a Lin.t =
  match p with
  | (c, m) :: r ->
    let lexpr =
      match m with
...
let rec lin_of_tree man (x : 'a Tree.t) : 'a Lin.t =
  ...
  | Tree.Binop (op, e1, e2, t, r) ->
    ...
    (match op with
     ...
     | Tree.Mul ->
       if l1.Lin.lterm = []
       then Lin.scale l1.Lin.cst l2
       else if l2.Lin.lterm = []
       then Lin.scale l2.Lin.cst l1
       else raise Exit
     | Tree.Div ->
       if l2.Lin.lterm = [] then Lin.scale (Mpqf.inv l2.Lin.cst) l1 else raise Exit
     | Tree.Mod ->
       if
         l1.Lin.lterm = []
         && l2.Lin.lterm = []
         && Mpzf.cmp_int (Mpqf.get_den l1.Lin.cst) 1 = 0
         && Mpzf.cmp_int (Mpqf.get_den l2.Lin.cst) 1 = 0
       then
         Lin.cst
           (Mpqf.of_mpz (Mpzf.gmod (Mpqf.get_num l1.Lin.cst) (Mpqf.get_num l1.Lin.cst)))
       else raise Exit
     | Tree.Pow -> raise Exit)
  | _ -> raise Exit
;;
...
let tree_of_lin (lin : 'a Lin.t) : 'a Tree.t =
  List.fold_left
    (fun res (c, v) ->
       Tree.add res (Tree.mul (Tree.Cst (Apron.Coeff.s_of_mpqf c)) (Tree.Var v)))
...
let tree_of_poly (poly : 'a Poly.t) : 'a Tree.t =
  ...
  List.fold_left
    (fun res (c, mon) ->
       Tree.add res (Tree.mul (Tree.Cst (Apron.Coeff.s_of_mpqf c)) (tree_of_monomial mon)))
...
let is_dependent_on_integer_only typ_of_var expr =
  match expr with
...
let normalize man expr =
  ...
  | Lin _ -> expr
  | Poly p ->
    (try Lin (lin_of_poly man p) with
     | Exit -> expr)
  | Tree t ->
    (try
...
let var man typ_of_var x =
  ...
  | _ ->
    failwith
      (Print.sprintf
         "Arith.var: reference %a undeclared or of wrong type in environment"
         man.print
         x)
;;
...
let one = Lin Lin.one
let cst (x : Apron.Coeff.t) = Lin (Lin.cst (mpqf_of_coeff x))
...
let cast ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e =
  if typ = Apron.Texpr1.Real
  then e
  else Tree (Tree.Unop (Tree.Cast, to_tree e, typ, round))
;;
...
(* Assume normalized expressions *)
let normalize_as_constraint expr =
  match expr with
...
let of_linexpr1 man linexpr1 = Lin (Lin.of_linexpr1 man linexpr1)

let of_linexpr0 man env linexpr0 =
  of_linexpr1 man { Apron.Linexpr1.linexpr0; Apron.Linexpr1.env }
;;
...
let of_texpr0 man env texpr0 = of_texpr1 man { Apron.Texpr1.texpr0; Apron.Texpr1.env }
...
let to_apron1 man (env : Apron.Environment.t) (expr : 'a t) =
  match expr with
  | Lin e -> `Lin (Lin.to_linexpr1 man env e)
  | _ -> `Tree (to_texpr1 man env expr)
;;

let to_apron0 man (env : Apron.Environment.t) expr =
  match expr with
...
let extract_fstcoeff expr =
  ...
  | Lin e ->
    if e.Lin.lterm <> []
    then fst (List.hd e.Lin.lterm)
    else if Mpqf.sgn e.Lin.cst <> 0
    then e.Lin.cst
    else Mpqf.of_int 0
  | Poly lterm ->
    (match lterm with
     | (_, []) :: term :: _ | term :: _ -> fst term
     | [] -> Mpqf.of_int 0)
  | Tree expr ->
    let rec parcours = function
      | Tree.Cst coeff ->
        if Apron.Coeff.is_zero coeff
        then None
        else (
          let sgn =
            match coeff with
            ...
            | Apron.Coeff.Interval itv ->
              ...
              if sgn <> 0 then sgn else Apron.Scalar.sgn itv.Apron.Interval.sup
          in
          assert (sgn <> 0);
...
let modify_cst expr cst =
  match expr with
  | Lin e -> Lin { Lin.cst; Lin.lterm = e.Lin.lterm }
  | Poly e ->
    let s = Mpqf.sgn cst in
    Poly
      (match e with
       | (_, []) :: r -> if s <> 0 then (cst, []) :: r else r
       | _ as r -> if s <> 0 then (cst, []) :: r else r)
  | _ -> raise (Invalid_argument "")
;;
...
module Condition = struct
  ...
  let print_typ fmt typ = pp_print_string fmt (Apron.Lincons0.string_of_typ typ)
  ...
  let print man fmt =
    ...
    | typ, Lin { Lin.lterm = [ (coeff, var) ]; Lin.cst } ->
      let c, op, cst =
        if Mpqf.sgn coeff > 0
        then coeff, Apron.Lincons0.string_of_typ typ, Mpqf.neg cst
        else Mpqf.neg coeff, revcmp typ, cst
      in
      fprintf fmt "%a%a%s%a" print_coeff c man.print var op Mpqf.print cst
    | typ, expr -> fprintf fmt "%a%a0" (print man) expr print_typ typ
  ;;
  ...
  let normalize typ_of_var (cons : 'a t) : [ `Cond of 'a t | `Bool of bool ] =
    ...
    let cons =
      ...
      | EQ | DISEQ ->
        ...
        let sgn = Mpqf.sgn coeff in
        if sgn < 0 then typ, negate expr else cons
      | _ -> cons
    in
    match expr with
    | Tree _ -> `Cond cons
    | _ ->
      if is_dependent_on_integer_only typ_of_var expr
      then (
        let cst = extract_cst expr in
        let is_integer = Mpzf.cmp_int (Mpqf.get_den cst) 1 = 0 in
        match typ, is_integer with
        | EQ, false -> `Bool false
        | DISEQ, false -> `Bool true
        | SUPEQ, false | SUP, false ->
          let ncst = Mpzf.fdiv_q (Mpqf.get_num cst) (Mpqf.get_den cst) in
  ...
  let negate typ_of_var (cons : 'a t) =
    let ncons =
      ...
      | SUPEQ, expr -> normalize typ_of_var (SUP, negate expr)
      | SUP, expr -> normalize typ_of_var (SUPEQ, negate expr)
      | EQMOD _, _ -> failwith ""
    in
    match ncons with
    | `Bool _ -> failwith ""
  ...
  let compare man (t1, e1) (t2, e2) : int =
    ...
    | _, _ ->
      if asgn >= 2
      then if sgn > 0 then 3 else -3
      else if asgn = 1
      then (
        match t1, t2 with
        | EQMOD _, _ | _, EQMOD _ -> failwith ""
        | DISEQ, _ -> 2 * sgn
        | EQ, DISEQ -> sgn
        | _, DISEQ -> 2 * sgn
        | _, EQ -> 2 * sgn
  ...
  let of_lincons0 man typ_of_var env lincons0 =
    of_lincons1 man typ_of_var { Apron.Lincons1.lincons0; Apron.Lincons1.env }
  ;;
  ...
  let of_tcons0 man typ_of_var env tcons0 =
    of_tcons1 man typ_of_var { Apron.Tcons1.tcons0; Apron.Tcons1.env }
  ;;

  let to_lincons1 man env (typ, expr) = Apron.Lincons1.make (to_linexpr1 man env expr) typ
  let to_lincons0 man env (typ, expr) = Apron.Lincons0.make (to_linexpr0 man env expr) typ
  let to_tcons1 man env (typ, expr) = Apron.Tcons1.make (to_texpr1 man env expr) typ
  let to_tcons0 man env (typ, expr) = Apron.Tcons0.make (to_texpr0 man env expr) typ
