...
let special_is_leq apron dd1 dd2 =
  if Cudd.Mtbddc.is_cst dd1 && Apron.Abstract0.is_bottom apron (mydval dd1)
  then Some true
  else if Cudd.Mtbddc.is_cst dd2 && Apron.Abstract0.is_bottom apron (mydval dd2)
  then Some false
  else None
;;
...
let make_global (apron : 'a Apron.Manager.t) (table : 'a table) : 'a global =
  ...
  { op_is_leq; op_join; op_meet; op_exist; op_forall }
;;

let make_man ?(global = false) (apron : 'a Apron.Manager.t) =
  let table = make_table apron in
  let oglobal = if global then Some (make_global apron table) else None in
  let man = { apron; table; oglobal } in
  man
;;
...
let print ?(print_apron = Apron.Abstract0.print) print_bdd string_of_dim fmt (t : 'a t) =
  if Cudd.Mtbddc.is_cst t
  then fprintf fmt "{ %a }" (print_apron string_of_dim) (mydval t)
  else (
    let nb = Cudd.Mtbddc.nbpaths t in
    if nb > float_of_int !Cudd.Man.print_limit
    then
      fprintf
        fmt
        "idd with %i nodes, %i leaves and %g paths"
        (Cudd.Mtbddc.size t)
        (Cudd.Mtbddc.nbleaves t)
        nb
    else (
      ...
      for i = Array.length leaves_u - 1 downto 0 do
        ...
        if not (Apron.Abstract0.is_bottom (Apron.Abstract0.manager leaf) leaf)
        then (
...
let cst ~cudd man abs : 'a t = Cudd.Mtbddc.cst cudd man.table abs

let bottom ~cudd man dim : 'a t =
  cst cudd man (Apron.Abstract0.bottom man.apron dim.Apron.Dim.intd dim.Apron.Dim.reald)
;;

let top ~cudd man dim : 'a t =
  cst cudd man (Apron.Abstract0.top man.apron dim.Apron.Dim.intd dim.Apron.Dim.reald)
;;
...
let is_eq (man : 'a man) = Cudd.Mtbddc.is_equal
...
let is_leq man (x : 'a t) (y : 'a t) =
  ...
  | None ->
    Cudd.User.map_test2
      ...
      x
      y
  | Some global -> Cudd.User.apply_test2 global.op_is_leq x y
;;
...
let join man (x : 'a t) (y : 'a t) : 'a t =
  ...
  | None ->
    Cudd.User.map_op2
      ...
      x
      y
  | Some global -> Cudd.User.apply_op2 global.op_join x y
;;
...
let meet man (x : 'a t) (y : 'a t) : 'a t =
  ...
  | None ->
    Cudd.User.map_op2
      ...
      x
      y
  | Some global -> Cudd.User.apply_op2 global.op_meet x y
;;
...
let widening man (x : 'a t) (y : 'a t) : 'a t =
  Cudd.User.map_op2
    ...
    (fun x y ->
       myunique man.table (Apron.Abstract0.widening man.apron (myget x) (myget y)))
    x
    y
;;
...
let widening_threshold man (x : 'a t) (y : 'a t) tlincons0 : 'a t =
  Cudd.User.map_op2
    ...
    (fun x y ->
       myunique
         man.table
         (Apron.Abstract0.widening_threshold man.apron (myget x) (myget y) tlincons0))
    x
    y
;;
...
let meet_tcons_array man (x : 'a t) tcons : 'a t =
  Cudd.User.map_op1
    (fun x ->
       myunique man.table (Apron.Abstract0.meet_tcons_array man.apron (myget x) tcons))
    x
;;
...
let forget_array man (x : 'a t) tdim : 'a t =
  Cudd.User.map_op1
    (fun x ->
       myunique man.table (Apron.Abstract0.forget_array man.apron (myget x) tdim false))
    x
;;
...
let permute_dimensions man (x : 'a t) perm : 'a t =
  Cudd.User.map_op1
    (fun x ->
       myunique man.table (Apron.Abstract0.permute_dimensions man.apron (myget x) perm))
    x
;;
...
let add_dimensions man (x : 'a t) change project : 'a t =
  Cudd.User.map_op1
    (fun x ->
       myunique
         man.table
         (Apron.Abstract0.add_dimensions man.apron (myget x) change project))
    x
;;
...
let apply_dimchange2 man (x : 'a t) change2 project =
  Cudd.User.map_op1
    (fun x ->
       myunique
         man.table
         (Apron.Abstract0.apply_dimchange2 man.apron (myget x) change2 project))
    x
;;
...
let fun_of_asssub asssub symbol man env org tdim texpr odest =
  ...
  let texpr =
    Array.map (fun expr -> Apronexpr.to_texpr0 symbol env (Cudd.Mtbdd.get expr)) texpr
  in
  let res =
    match asssub with
    | Assign ->
...
let asssub_texpr
      ...
      (org : 'a t)
      (dim : Apron.Dim.t)
      (expr : 'b ApronexprDD.t)
      (odest : 'a t option)
  : 'a t
  =
  let var =
    symbol.Bdd.Env.unmarshal (Apron.Var.to_string (Apron.Environment.var_of_dim env dim))
  in
  match odest with
  | None ->
    (* implies assign mode *)
    Cudd.User.map_op2
      ~special:(fun ddorg ddexpr ->
        ...
        then Some ddorg
          ...
          | Apronexpr.Lin
              { Apronexpr.Lin.cst; Apronexpr.Lin.lterm = [ (coeff, varexpr) ] }
            when Mpqf.sgn cst = 0
                 && Mpqf.cmp_int coeff 1 = 0
                 && symbol.Bdd.Env.compare var varexpr = 0 -> true
          | _ -> false
        then Some ddorg
        else None)
      (fun org expr -> fun_of_asssub asssub symbol man env org [| dim |] [| expr |] None)
      org
      expr
  | Some dest ->
    Cudd.User.map_op3
      ~special:(fun ddorg dddest ddexpr ->
        ...
        then Some ddorg
          ...
          Cudd.Mtbddc.is_cst dddest && Apron.Abstract0.is_bottom man.apron (mydval dddest)
        then Some dddest
        else None)
      (fun org dest expr ->
...
let asssub_texpr_array
  ...
  let combine =
    fun (bdd, res1) res2 -> join man (Cudd.Mtbddc.ite bdd res1 default) res2
  in
  let absorbant x = neutral_join (myget x) in
  let tabsorbant = Array.make (Array.length texpr) None in
  if Array.length tdim = 1 && asssub_bdd = None
  then asssub_texpr asssub symbol man env org tdim.(0) texpr.(0) odest
  else if texpr = [||]
  then (
    ...
    | Some f, None ->
      ...
      Cudd.Mapleaf.combineleaf1
        ~default
        ~combine
        (fun guard (org : 'a leaf Cudd.Mtbddc.unique) ->
           f guard, Cudd.Mtbddc.cst_u cudd org)
        org
    | Some f, Some dest ->
      let source =
        match asssub with
    ...
    | None, Some dest ->
      Cudd.Mapleaf.combineleaf2_array
        ~default
        ~combine
        ~absorbant1:absorbant
        ~absorbant2:absorbant
        ~tabsorbant
           ...
           | Assign ->
             ...
             let res =
               Cudd.Mapleaf.combineleaf1
                 ...
                 (fun guard dest ->
                    let res =
                      fun_of_asssub asssub symbol man env org tdim texpr (Some dest)
                    in
                    guard, Cudd.Mtbddc.cst_u cudd res)
                 dest
             in
...
let exist man ~(supp : Cudd.Man.v Cudd.Bdd.t) (t : 'a t) : 'a t =
  ...
  | None ->
    ...
    let exist = Cudd.User.make_exist ~memo:(Cudd.Memo.Hash (Cudd.Hash.create 2)) op2 in
    let res = Cudd.User.apply_exist exist ~supp t in
    Cudd.User.clear_op2 op2;
    Cudd.User.clear_exist exist;
    res
  | Some global -> Cudd.User.apply_exist global.op_exist ~supp t
;;
    ...
    | None ->
      ...
      let op2 =
        Cudd.User.make_op2
          ...
          ~idempotent:true
          ~special:(fun dd1 dd2 ->
            if Cudd.Mtbddc.is_equal dd1 bottomdd || Cudd.Mtbddc.is_equal dd2 bottomdd
            then Some bottomdd
            else None)
          (fun x y ->
             myunique man.table (Apron.Abstract0.join man.apron (myget x) (myget y)))
      in
      op2
    | Some global -> global.op_join
  in
  let existand =
    Cudd.User.make_existand ~memo:(Cudd.Memo.Hash (Cudd.Hash.create 3)) ~bottom op2
  in
...
let forall man ~(supp : Cudd.Man.v Cudd.Bdd.t) (t : 'a t) : 'a t =
  ...
  | None ->
    ...
    let forall = Cudd.User.make_exist ~memo:(Cudd.Memo.Hash (Cudd.Hash.create 2)) op2 in
    let res = Cudd.User.apply_exist forall ~supp t in
    Cudd.User.clear_op2 op2;
    Cudd.User.clear_exist forall;
    res
  | Some global -> Cudd.User.apply_exist global.op_forall ~supp t
;;
