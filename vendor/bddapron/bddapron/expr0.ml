...
module O = struct
  let print_bdd env cond fmt bdd =
    Bdd.Expr0.O.print_bdd
      ~print_external_idcondb:(fun fmt idb ->
  ...
  let check_typ2 env e2 e3 =
    ...
    then
      failwith
        (Print.sprintf
           "Expr.ite: 2 branches have different types %a and %a"
  ...
  let permute_list ?memo le tab =
    match memo with
    | Some memo -> List.map (fun x -> permute ~memo x tab) le
    | None ->
  ...
  let print env cond (fmt : Format.formatter) (expr : [< 'a t ]) =
    match expr with
  ...
  let conditions_support' { cudd } cond le =
    List.fold_left
      ...
      (Cudd.Bdd.dtrue cudd)
      le
    |> Cudd.Bdd.support_inter cond.supp
  ;;
  ...
  let fold_filter_conditions env cond le nsub handle_cond acc =
    List.fold_left
      ...
      acc
      (conditions_support' env cond le |> Cudd.Bdd.list_of_support)
  ;;

  let compose_of_lvarexpr env cond (le : 'a t list) (substitution : ('a * 'a t) list)
    : Cudd.Bdd.vt array option * ('a, 'a t) PMappe.t
    =
    if false
    then
      printf
        "compose_of_lvarexpr@   %a@."
        (Print.list (fun fmt (var, expr) ->
           fprintf
             fmt
             "%a <- %a@ "
             env.Bdd.Env.symbol.Bdd.Env.print
             var
             (print env cond)
             expr))
        substitution;
    ...
    let bsub, osub =
      List.fold_left
        (fun (bsub, osub) (var, expr) ->
           match expr with
           | #Bdd.Expr0.t as x -> (var, x) :: bsub, osub
           | _ -> bsub, PMappe.add var expr osub)
    ...
    let otab =
      if PMappe.is_empty osub
      then otab
      else
        (* we now take care of other conditions *)
        fold_filter_conditions
          env
          cond
          le
          osub
  ...
  let substitute_list ?memo env cond (le : 'a t list) (substitution : ('a * 'a t) list)
    : 'a t list
    =
    if substitution = [] || le = []
    then le
      ...
      let res =
        ...
        then (
          ...
          List.map
            (function
              | #Bdd.Expr0.t as x -> (Bdd.Expr0.O.compose ?memo x tab :> 'a t)
              | `Apron mtbdd -> `Apron (Cudd.Mtbdd.vectorcompose ?memo tab mtbdd))
            le)
        else
          List.map
            (fun e ->
  ...
  let substitute ?memo env cond (e : 'a t) (substitution : ('a * 'a t) list) : 'a t =
    List.hd (substitute_list ?memo env cond [ e ] substitution)
  ;;

  let var env cond (var : 'a) : 'a t =
    let typ = Env.typ_of_var env var in
    match typ with
    | #Bdd.Env.typ -> (Bdd.Expr0.O.var env var :> 'a t)
    | `Int | `Real -> `Apron (ApronexprDD.var env var)
    | _ -> raise Not_found
  ;;
  ...
  let substitute_by_var_list
    ...
    let bsub, nsub =
      List.partition
        (fun (v, _) ->
           match Env.typ_of_var env v with
           | #Bdd.Env.typ -> true
           | _ -> false)
        substitution
    in
    ...
    let nsub =
      List.fold_left
        (fun nsub (v, v') -> PMappe.add v v' nsub)
        (PMappe.empty symbol.compare)
        nsub
    in
    ...
    List.map
      (function
        | #Bdd.Expr0.t as e ->
          ((match operms with
            | None -> e
            | Some perms -> Bdd.Expr0.O.permute ?memo e perms)
            :> 'a t)
        | `Apron a ->
          ...
          let a =
            match operms with
            | None -> a
            | Some perms -> permute ?memo a perms
          in
          `Apron
            (if not num_sub
             then a
               ...
               fold_guardleaves
                 (fun g n ->
                    let n' = Apronexpr.substitute_by_var symbol n nsub in
                    ite g (ApronexprDD.of_apronexpr env n'))
                 a
                 (cst_u cudd (pick_leaf_u a))))
      le
  ;;

  let substitute_by_var ?memo env cond e (substitution : ('a * 'a) list) =
  ...
  module Bool = struct
    ...
    let substitute_by_var ?memo env cond (e : 'a t) sub =
      of_expr (ddsubstitute_by_var ?memo env cond (to_expr e) sub)
    ;;

    let substitute ?memo env cond (e : 'a t) sub =
      of_expr (ddsubstitute ?memo env cond (to_expr e) sub)
    ;;
  ...
  module Apron = struct
    ...
    let print env cond fmt x = ApronexprDD.print (print_bdd env cond) env.symbol fmt x
    ...
    let ite env cond b (t1 : 'a t) (t2 : 'a t) : 'a t = Cudd.Mtbdd.ite b t1 t2
  ...
  let eq env cond (e1 : 'a t) (e2 : 'a t) : 'a Bool.t =
    ...
    match t with
    | `Bool -> Bool.eq env cond (Bool.of_expr e1) (Bool.of_expr e2)
    | `Bint _ -> Bint.eq env cond (Bint.of_expr e1) (Bint.of_expr e2)
    | `Benum _ -> Benum.eq env cond (Benum.of_expr e1) (Benum.of_expr e2)
    | `Real ->
      let diff = Apron.sub env cond (Apron.of_expr e1) (Apron.of_expr e2) in
  ...
  let ite env cond (condition : 'a Bool.t) (e1 : 'a t) (e2 : 'a t) : 'a t =
    let t = check_typ2 env e1 e2 in
    match t with
    | `Bool -> `Bool (Bool.ite env cond condition (Bool.of_expr e1) (Bool.of_expr e2))
    | `Bint _ -> `Bint (Bint.ite env cond condition (Bint.of_expr e1) (Bint.of_expr e2))
    | `Benum _ ->
  ...
  let support_cond cudd (expr : 'a t) : Cudd.Bdd.vt =
    match expr with
    | #Bdd.Expr0.t as x -> Bdd.Expr0.O.support_cond cudd (x :> Cudd.Man.v Bdd.Expr0.t)
    | `Apron x -> Apron.support x
  ;;

  let support env cond (expr : 'a t) : 'a PSette.t =
    ...
    (match expr with
     | #Bdd.Expr0.t -> ()
     | `Apron expr -> set := PSette.union (Apron.support_leaf env expr) !set);
    !set
  ;;

  let normalize ?(reduce = false) ?(careset = false) ((cond : ('a, 'b) Cond.O.t), lexpr) =
    let ncond = Bdd.Cond.copy cond in
    let support lexpr =
      List.fold_left
        (fun supp expr ->
           Cudd.Bdd.support_union
             supp
             (Cudd.Bdd.support_inter ncond.supp (support_cond ncond.Bdd.Cond.cudd expr)))
    ...
    let lexpr =
      ...
      then (
        ...
        then (
          let supp = support lexpr in
          if not (Cudd.Bdd.is_equal supp ncond.supp)
          then (
            Bdd.Cond.reduce_with ncond supp;
            let perm = Bdd.Cond.normalize_with ncond in
            let lexpr = permute_list lexpr perm in
