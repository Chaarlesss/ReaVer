...
type ('a, 'b, 'c, 'd) man =
  ...
  ; assign_lexpr :
      ...
      -> 'd
  ; substitute_lexpr :
      'c -> 'a Env.t -> 'a Cond.t -> 'd -> 'a list -> 'a Expr0.t list -> 'd option -> 'd
...
let assign_lexpr ?relational ?nodependency man =
  man.assign_lexpr ?relational ?nodependency man.man
;;
...
let man_of_mtbdd (man : ('a, 'b) mtbdd) : ('a, 'b, 'c, 'd) man = Obj.magic man

let of_mtbdd (manabs : ('a, 'b) mtbdd * 'b Mtbdddomain0.t) : ('a, 'b, 'c, 'd) man * 'd t =
  Obj.magic manabs
;;

let man_is_mtbdd (man : ('a, 'b, 'c, 'd) man) = man.typ = "mtbdd"

let man_to_mtbdd (man : ('a, 'b, 'c, 'd) man) : ('a, 'b) mtbdd =
  if man_is_mtbdd man then Obj.magic man else failwith ""
;;

let to_mtbdd (manabs : ('a, 'b, 'c, 'd) man * 'd t) : ('a, 'b) mtbdd * 'b Mtbdddomain0.t =
  if man_is_mtbdd (fst manabs) then Obj.magic manabs else failwith ""
;;
...
let man_of_bdd (man : ('a, 'b) bdd) : ('a, 'b, 'c, 'd) man = Obj.magic man

let of_bdd (manabs : ('a, 'b) bdd * 'b Bdddomain0.t) : ('a, 'b, 'c, 'd) man * 'd t =
  Obj.magic manabs
;;

let man_is_bdd man = man.typ = "bdd"

let man_to_bdd (man : ('a, 'b, 'c, 'd) man) : ('a, 'b) bdd =
  if man_is_bdd man then Obj.magic man else failwith ""
;;

let to_bdd (manabs : ('a, 'b, 'c, 'd) man * 'd t) : ('a, 'b) bdd * 'b Bdddomain0.t =
  if man_is_bdd (fst manabs) then Obj.magic manabs else failwith ""
;;
