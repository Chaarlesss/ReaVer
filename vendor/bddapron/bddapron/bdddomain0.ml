...
module O = struct
  ...
  module L = struct
    ...
    *)
    let check_unicity man list =
      ignore (Bddleaf.check_unicity ~is_equal:(Apron.Abstract0.is_eq man.apron) list);
      List.iter
        (fun elt ->
           if Apron.Abstract0.is_bottom man.apron elt.leaf
           then failwith "Bddapron.Bdddomain0.check_unicity: empty APRON value in a list")
        list;
      true
    ;;
    ...
    let check_disjointness man list = Bddleaf.check_disjointness list

    (* Performs the join of a list with an element *)
    let cons_unique man elt1 list2 =
      Bddleaf.cons_unique ~is_equal:(Apron.Abstract0.is_eq man.apron) elt1 list2
    ;;
    ...
    let append_unique man list1 list2 =
      Bddleaf.append_unique ~is_equal:(Apron.Abstract0.is_eq man.apron) list1 list2
    ;;
    ...
    let meet_cube apron env cond (elt : 'a elt) cube =
      ...
      let nelt =
        { guard = nguard
        ; leaf =
            (if Cudd.Bdd.is_cst cubecond || Cudd.Bdd.is_false nguard
             then elt.leaf
             else (
  ...
  *)
  let check_wellformed man t =
    ...
    if not (Cudd.Bdd.is_equal t.bottom.guard (Cudd.Bdd.dnot guardofnonbottom))
    then failwith "The guard of bottom is not the complement of the other guards";
  ...
  let canonicalize ?(apron = false) ?(unique = true) ?(disjoint = true) man t =
    ...
    if (unique && not t.unique) || (disjoint && not t.disjoint)
    then (
    ...
    if apron
    then List.iter (fun elt -> Apron.Abstract0.canonicalize man.apron elt.leaf) t.list;
    ()
  ;;
  ...
  let print ?(print_apron = Apron.Abstract0.print) env fmt t =
    if t.list = []
    then pp_print_string fmt "bottom"
    else (
      let string_of_dim dim = Env.string_of_aprondim env dim in
      let print_elt fmt elt =
        if Bdd.Expr0.O.Bool.is_true env elt.guard
        then fprintf fmt "@[<hv>%a@]" (print_apron string_of_dim) elt.leaf
        else
          fprintf
            fmt
            "@[<hv>(%a) and@ %a@]"
            ...
            elt.guard
            (print_apron string_of_dim)
            elt.leaf
      in
      Print.list ~first:"{ @[<v>" ~sep:" or@," ~last:"@] }" print_elt fmt t.list)
  ;;
  ...
  let top man env =
    ...
    { list =
        [ { guard = Cudd.Bdd.dtrue cudd; leaf = abs_of_env Apron.Abstract0.top man env } ]
    ; bottom =
        { guard = Cudd.Bdd.dfalse cudd; leaf = abs_of_env Apron.Abstract0.bottom man env }
    ; unique = true
    ; disjoint = true
    }
  ;;
  ...
  let of_apron man env abs0 =
    ...
    then
      { list = []
      ; bottom = { guard = Cudd.Bdd.dtrue env.cudd; leaf = abs0 }
      ; unique = true
      ; disjoint = true
      }
    else
      { list = [ { guard = Cudd.Bdd.dtrue cudd; leaf = abs0 } ]
      ; bottom =
  ...
  let of_bddapron man env lbddabs0 =
    ...
    let res =
      { list =
          List.map
            (fun (bdd, abs0) -> { Bddleaf.guard = bdd; Bddleaf.leaf = abs0 })
            lbddabs0
      ; bottom =
  ...
  let is_bottom man t = Cudd.Bdd.is_true t.bottom.guard

  let is_top man t =
    Cudd.Bdd.is_false t.bottom.guard
    &&
    (canonicalize man t;
     match t.list with
  ...
  let is_leq man t1 t2 =
    Cudd.Bdd.is_included_in t2.bottom.guard t1.bottom.guard
    &&
    (canonicalize man t2;
     try
       Bddleaf.iter2
         (fun elt2 elt1 ->
            if
              (not (Cudd.Bdd.is_inter_empty elt2.guard elt1.guard))
  ...
  let is_eq man t1 t2 =
    let res =
      Cudd.Bdd.is_equal t1.bottom.guard t2.bottom.guard
      &&
      (canonicalize man t1;
       canonicalize man t2;
      ...
      let nlist =
        fold2
          (fun res elt1 elt2 ->
             ...
             if Cudd.Bdd.is_false nguard
             then res
             else (
               let nleaf = Apron.Abstract0.meet man.apron elt1.leaf elt2.leaf in
               if Apron.Abstract0.is_bottom man.apron nleaf
               then res
               else (
                 let nelt = { guard = nguard; leaf = nleaf } in
                 if t1.disjoint && t2.disjoint
                 then L.cons_unique man nelt res
                 else L.cons ~unique:true ~disjoint man nelt res)))
          []
          t1.list
          t2.list
      in
  ...
  let join_internal ~disjoint man t1 t2 =
    ...
    then t1
      ...
      in
      { list = nlist
      ; bottom = { t1.bottom with guard = Cudd.Bdd.dand t1.bottom.guard t2.bottom.guard }
      ; unique = true
      ; disjoint = disjoint || (t1.disjoint && t2.disjoint)
      })
  ;;
  ...
  let widening_generic man t1 t2 otlincons0 =
    ...
    then t2
        ...
        fold2
          (fun res elt1 elt2 ->
             ...
             then res
               ...
               let nleaf =
                 match otlincons0 with
                 | None -> Apron.Abstract0.widening man.apron elt1.leaf elt2.leaf
                 | Some tlincons0 ->
                   Apron.Abstract0.widening_threshold
                     man.apron
                     elt1.leaf
                     elt2.leaf
                     tlincons0
      ...
      List.iter
        (fun elt1 ->
           if not (Cudd.Bdd.is_inter_empty elt1.guard t2.bottom.guard)
           then failwith "Bddapron.widening: second value does not contain first value")
        t1.list;
      let nlist =
        List.fold_left
          (fun res elt2 ->
      ...
      let res =
        { list = nlist
        ; bottom =
            { t1.bottom with guard = Cudd.Bdd.dand t1.bottom.guard t2.bottom.guard }
        ; unique = true
        ; disjoint = true
        }
      in
  ...
  let widening_threshold man t1 t2 tlincons0 = widening_generic man t1 t2 (Some tlincons0)
  ...
  let elt_descend_bdd ~merge ~maxdepth apron env cond =
    ...
    Bdd.Decompose.descend
      ...
      ~maxdepth
      ~nocare:(fun (elt, bdd) ->
        let res = Cudd.Bdd.is_false bdd || L.is_bottom apron elt in
        res)
      ...
      ~cofactor:(fun (elt, bdd) cube ->
        ...
        if false then printf "cofactor => %a@." myprint_elt nelt;
        let nelt = L.meet_cube apron env cond elt cube in
        nelt, Cudd.Bdd.cofactor bdd cube)
      ~select:(fun (elt, bdd) ->
        if false then printf "select bdd=%a@." (Cudd.Bdd.print_minterm pp_print_int) bdd;
        let res = Bdd.Decompose.select_cond_bdd cond bdd in
        if false then printf "select => %i@." res;
        res)
      ~ite:(fun ~depth ~newcube ~cond ~dthen ~delse ->
        let res =
          match dthen, delse with
          | None, x | x, None -> x
          | Some glista, Some glistb -> Some (descend_merge ~merge glista glistb)
        in
        res)
  ;;

  (* main function *)
  let meet_cond_internal
        ~unique
        ~disjoint
        ~maxdepth
        (man : ('a, 'b) man)
    ...
    if Cudd.Bdd.is_true supp
    then (
      let nlist =
        List.fold_left
          (fun res elt ->
             let nguard = Cudd.Bdd.dand elt.guard bdd in
             if Cudd.Bdd.is_false nguard then res else { elt with guard = nguard } :: res)
          []
          t.list
      in
      { t with
        list = nlist
      ; bottom = { t.bottom with guard = Cudd.Bdd.dnot (Bddleaf.guard ~cudd nlist) }
      })
    else (
      ...
      let nlist =
        List.fold_left
          (fun res elt ->
             if false then printf "nlist elt=%a@." myprint_elt elt;
             if Cudd.Bdd.is_inter_empty elt.guard bdd
             then res
             else (
               ...
               let oglist =
                 elt_descend_bdd
                   ...
                   ~terminal:(fun ~depth ~newcube ~cube ~down ->
                     let elt, bdd = down in
                     let nbdd =
                       if depth < max_int then Cudd.Bdd.exist cond.supp bdd else bdd
                     in
               ...
               | None -> res
               | Some (g, list) ->
                 if t.disjoint && disjoint
                 then if res = [] then list else L.append_unique man res list
                 else L.append ~unique ~disjoint man res list))
          []
          t.list
      in
      let nbottom =
      ...
      in
      { list = nlist; bottom = nbottom; disjoint; unique })
  ;;
    ...
    let ores =
      Bdd.Decompose.descend
        ...
        ~select:(fun (elt, texpr) ->
          ...
          if Cudd.Bdd.is_cst suppcond then -1 else Cudd.Bdd.topvar suppcond)
        ~ite:(fun ~depth ~newcube ~cond ~dthen ~delse ->
          let res =
            match dthen, delse with
            | None, x | x, None -> x
        ...
        ~terminal:(fun ~depth ~newcube ~cube ~down ->
          ...
          let ((g, list) as glist) =
            if taexpr = [||]
            then asssub ~merge apron env org lbvar lbexpr [||] [||] dest
            else
              Cudd.Mapleaf.combineleaf_array
                ...
                (fun guard taexpr ->
                   ...
                   let taexpr =
                     Array.map (Apronexpr.to_texpr0 env.symbol eapron) taexpr
                   in
  ...
  let assign_terminal
        ...
        tadim
        taexpr
        (oldest : 'b elt list option)
    =
    if Cudd.Bdd.is_false org.guard
    then org.guard, []
    else (
      let nguard =
        Bdd.Domain0.O.assign_lexpr ?relational ?nodependency env org.guard lbvar lbexpr
      in
      let nlist =
        match oldest with
        | None ->
          let nleaf =
            Apron.Abstract0.assign_texpr_array apron org.leaf tadim taexpr None
          in
  ...
  let substitute_terminal ~merge apron env org lbvar lbexpr tadim taexpr ldest =
    let glist =
      if Cudd.Bdd.is_false org.guard
      then org.guard, []
      else (
        ...
        List.fold_left
          (fun res dest ->
             ...
             then res
               ...
               if Apron.Abstract0.is_bottom apron nleaf
               then res
               else descend_merge ~merge res (nguard, [ { guard = nguard; leaf = nleaf } ])))
          (dfalse, [])
          ldest)
    in
    glist
  ...
  let forget_list man env t lvar =
    ...
    then t
      ...
      let nlist = List.map (fun elt -> L.forget man.apron elt bsupp tadim) t.list in
      let nbottom =
        let guardnonbottom = Bddleaf.guard ~cudd:env.cudd nlist in
        { t.bottom with guard = Cudd.Bdd.dnot guardnonbottom }
      in
      ...
      in
      canonicalize ~disjoint:man.forget_disjoint ~unique:man.forget_unique man res;
      res)
  ;;
  ...
  let forall_bool_list man ({ cudd } as env) t lvar =
    ...
    then t
      ...
      then
        failwith
          "Bddapron.forall_bool_list: numerical variables in universalquantification";
      ...
      let disj = Bddleaf.guard ~cudd t.list in
      (* G *)
      let nguard = Cudd.Bdd.forall bsupp disj in
      let nlist =
        List.fold_left
          (fun nlist { guard; leaf } ->
             (* G_i, L_i *)
      ...
      let nlist, guard_disj =
        List.fold_left
          (fun (nlist, guard_disj) ({ guard; leaf } as elt) ->
             if Apron.Abstract0.is_bottom man.apron leaf
             then nlist, guard_disj
      ...
      in
      canonicalize
        ~disjoint:man.forall_bool_disjoint
        ~unique:man.forall_bool_unique
        man
        res;
      res)
  ;;
  ...
  let apply_change ~bottom man t change =
    let notbdd = change.cbdd.intro = None && change.cbdd.remove = None in
    let notapron =
      ...
      List.map
        (fun elt ->
           let nguard =
             if notbdd
             then elt.guard
             else Bdd.Domain0.O.apply_change elt.guard change.cbdd
           in
           let nleaf =
             if notapron
             then elt.leaf
             else Apron.Abstract0.apply_dimchange2 man.apron elt.leaf change.capron false
           in
           { guard = nguard; leaf = nleaf })
        t.list
    in
    ...
    let res = { list = nlist; bottom = nbottom; disjoint; unique } in
    canonicalize
      ~disjoint:man.change_environment_disjoint
      ~unique:man.change_environment_unique
      man
      res;
    res
  ;;
  ...
  let apply_permutation man t (operm, oapronperm) =
    ...
    then t
      ...
      let nlist =
        List.map
          (fun elt ->
             ...
             in
             let nleaf =
               match oapronperm with
               | None -> elt.leaf
               | Some perm -> Apron.Abstract0.permute_dimensions man.apron elt.leaf perm
             in
             { guard = nguard; leaf = nleaf })
          t.list
      in
      ...
      in
      let res =
        { list = nlist; bottom = nbottom; disjoint = t.disjoint; unique = t.unique }
      in
      res)
  ;;
end
...
let make_man apron =
  ...
  ; meet_cond_disjoint = true
  ; (* false *)
    meet_cond_depth = max_int
  ; assign_unique = true
  ; assign_disjoint = true
  ; (* false *)
    substitute_unique = true
  ; substitute_disjoint = true
  ; (* false *)
    forget_unique = true
  ; forget_disjoint = true
  ; (* false *)
    forall_bool_unique = true
  ; forall_bool_disjoint = true
  ; (* false *)
