...
module O = struct
  ...
  type ('a, 'b) t = ('b, 'a Expr0.t) Env.value constraint 'b = ('a, 'c, 'd, 'e) Env.O.t
  type ('a, 'b) expr = ('a, 'b) t
  ...
  let substitute
        ...
        (cond : ('a, 'b) Cond.O.t)
        (e : ('a, 'b) t)
        (substitution : ('a * ('a, 'b) t) list)
    : ('a, 'b) t
    =
    let lvarexpr = Env.check_lvarvalue e.env substitution in
  ...
  let var env cond (var : 'a) : ('a, 'b) t = make_value env (Expr0.O.var env cond var)
  ...
  module Bool = struct
    ...
    let substitute ?memo cond e sub = of_expr (ddsubstitute ?memo cond (to_expr e) sub)
  end
  ...
  module Apron = struct
    ...
    let to_expr (e : ('a, 'b) t) = make_value e.env (`Apron e.val0)

    let extend_environment e nenv =
      if Env.is_eq e.env nenv
      then e
      else (
        if not (Env.is_leq e.env nenv)
        then
          failwith
            "Apron.extend_environment: the given environment is not a superenvironment ";
    ...
    let add cond ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
      Env.mapbinop (ApronexprDD.add e1.env ~typ ~round) e1 e2
    ;;

    let mul cond ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
      Env.mapbinop (ApronexprDD.mul e1.env ~typ ~round) e1 e2
    ;;

    let sub cond ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
      Env.mapbinop (ApronexprDD.sub e1.env ~typ ~round) e1 e2
    ;;

    let div cond ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
      Env.mapbinop (ApronexprDD.div e1.env ~typ ~round) e1 e2
    ;;

    let gmod cond ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
      Env.mapbinop (ApronexprDD.gmod e1.env ~typ ~round) e1 e2
    ;;
    ...
    let ite cond e1 e2 e3 = Env.mapterop Cudd.Mtbdd.ite e1 e2 e3
  ...
  let permute (e : 'a Expr0.t) (tab : int array) : 'a Expr0.t =
    match e with
    | #Bdd.Expr0.t as e ->
  ...
  let extend_environment abs nenv =
    if Env.is_eq abs.env nenv
    then abs
    else (
      if not (Env.is_leq abs.env nenv)
      then
        failwith
          "BddapronexprE.O.extend_environment: the given environment is not a \
           superenvironment";
  ...
  let eq cond e1 e2 =
    ...
    match t with
    | `Bool -> Bool.eq cond (Bool.of_expr e1) (Bool.of_expr e2)
    | `Bint _ -> Bint.eq cond (Bint.of_expr e1) (Bint.of_expr e2)
    | `Benum _ -> Benum.eq cond (Benum.of_expr e1) (Benum.of_expr e2)
    | `Real ->
      let diff = Apron.sub cond (Apron.of_expr e1) (Apron.of_expr e2) in
      Apron.eq cond diff
    | _ -> failwith ""
  ;;
  ...
  let print cond fmt (e : ('a, 'b) t) : unit = Expr0.O.print e.env cond fmt e.val0
  let make = make_value
  ...
  let normalize ?reduce ?careset (cond, lexpr) =
    ...
    let lexpr = List.rev_map2 make_value lenv lexpr in
    cond, lexpr
  ;;
  ...
  module List = struct
    ...
    let extend_environment e nenv = Bdd.Env.extend_environment Expr0.O.permute_list e nenv
    ...
    let print
          ...
          (x : ('a, 'b) t)
      =
      Print.list ~first ~sep ~last (Expr0.O.print x.env cond) fmt x.val0
    ;;
  end
end
