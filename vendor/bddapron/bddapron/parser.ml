...
let expr0_of_string env cond str =
  try
    ...
    try expr0_of_lexbuf env cond lexbuf with
    | Parsing.Parse_error ->
      error
        ...
        str
  with
  | Lex.Error (s, e) -> error "Lexical error, characters %d-%d in expression %s" s e str
;;

let expr1_of_string env cond str = Env.make_value env (expr0_of_string env cond str)
