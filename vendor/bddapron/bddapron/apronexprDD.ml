...
let print print_bdd symbol fmt (expr : 'a t) =
  Cudd.Mtbdd.print print_bdd (Apronexpr.print symbol) fmt expr
;;
...
let absorbant_zero env expr =
  ...
  if Apronexpr.equal env.symbol expr0 Apronexpr.zero then Some expr else None
;;

let absorbant_one env expr =
  let expr0 = Cudd.Mtbdd.get expr in
  if Apronexpr.equal env.symbol expr0 Apronexpr.one then Some expr else None
;;

let of_apronexpr env apronexpr = Cudd.Mtbdd.cst env.cudd env.ext.table apronexpr
let cst env coeff = of_apronexpr env (Apronexpr.cst coeff)
let var env v = of_apronexpr env (Apronexpr.var env.symbol (Env.typ_of_var env) v)

let add env ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
  Cudd.User.map_op2
    ~commutative:true
    ~special:(fun dd1 dd2 ->
      if Cudd.Mtbdd.is_cst dd1
      then if is_zero env (Cudd.Mtbdd.dval dd1) then Some dd2 else None
      else if is_zero env (Cudd.Mtbdd.dval dd2)
      then Some dd1
      else None)
    (fun e1 e2 ->
       Cudd.Mtbdd.unique
         env.ext.table
         (Apronexpr.add env.symbol ~typ ~round (Cudd.Mtbdd.get e1) (Cudd.Mtbdd.get e2)))
    e1
    e2
;;
...
let sub env ?typ ?round e1 e2 =
  Cudd.User.map_op2
    ~special:(fun dd1 dd2 ->
      if Cudd.Mtbdd.is_cst dd2 && is_zero env (Cudd.Mtbdd.dval dd2)
      then Some dd1
      else None)
    (fun e1 e2 ->
       Cudd.Mtbdd.unique
         env.ext.table
         (Apronexpr.sub env.symbol ?typ ?round (Cudd.Mtbdd.get e1) (Cudd.Mtbdd.get e2)))
    e1
    e2
;;
  ...
  Cudd.User.map_op2
    ...
    ~special:(fun dd1 dd2 ->
      ...
      then (
        ...
        if is_zero env v then Some dd1 else if is_one env v then Some dd2 else None)
      else (
        let v = Cudd.Mtbdd.dval dd2 in
        if is_zero env v then Some dd2 else if is_one env v then Some dd1 else None))
    (fun e1 e2 ->
       Cudd.Mtbdd.unique
         env.ext.table
         (Apronexpr.mul env.symbol ~typ ~round (Cudd.Mtbdd.get e1) (Cudd.Mtbdd.get e2)))
    e1
    e2
;;
...
let div env ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
  Cudd.User.map_op2
    ~special:(fun dd1 dd2 ->
      if Cudd.Mtbdd.is_cst dd1
      then if is_zero env (Cudd.Mtbdd.dval dd1) then Some dd1 else None
      else if is_one env (Cudd.Mtbdd.dval dd2)
      then Some dd1
      else None)
    (fun e1 e2 ->
       Cudd.Mtbdd.unique
         env.ext.table
         (Apronexpr.div env.symbol ~typ ~round (Cudd.Mtbdd.get e1) (Cudd.Mtbdd.get e2)))
    e1
    e2
;;
...
let gmod env ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e1 e2 =
  Cudd.User.map_op2
    (fun e1 e2 ->
       Cudd.Mtbdd.unique
         env.ext.table
         (Apronexpr.gmod env.symbol ~typ ~round (Cudd.Mtbdd.get e1) (Cudd.Mtbdd.get e2)))
    e1
    e2
;;
...
let cast env ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e =
  Cudd.User.map_op1
    (fun e ->
       Cudd.Mtbdd.unique env.ext.table (Apronexpr.cast ~typ ~round (Cudd.Mtbdd.get e)))
    e
;;
...
let sqrt env ?(typ = Apron.Texpr1.Real) ?(round = Apron.Texpr1.Rnd) e =
  Cudd.User.map_op1
    (fun e ->
       Cudd.Mtbdd.unique env.ext.table (Apronexpr.sqrt ~typ ~round (Cudd.Mtbdd.get e)))
    e
;;
...
let substitute_linexpr
  ...
  let substitute_linterm (term : 'a Apronexpr.Lin.term) : 'a t =
    ...
    let coeffexpr = of_apronexpr env (Apronexpr.cst (Apron.Coeff.s_of_mpqf mpqf)) in
  ...
  in
  if lterm1 = []
  then of_apronexpr env (Apronexpr.Lin linexpr)
  else (
    let linexpr2 =
      { Apronexpr.Lin.cst = linexpr.Apronexpr.Lin.cst; Apronexpr.Lin.lterm = lterm2 }
    in
    let expr2 = of_apronexpr env (Apronexpr.Lin linexpr2) in
    let expr =
      List.fold_left (fun res term -> add env res (substitute_linterm term)) expr2 lterm1
    in
    expr)
;;
...
let substitute_polyexpr
  ...
  let substitute_term (term : 'a Apronexpr.Poly.term) : 'a t =
    ...
    let res = of_apronexpr env (Apronexpr.cst (Apron.Coeff.s_of_mpqf mpqf)) in
    substitute_monomial res monomial
  in
  let res = of_apronexpr env (Apronexpr.cst (Apron.Coeff.s_of_int 0)) in
  List.fold_left (fun res term -> add env res (substitute_term term)) res polyexpr
;;
...
let substitute_treeexpr
  ...
  let rec parcours = function
    | Apronexpr.Tree.Cst coeff -> of_apronexpr env (Apronexpr.cst coeff)
    | Apronexpr.Tree.Var var ->
      (try of_expr (PMappe.find var substitution) with
       | Not_found -> of_apronexpr env (Apronexpr.Lin (Apronexpr.Lin.var var)))
...
module Condition = struct
  ...
  let of_condition (env : 'b) (cond : ('a, 'b) Cond.O.t) condition =
    ...
    | `Bool b -> if b then Cudd.Bdd.dtrue env.cudd else Cudd.Bdd.dfalse env.cudd
    | `Cond x -> of_apronexpr env cond x
  ;;
