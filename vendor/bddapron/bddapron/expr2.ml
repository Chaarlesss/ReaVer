...
module O = struct
  ...
  let print fmt (expr : ('a, 'b) t) = Expr1.O.print expr.cond fmt expr.val1
  ...
  let of_expr1
    ...
    =
    if normalize
    then of_expr0 ~normalize ?reduce ?careset expr1.Env.env cond expr1.Env.val0
    else Bdd.Cond.make_value cond expr1
  ;;
  ...
  let extend_environment e nenv =
    Bdd.Cond.make_value e.cond (Expr1.O.extend_environment e.val1 nenv)
  ;;
  ...
  module Bool = struct
    ...
    let print fmt (expr : ('a, 'b) t) = Expr1.O.Bool.print expr.cond fmt expr.val1
    ...
    let extend_environment e nenv =
      Bdd.Cond.make_value e.cond (Expr1.O.Bool.extend_environment e.val1 nenv)
    ;;
  ...
  module List = struct
    ...
    let of_listexpr1
          ...
          ?careset
          (cond : ('a, 'b) Cond.O.t)
          (listexpr1 : ('a, 'b) Expr1.O.List.t)
      : ('a, 'b) t
      =
      if normalize
      then of_lexpr0 ~normalize ?reduce ?careset listexpr1.Env.env cond listexpr1.Env.val0
      else Bdd.Cond.make_value cond listexpr1
    ;;
    ...
    let extend_environment e nenv =
      Bdd.Cond.make_value e.cond (Expr1.O.List.extend_environment e.val1 nenv)
    ;;
