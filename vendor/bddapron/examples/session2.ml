...
printf "%a@." (Cudd.Mtbdd.print__minterm Apronexpr.print) expr0;;
printf "%a@." (Cudd.Mtbdd.print_minterm pp_print_int Apronexpr.print) expr0;;
...
let f i =
  ...
  then
    Expr1.Bool.dand
      ...
      (Expr1.Apron.supeq
         cond
         (Expr1.Apron.sub cond x (Expr1.Apron.cst env cond (Apron.Coeff.s_of_int 1))))
  else
    Expr1.Bool.dand
      cond
      ...
      (Expr1.Bool.dand
         cond
         (Expr1.Apron.supeq cond (Expr1.Apron.negate cond x))
         (Expr1.Apron.supeq
            cond
            (Expr1.Apron.add cond x (Expr1.Apron.cst env cond (Apron.Coeff.s_of_int 3)))))
;;
...
let tabs = Array.map (fun cond2 -> Domain1.meet_condition bddapron cond top cond2) tcond
let abs = Domain1.join bddapron tabs.(1) tabs.(2);;

Domain1.forget_list bddapron abs [ "b0" ];;

Domain1.assign_lexpr
  bddapron
  cond
  abs
  [ "b1" ]
  [ Expr1.Bool.to_expr (Expr1.Bool.dtrue env cond) ]
  None
;;
...
Domain1.assign_lexpr
  ...
  abs
  [ "e" ]
  [ Expr1.Benum.to_expr (Expr1.Benum.var env cond "l2") ]
  None
;;

let abs = Array.fold_left (Domain1.join bddapron) tabs.(0) tabs;;
