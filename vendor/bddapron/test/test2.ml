...
Env.add_typ_with env "typ" (`Benum [| "a"; "b"; "c" |]);;
...
let _ =
  ...
  let c = Expr0.Bool.of_expr (Parser.expr0_of_string env cond "x0>=0") in
...
let expr =
  Parser.expr1_of_string
    env
    cond
    "if (b0==b1) and b2 then x0+2 else if not (b0==b1) then x0+1 else x0"
;;
...
let print_table fmt table =
  Cudd.Mtbddc.print_table (fun x -> Apron.Abstract0.print string_of_dim x) fmt table
;;
...
let bexpr0 = Parser.boolexpr2_of_string env cond "e0 == a"
let bexpr1 = Parser.boolexpr2_of_string env cond "x0>=0"
...
let bexpr1 =
  Parser.boolexpr2_of_string env cond "x0+2*x1>=0 and x0+x1<=4 and x2>=0 and x2<=10"
;;

let bexpr1 = bexpr1.Cond.val1.Env.val0

let bexpr2 =
  Parser.boolexpr2_of_string env cond "x0+2*x1>=2 and x0+x1<=6 and x2>=3 and x2<=11"
;;
