...
let multipick_set nb filter set =
  let list = PSette.fold (fun elt res -> if filter elt then elt :: res else res) set [] in
...
let pick_set filter set = List.hd (multipick_set 1 filter set)

module Expr = struct
  let pick_typ env typ =
    let alltyps =
      PMappe.fold
        (fun var typ res -> PSette.add typ res)
        env.Bdd.Env.vartyp
  ...
  let pick_var env typ = List.hd (multipick_var 1 env typ)
  ...
  let rec bool ?(lin = true) env cond depth =
    if depth = 0
    then Expr1.Bool.of_bool env cond (Random.bool ())
    else if depth = 1
    then Expr1.Bool.var env cond (pick_var env `Bool)
    else if Random.int !ratio_bool_nbool = 0
    then (
  ...
  and bint ?(lin = true) env cond typ depth =
    ...
    then Expr1.Bint.var env cond (pick_var env typ)
    else (
      let e1 = bint ~lin env cond typ (depth - 1) in
      match Random.int 3 with
      | 0 ->
        (match Random.int 5 with
         ...
         | 2 -> if size > 0 then Expr1.Bint.shift_left cond (Random.int size) e1 else e1
         | 3 -> if size > 0 then Expr1.Bint.shift_right cond (Random.int size) e1 else e1
         | 4 ->
           if size > 0
  ...
  and benum ?(lin = true) env cond typ depth =
    ...
    then Expr1.Benum.var env cond (pick_var env typ)
    else (
      let e1 = benum ~lin env cond typ (depth - 1) in
      let e2 = benum ~lin env cond typ (depth - 1) in
      let e = bool ~lin env cond (depth - 1) in
  ...
  and apron ?(lin = true) env cond depth =
      ...
      then (
        ...
        List.fold_left
          (fun res var ->
             ...
             let term = Expr1.Apron.mul cond cst (Expr1.Apron.var env cond var) in
    ...
    then Expr1.Apron.var env cond (pick_var env `Real)
    else (
      let e1 = apron ~lin env cond (depth - 1) in
...
module D = struct
  ...
  type t = { f : 'a 'b 'c. 'a -> 'c Env.t -> 'b }

  let print man fmt abs =
    fprintf
      fmt
      "@[<v>abs1 = %a;@ abs2 = %a;@]"
      (Domain1.print (fst man))
      (fst abs)
      (Domain1.print (snd man))
      (snd abs)
  ;;
  ...
  let is_cst man abs = is_bottom man abs || is_top man abs
end
...
let check man abs =
  ...
  let list =
    List.map (fun (bdd, abs) -> { Bddleaf.guard = bdd; Bddleaf.leaf = abs }) lbddabs0
  in
  ...
  let bdd2 =
    ...
    ; Bdddomain0.bottom =
        ...
        ; Bddleaf.leaf =
            (Apron.Abstract1.bottom mbdd.Bdddomain0.apron env.ext.eapron)
  ...
  if not (Bdddomain1.is_eq mbdd bdd bdd2)
  then (
...
let condition man env =
  ...
  let res =
    Expr2.Bool.of_expr1 ~normalize:true ~reduce:true ~careset:true cond condition
  in
  res
;;
...
let abs_rand man env =
  ...
  try D.of_condition2 check man env condition with
  | _ as exn ->
...
let test_meet man env =
  ...
  while !i < !nbiterations do
    ...
    let abs =
      try D.meet check man abs1 abs2 with
      | _ as exn ->
        printf "@.abs1 = %a;@.abs2 = %a;@." (D.print man) abs1 (D.print man) abs2;
        raise exn
    in
    assert (D.is_leq man abs abs1);
    assert (D.is_leq man abs abs2);
    if not (D.is_cst man abs1 || D.is_cst man abs2 || D.is_cst man abs)
    then (
      if false
      then
        printf
...
let test_join man env =
  ...
  while !i < !nbiterations do
    ...
    let abs2 = abs_rand man env in
    let abs =
      try D.join check man abs1 abs2 with
      | _ as exn ->
        printf "@.abs1 = %a;@.abs2 = %a;@." (D.print man) abs1 (D.print man) abs2;
        raise exn
    in
...
let test_widening man env =
  ...
  while !i < !nbiterations do
    ...
    let abs =
      try D.widening check man abs1 abs2 with
      | _ as exn ->
        printf "@.abs1 = %a;@.abs2 = %a;@." (D.print man) abs1 (D.print man) abs2;
        raise exn
    in
...
let test_asssub asssub man env =
  ...
  while !i < !nbiterations do
    ...
    let cond =
      Bddapron.Cond.make ~bddindex0:0 ~bddsize:200 ~symbol:string_symbol env.cudd
    in
...
*)
        printf
          "@.lvar = %a@.lexpr=%a@.absorg = %a;@.absdest = %a;@."
          (Print.list pp_print_string)
          lvar
          (Print.list (Expr1.print cond))
          lexpr
          (D.print man)
          absorg
          (D.print man)
          (match oabsdest with
           | None -> D.top check man env
           | Some x -> x);
        raise exn
    in
    if not (D.is_cst man absorg || D.is_cst man abs)
    then (
      if true
      then
        printf
          "lvar = %a@ lexpr=%a@ absorg = %a;@ absdest = %a;@ abs = %a;@ "
          (Print.list pp_print_string)
          lvar
          (Print.list (Expr1.print cond))
          lexpr
    ...
    | `Substitute ->
      ...
      then
        printf
          "@.lvar = %a@.lexpr=%a@.absorg = %a;@.absdest = %a;@.abs = %a@.abs2=%a@."
          (Print.list pp_print_string)
          lvar
          (Print.list (Expr1.print cond))
          lexpr
...
let test_forget man env =
  ...
  while !i < !nbiterations do
    ...
    let lvar = multipick_set !nbasssub (fun _ -> true) vars in
    let abs =
      try D.forget_list check man absorg lvar with
      | _ as exn ->
        printf
          "@.absorg = %a@.lvar=%a@."
          (D.print man)
...
let case_loop cudd apron =
  ...
  let l23 =
    D.meet_condition
      ...
      l20
      (Expr1.Bool.of_expr
         (Parser.expr1_of_string env cond "ITERMAX-iter-1>=0 and eps-m>=0"))
  in
  printf "l23=%a@." (D.print man) l23;
  let l23 =
    D.assign_lexpr check man cond l23 [ "i" ] [ Parser.expr1_of_string env cond "0" ] None
  in
  printf "l23=%a@." (D.print man) l23;
  let contrib =
    D.meet_condition
      ...
      cond
      l23
      (Expr1.Bool.of_expr (Parser.expr1_of_string env cond "i-n>=0"))
  in
  ...
  let contrib =
    D.assign_lexpr
      ...
      contrib
      [ "iter" ]
      [ Parser.expr1_of_string env cond "iter+1" ]
      None
  in
  printf "contrib=%a@." (D.print man) contrib;
