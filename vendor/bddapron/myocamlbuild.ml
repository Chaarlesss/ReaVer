...
let dvipdf =
  try Sys.getenv "DVIPDF" with
  | _ -> "dvipdf"
;;

let makeindex =
  try Sys.getenv "MAKEINDEX" with
  | _ -> "makeindex"
;;

let latex =
  try Sys.getenv "LATEX" with
  | _ -> "latex"
;;

let cat =
  try Sys.getenv "CAT" with
  | _ -> "cat"
;;

let ocamldoc_rules () =
  rule
    "ocamldoc: document ocaml project odocl & odoci (intro) & *odoc -> docdir (html)"
    ~insert:`top
    ~deps:[ "%.odocl"; "%.odoci" ]
    ...
    (fun env build ->
       ...
       let to_build =
         List.map
           (fun module_name -> expand_module include_dirs module_name [ "odoc" ])
           contents
       in
       let odoc_paths = List.map Outcome.good (build to_build) in
       let tags =
         Tags.union (tags_of_pathname docout) (tags_of_pathname docdir) ++ "ocaml"
       in
       Seq
         [ Cmd (S [ A "rm"; A "-rf"; Px docdir ])
  ...
  rule
    "latex: document ocaml project odocl & *odoc -> tex"
    ~insert:`top
    ~dep:"%.odocl"
  ...
  rule "latex: dvi -> pdf" ~dep:"%.dvi" ~prod:"%.pdf" (fun env build ->
    let dvi = env "%.dvi" in
    let pdf = env "%.pdf" in
...
let ocamlpack_rules () =
  rule
    "ocamlpack: ocamlpack & mlpack & mlpacki & mli* -> mli"
    ...
    (fun env build ->
       ...
       let to_build =
         List.map
           (fun module_name -> expand_module include_dirs module_name [ "mli" ])
           contents
       in
  ...
  rule
    "ocamlpack: ocamlpack mlpack & mlpacki & ml* -> ml"
    ~deps:[ "ocamlpack"; "%.mlpack"; "%.mlpacki" ]
    ~prod:"%.ml"
    (fun env build ->
       let mlpack = env "%.mlpack" in
       ...
       let to_build =
         List.map
           (fun module_name -> expand_module include_dirs module_name [ "ml" ])
           contents
       in
       let ml_paths = List.map Outcome.good (build to_build) in
       Cmd
