...
module O = struct
  ...
  let check_envvar (env : ('a, 'b, 'd) #Env.O.t) (var : string) : unit =
    ...
    | Not_found ->
      failwith
        (Format.sprintf
           "The variable %s is unknown or has a wrong type in the environement of the \
            value"
           var)
  ;;
  ...
  let mapunop f e = make_value e.env (f e.value)

  let normalize permute t nenv =
    if nenv == Obj.magic t.env
    then t.value
    else permute t.value (Env.permutation12 t.env nenv)
  ;;
  ...
  let mapterop permute1 permute2 permute3 f t1 t2 t3 =
    let nenv, value1, value2, value3 = check_value3 permute1 permute2 permute3 t1 t2 t3 in
    make_value nenv (f value1 value2 value3)
  ;;

  let check_lvarvalue permute permutelist expr lvarexpr =
    let nenv =
      List.fold_left (fun nenv (var, expr) -> Env.lce nenv expr.env) expr.env lvarexpr
    in
    let expr = normalize permute expr nenv in
    let lvarexpr =
      List.map (fun (var, expr) -> var, normalize permutelist expr nenv) lvarexpr
    in
    nenv, expr, lvarexpr
  ;;
  ...
  type ('a, 'b) t = ('a, 'b Expr0.expr) Env.value constraint 'a = ('c, 'd, 'b) #Env.O.t
  ...
  let extend_environment e nenv = Env.extend_environment Expr0.O.permute e nenv

  let ite e1 e2 e3 =
    mapterop Expr0.O.Bool.permute Expr0.O.permute Expr0.O.permute Expr0.O.ite e1 e2 e3
  ;;

  let cofactor e1 e2 = mapbinop Expr0.O.permute Expr0.O.Bool.permute Expr0.cofactor e1 e2
  let restrict e1 e2 = mapbinop Expr0.O.permute Expr0.O.Bool.permute Expr0.restrict e1 e2

  let tdrestrict e1 e2 =
    mapbinop Expr0.O.permute Expr0.O.Bool.permute Expr0.tdrestrict e1 e2
  ;;
  ...
  let substitute e lvarexpr =
    let nenv, e, lvarexpr = check_lvarvalue Expr0.O.permute Expr0.O.permute e lvarexpr in
    make_value nenv (Expr0.O.substitute nenv e lvarexpr)
  ;;

  let eq e1 e2 = mapbinope Expr0.O.permute Expr0.O.permute Expr0.O.eq e1 e2
  let support (e : ('a, 'b) expr) = Expr0.O.support e.env e.value
  let support_cond (e : ('a, 'b) expr) = Expr0.O.support_cond e.env e.value
  let print fmt (e : ('a, 'b) expr) : unit = Expr0.O.print e.env fmt e.value
  ...
  module Bool = struct
    type ('a, 'b) t = ('a, 'b Cudd.Bdd.t) Env.value constraint 'a = ('c, 'd, 'b) #Env.O.t
    ...
    let to_expr (e : ('a, 'b) t) = make_value e.env (`Bool e.value)
    ...
    let exist (lvar : string list) e =
      check_lvar e lvar;
      make_value e.env (Cudd.Bdd.exist (Expr0.O.bddsupport e.env lvar) e.value)
    ;;

    let forall (lvar : string list) e =
      check_lvar e lvar;
      make_value e.env (Cudd.Bdd.forall (Expr0.O.bddsupport e.env lvar) e.value)
    ;;
    ...
    let substitute_by_var e lvarvar =
      make_value e.Env.env (Expr0.O.Bool.substitute_by_var e.Env.env e.Env.value lvarvar)
    ;;

    let substitute e lvarexpr = of_expr (substitute (to_expr e) lvarexpr)
    let print fmt (x : ('a, 'b) t) = Expr0.O.print_bdd x.env fmt x.value
  end
  ...
  module Bint = struct
    type ('a, 'b) t = ('a, 'b Int.t) Env.value constraint 'a = ('c, 'd, 'b) #Env.O.t
    ...
    let to_expr (e : ('a, 'b) t) = make_value e.env (`Bint e.value)
    let extend_environment e nenv = Env.extend_environment Int.permute e nenv

    let of_int env typ cst =
      match typ with
      | `Tbint (sgn, size) -> make_value env (Int.of_int env#cudd sgn size cst)
    ;;

    let var env (var : string) = make_value env (Expr0.O.Bint.var env var)
    ...
    let ite e1 e2 e3 = mapterop Cudd.Bdd.permute Int.permute Int.permute Int.ite e1 e2 e3
    ...
    let cofactor e1 e2 = mapbinop Int.permute Cudd.Bdd.permute Int.cofactor e1 e2
    let restrict e1 e2 = mapbinop Int.permute Cudd.Bdd.permute Int.restrict e1 e2
    let tdrestrict e1 e2 = mapbinop Int.permute Cudd.Bdd.permute Int.tdrestrict e1 e2

    let substitute_by_var e lvarvar =
      make_value e.Env.env (Expr0.O.Bint.substitute_by_var e.Env.env e.Env.value lvarvar)
    ;;
  ...
  module Benum = struct
    type ('a, 'b) t = ('a, 'b Enum.t) Env.value constraint 'a = ('c, 'd, 'b) #Env.O.t
    ...
    let to_expr (e : ('a, 'b) t) = make_value e.env (`Benum e.value)
    let extend_environment e nenv = Env.extend_environment Enum.permute e nenv
    let var env (var : string) = make_value env (Expr0.O.Benum.var env var)

    let ite e1 e2 e3 =
      mapterop Cudd.Bdd.permute Enum.permute Enum.permute Enum.ite e1 e2 e3
    ;;

    let eq e1 e2 = mapbinope Enum.permute Enum.permute Enum.equal e1 e2
    let eq_label e label = make_value e.env (Enum.equal_label e.env e.value label)
    let cofactor e1 e2 = mapbinop Enum.permute Cudd.Bdd.permute Enum.cofactor e1 e2
    let restrict e1 e2 = mapbinop Enum.permute Cudd.Bdd.permute Enum.restrict e1 e2
    let tdrestrict e1 e2 = mapbinop Enum.permute Cudd.Bdd.permute Enum.tdrestrict e1 e2

    let substitute_by_var e lvarvar =
      make_value e.Env.env (Expr0.O.Benum.substitute_by_var e.Env.env e.Env.value lvarvar)
    ;;
  ...
  let var env (var : string) : ('a, 'b) expr = make_value env (Expr0.O.var env var)
end
