...
let extend man (n : int) (a : 'a t) : 'a t =
  { signed = a.signed; reg = Reg.extend man ~signed:a.signed n a.reg }
;;
...
let check (a : 'a t) (b : 'a t) : unit =
  if not (a.signed = b.signed && Array.length a.reg = Array.length b.reg)
  then error "bounded integer of different types"
;;

let rec neg (a : 'a t) : 'a t =
  if a.signed
  then { signed = true; reg = Reg.neg a.reg }
  else error "Bddint.neg: negation of an unsigned integer"
;;
...
let mul a b =
  check a b;
  if a.reg = [||]
  then { signed = a.signed || b.signed; reg = [||] }
  else (
    let man = Cudd.Bdd.manager a.reg.(0) in
...
let shift_left n a =
  ...
  then a
    ...
    let man = Cudd.Bdd.manager a.reg.(0) in
    { signed = a.signed; reg = fst (Reg.shift_left man n a.reg) })
;;
...
let shift_right n a =
  ...
  then a
    ...
    { signed = a.signed
    ; reg =
        fst ((if a.signed then Reg.shift_right else Reg.shift_right_logical) man n a.reg)
    })
;;

let scale n a = { signed = a.signed || n < 0; reg = Reg.scale n a.reg }

let ite bdd a b =
  check a b;
  { signed = a.signed; reg = Reg.ite bdd a.reg b.reg }
;;
...
let of_int man sign size cst = { signed = sign; reg = Reg.of_int man size cst }
let to_int a = Reg.to_int a.signed a.reg
...
module Minterm = struct
  ...
  let iter ~(signed : bool) (f : int -> unit) (minterm : Reg.Minterm.t) : unit =
    let rec parcours minterm i =
      ...
      then f (Reg.Minterm.to_int ~signed minterm)
      else (
...
let print f fmt t =
  fprintf fmt "{ @[<hv>signed=%b;@ reg=%a@] }" t.signed (Reg.print f) t.reg
;;

let print_minterm print_bdd fmt t = Reg.print_minterm ~signed:t.signed print_bdd fmt t.reg
