...
let bdd_of_vdd vdd = Cudd.Vdd.guard_of_leaf vdd true
...
let split_level (bdd : Cudd.Bdd.vt) (level : int) : (Cudd.Bdd.vt * Cudd.Bdd.vt) list =
  ...
  Array.fold_left
    (fun res condexpr ->
       if Cudd.Vdd.is_cst condexpr && Cudd.Vdd.dval condexpr = false
       then res
       else (Cudd.Vdd.guard_of_node vdd condexpr, bdd_of_vdd condexpr) :: res)
    []
    tcondexpr
;;

let splitpermutation_of_envcond env cond typ : int * (int array * int array) option =
  let r = make_info env cond in
  if typ = `BoolCond && r.maxlevelbool < r.minlevelcond
  then r.minlevelcond, None
  else if typ = `CondBool && r.maxlevelcond < r.minlevelbool
  then r.minlevelbool, None
  else (
...
let split_bdd ?memo1 ?memo2 (level, opermutations) (bexpr : 'a Cudd.Bdd.t)
  : ('a Cudd.Bdd.t * 'a Cudd.Bdd.t) list
  =
  match opermutations with
  | None -> split_level bexpr level
...
let decompose_bdd_boolcond env cond (bexpr : 'a Cudd.Bdd.t)
  : ('a Cudd.Bdd.t * 'a Cudd.Bdd.t) list
  =
  if Cudd.Bdd.is_true bexpr
  then [ bexpr, bexpr ]
  else if Cudd.Bdd.is_false bexpr
  then []
  else (
...
let decompose_bdd_condbool env cond (bexpr : 'a Cudd.Bdd.t)
  : ('a Cudd.Bdd.t * 'a Cudd.Bdd.t) list
  =
  if Cudd.Bdd.is_true bexpr
  then [ bexpr, bexpr ]
...
let bdd_support_cond cond bdd = Cudd.Bdd.support_inter cond.supp (Cudd.Bdd.support bdd)
let vdd_support_cond cond vdd = Cudd.Bdd.support_inter cond.supp (Cudd.Vdd.support vdd)

let tbdd_tvdd_support_cond cond (tbdd, tvdd) =
  let cudd = cond.Cond.cudd in
  let supp = ref (Cudd.Bdd.dtrue cudd) in
  Array.iter
    (fun bdd -> supp := Cudd.Bdd.support_union !supp (bdd_support_cond cond bdd))
    tbdd;
  Array.iter
    (fun vdd -> supp := Cudd.Bdd.support_union !supp (vdd_support_cond cond vdd))
    tvdd;
  !supp
;;
...
let descend
      ...
      ~(select : 'a -> int)
      ~(terminal :
         depth:int -> newcube:'c Cudd.Bdd.t -> cube:'c Cudd.Bdd.t -> down:'a -> 'b option)
      ~(ite :
         depth:int
    ...
    then None
      ...
      then None
        ...
        then (* End case *)
          ...
          let dthen = map (depth + 1) (Cudd.Bdd.dand cube var) (cofactor down var)
          and delse = map (depth + 1) (Cudd.Bdd.dand cube nvar) (cofactor down nvar) in
          ite ~depth ~newcube ~cond ~dthen ~delse)))
  in
  map 0 (Cudd.Bdd.dtrue cudd) down
;;

let decompose_dd_treecondbool ?careset ~topvar ~support ~cofactor env cond (dd : 'a)
  ...
  let select =
    let r = make_info env cond in
    if r.maxlevelcond < r.minlevelbool
    then fun dd -> topvar dd
    else fun dd -> select_cond (support dd)
  in
  let otree =
    match careset with
    | None ->
    ...
    | Some careset ->
      descend
        ...
        ~cube_of_down:(fun _ -> dtrue)
        ~cofactor:(fun (dd, careset) cube ->
          cofactor dd cube, Cudd.Bdd.cofactor careset cube)
...
let decompose_bdd_treecondbool env cond (bdd : 'a Cudd.Bdd.t)
...
let decompose_vdd_treecondbool ?careset env cond (vdd : 'a Cudd.Vdd.t)
...
let decompose_tbdd_tvdd_treecondbool ?careset env cond ddarray =
  let cudd = cond.Cond.cudd in
  decompose_dd_treecondbool
    ?careset
...
let dnf_of_bdd ?first ?last of_idb bdd =
  ...
  if Cudd.Bdd.is_false bdd
  then Disjunction []
  else if Cudd.Bdd.is_true bdd
  then Dtrue
  else (
    let disj = ref [] in
    Cudd.Bdd.iter_cube
