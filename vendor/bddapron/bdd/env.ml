...
let (gtvars : 'a group_tree -> 'a list), (gtlvars : 'a group_tree list -> 'a list) =
  ...
  in
  (fun l -> acc_gtvars [] l), fun l -> List.fold_left acc_gtvars [] l
;;
...
let (gtidxs : 'a group_tree -> int list), (gtlidxs : 'a group_tree list -> int list) =
  ...
  in
  (fun l -> acc_gtidxs [] l), fun l -> List.fold_left acc_gtidxs [] l
;;

let gcmpidx a b =
  let c = compare (gidx a) (gidx b) in
  assert (c <> 0);
  c
;;
...
let pp_gt pp_symb =
  let rec pp_gt fmt = function
    | Var (v, i, 1) -> Format.fprintf fmt "%a (%d)" pp_symb v i
    | Var (v, i, l) -> Format.fprintf fmt "%a (%d:%d)" pp_symb v i (i + l - 1)
    | Node (g, i, l, `Fixed) -> Format.fprintf fmt "{F[%d:%d]:@ @[%a@]}" i l pp_gtl g
    | Node (g, i, l, `Reord) -> Format.fprintf fmt "{R[%d:%d]:@ @[%a@]}" i l pp_gtl g
  and pp_gtl fmt = function
    | [] -> ()
    | [ x ] -> Format.fprintf fmt "%a" pp_gt x
    | x :: t ->
      Format.fprintf fmt "%a,@ " pp_gt x;
      pp_gtl fmt t
  in
  pp_gt
;;
...
let compare_idb (id1, b1) (id2, b2) =
  let res = id1 - id2 in
  if res != 0 then res else (if b1 then 1 else 0) - if b2 then 1 else 0
;;

let print_typ print_symbol (fmt : Format.formatter) typ =
  match typ with
...
let print_typdef print_symbol (fmt : Format.formatter) typdef =
  match typdef with
  | `Benum array ->
    fprintf fmt "benum{%a}" (Print.array ~first:"" ~sep:"," ~last:"" print_symbol) array
...
let make_symbol
      ...
      print
  =
  { compare; marshal; unmarshal; print }
;;
...
module O = struct
  type ('a, 'b, 'c, 'd, 'e) t = ('a, 'b, 'c, 'd, 'e) t0
    constraint 'b = [> 'a typ ] constraint 'c = [> 'a typdef ]

  let print print_typ print_typdef print_ext fmt (env : ('a, 'b, 'c, 'd, 'e) t) =
    fprintf
      fmt
      "{@[<v>typdef = %a;@ vartyp = %a;@ bddindex0 = %i;@ bddindex = %i; bddincr = %i;@ \
       bddsize = %i;@ idcondvar = %a;@ vartid = %a;@ ext = %a@]}"
      (PMappe.print env.symbol.print print_typdef)
      env.typdef
      (PMappe.print ~first:"[@[" env.symbol.print print_typ)
      env.vartyp
      env.bddindex0
      env.bddindex
      env.bddincr
      env.bddsize
      (PMappe.print pp_print_int env.symbol.print)
      env.idcondvar
      (PMappe.print env.symbol.print print_tid)
      env.vartid
      print_ext
      env.ext
  ;;
  ...
  let make
    ...
    : ('a, 'b, 'c, 'd, 'e) t
    =
    for i = bddindex0 to bddsize - 1 do
    ...
    let e =
      ...
      ; print_external_idcondb =
          (fun fmt (id, b) -> fprintf fmt "%s%i" (if b then "not " else "") id)
      ; ext
      ; symbol
      ; copy_ext
      }
    ...
    let rec setup_gt = function
      ...
      | Node (lst, i, l, k) ->
        Cudd.Man.group
          ...
          i
          l
          (if k = `Reord then Cudd.Man.MTR_DEFAULT else Cudd.Man.MTR_FIXED);
...
let typ_of_var env label : 'a =
  try PMappe.find label env.vartyp with
  | Not_found ->
  ...
  try
    ...
    | _ ->
      (try
       ...
       | Exit -> ())
  with
  | Not_found -> env.print_external_idcondb fmt idb
;;
...
let print_order env (fmt : Format.formatter) : unit =
  ...
  Print.array
    ...
    ~last:"@]"
    (fun fmt (id, level) ->
       fprintf fmt "%3i => %3i, %a" level id (print_idcondb env) (id, true))
    fmt
    tab;
  ()
;;
...
let pack_gtl ?full env (gtl : 'a group_tree list as 'b) : int array * 'b =
  ...
  let rec slice index i = function
    ...
    | l ->
      perm.(i) <- index;
      slice (index + env.bddincr) (succ i) (pred l)
  in
...
let permute_indexes env perm =
  env.idcondvar
  <- PMappe.fold
       (fun idcond -> PMappe.add perm.(idcond))
       env.idcondvar
       (PMappe.empty ( - ));
...
let add_typ_with (env : ('a, 'b, 'c, 'd, 'e) O.t) (typ : 'a) (typdef : 'c) : unit =
  if PMappe.mem typ env.typdef
  then
    failwith
      (Print.sprintf "Bdd.Env.add_typ: type %a already defined" env.symbol.print typ);
  env.typdef <- PMappe.add typ typdef env.typdef;
  match typdef with
  | `Benum labels ->
    let t = `Benum typ in
    Array.iter (fun label -> env.vartyp <- PMappe.add label t env.vartyp) labels
  | _ -> ()
;;
...
let compose_opermutation (operm1 : int array option) (operm2 : int array option)
  ...
  | Some perm1 ->
    (match operm2 with
     | None -> operm1
     | Some perm2 -> Some (compose_permutation perm1 perm2))
;;
...
let mem_typ env typ = PMappe.mem typ env.typdef
let mem_var env label = PMappe.mem label env.vartyp
...
let typdef_of_typ env typ =
  try PMappe.find typ env.typdef with
  | Not_found -> notfound "Bdd.Env.t#typdef_of_typ: unknown type %a" env.symbol.print typ
;;

let vars env = PMappe.maptoset env.vartid
...
let group_vars_with env (gtl : 'a group_tree_spec list) : int array =
  ...
  let rec trgt = function
    | `Var v ->
      let tid = PMappe.find v env.vartid in
...
let enum_size_of_typ env (typ : 'a) : int =
  let labels =
    ...
    | _ ->
      failwith
        (Print.sprintf
           "Bddenum.labels_of_typ: type %a not defined as an enumerated type"
           env.symbol.print
           typ)
  in
...
let add_var_with env ?(booking_factor = 1) var typ : unit =
  if PMappe.mem var env.vartyp
  then
    failwith
      (Print.sprintf
         "Bdd.Env.add_var_with: label/var %a already defined"
         env.symbol.print
         var);
  let vartid =
    match typ with
    | #typ as typ ->
      let tid =
        ...
        | `Benum s ->
          Array.init (enum_size_of_typ env s) (fun i -> env.bddindex + (env.bddincr * i))
      in
      let l = Array.length tid in
      let size = env.bddincr * booking_factor * l in
      if env.bddindex + size >= env.bddindex0 + env.bddsize then raise Bddindex;
      (* abort! *)
...
let add_vars_with
  ...
  try
    ...
    if bddindex = env.bddindex || packing = `None
    then None
    else if packing = `Normalize
    then Some (normalize_with env)
       ...
       (match typ with
        | #typ ->
          (try
             ...
             Array.iter (fun id -> env.idcondvar <- PMappe.remove id env.idcondvar) tid
           with
           | Not_found ->
             failwith
               (Print.sprintf
...
let rename_vars_with env lvarvar : int array option =
  ...
  let lvarvartyptidoset, lvarvartyp =
    List.fold_left
      (fun (res1, res2) (var, nvar) ->
         ...
         with
         | Not_found -> res1, (var, nvar, typ) :: res2)
      ([], [])
      lvarvar
  in
  List.iter
  ...
  List.iter
    (fun (var, nvar, typ, tid, oset) ->
       ...
       Array.iter (fun id -> env.idcondvar <- PMappe.add id nvar env.idcondvar) tid)
    lvarvartyptidoset;
  let perm = normalize_with env in
  Some perm
;;
...
let iter_ordered (env : ('a, 'b, 'c, 'd, 'e) O.t) (f : 'a -> int array -> unit) : unit =
  ...
  for level = 0 to size - 1 do
    ...
    try
      let var = PMappe.find id env.idcondvar in
      if not (PSette.mem var !processed)
      then (
        let tid = PMappe.find var env.vartid in
        f var tid;
...
let lce env1 env2 : ('a, 'b, 'c, 'd, 'e) O.t =
  ...
  then (
    ...
    if offset >= 0 then env1 else shift env1 (-offset))
  else if is_leq env1 env2
  then (
    let offset = env2.bddindex0 - env1.bddindex0 in
    if offset >= 0 then env2 else shift env2 (-offset))
  else (
    let typdef =
      PMappe.mergei
        (fun typ typdef1 typdef2 ->
           if typdef1 <> typdef2
           then
             failwith
               (Print.sprintf
                  "Bdd.Env.lce: two different definitions for (enumerated) type %a"
                  env1.symbol.print
                  typ);
           typdef1)
        env1.typdef
      ...
      PMappe.mergei
        (fun var typ1 typ2 ->
           ...
           then
             failwith
               (Print.sprintf
                  "Bdd.Env.lce: two different types for label/variable %a"
                  env1.symbol.print
                  var);
           typ1)
        env1.vartyp
    ...
    let labeltyp, vartyp =
      PMappe.partition
        (fun varlabel typ ->
           match typ with
           | `Benum _ ->
             not (PMappe.mem varlabel env1.vartid || PMappe.mem varlabel env2.vartid)
           | _ -> false)
        vartyp
    in
    ...
    env.groups <- [];
    if not (PMappe.is_empty vartyp)
    then ignore (add_vars_with env ~packing:`None (PMappe.bindings vartyp));
    env)
;;
...
let permutation12 env1 env2 : int array =
  assert (
    ...
    then true
      ...
      printf "env1=%a@.env2=%a@." print env1 print env2;
      false));
  ...
  PMappe.iter
    (fun var2 tid2 ->
       try
         ...
         Array.iter (fun id -> perm.(id) <- id + !offset) tid1
       with
       | Not_found -> offset := !offset + (Array.length tid2 * env2.bddincr))
    env2.vartid;
  perm
;;
...
let permutation21 env2 env1 : int array =
  ...
  PMappe.iter
    (fun var2 tid2 ->
       try
         let tid1 = PMappe.find var2 env1.vartid in
         Array.iter (fun id -> perm.(id + !offset) <- id) tid1
       with
       | Not_found -> offset := !offset + (Array.length tid2 * env2.bddincr))
    env2.vartid;
  perm
...
let compute_change env1 env2 =
  ...
  let intro =
    ...
    if PMappe.is_empty difftid
    then None
    else (
      let perm = Array.init (Cudd.Man.get_bddvar_nb env2.cudd) (fun i -> i) in
      PMappe.iter
        (fun _ (tid1, tid2) -> Array.iter2 (fun id1 id2 -> perm.(id1) <- id2) tid1 tid2)
        difftid;
      Some perm)
  in
  let remove =
    ...
    then None
      ...
      let mapvarid = PMappe.diffset lce.vartid (PMappe.maptoset env2.vartid) in
      ...
      let perm = Array.init (Cudd.Man.get_bddvar_nb env2.cudd) (fun i -> i) in
      if not (PMappe.is_empty difftid)
      then
        PMappe.iter
          (fun _ (tid1, tid2) -> Array.iter2 (fun id1 id2 -> perm.(id1) <- id2) tid1 tid2)
          difftid;
...
let make_value env val0 =
  assert (
    if PMappe.cardinal env.idcondvar = (env.bddindex - env.bddindex0) / env.bddincr
    then check_normalized env
    else (
      printf "Pb in Bdd.Env.make_value@.";
      printf "env=%a@." print env;
      false));
  { env; val0 }
;;
...
let extend_environment (permute : 'a -> int array -> 'a) value nenv =
  if is_eq value.env nenv
  then (
  ...
  else if is_leq value.env nenv
  then (
    let perm = permutation12 value.env nenv in
    make_value nenv (permute value.val0 perm))
  else
    failwith
      "Bdd.Env.extend_environment: the given environment is not a superenvironment "
;;
...
let check_var (env : ('a, 'b, 'c, 'd, 'e) O.t) (var : 'a) : unit =
  ...
  | Not_found ->
    failwith
      (Print.sprintf
         "The variable %a is unknown or has a wrong type in the environement of the value"
         env.symbol.print
         var)
;;

let check_lvar env lvar : unit = List.iter (check_var env) lvar

let check_value env t =
  if not (is_eq env t.env && env.bddindex0 = t.env.bddindex0)
  then
    failwith
      (Print.sprintf
         "Bdd.Env: value does not have the expected environement@.env=%a@.t.env=%a@."
         print
         env
         print
         t.env);
...
let check_value2 t1 t2 =
  if not (is_eq t1.env t2.env && t1.env.bddindex0 = t2.env.bddindex0)
  then
    failwith
      (Print.sprintf
         "Bdd.Env: operation called with non-equal environments:@.env1=%a@.env2=%a@."
         print
         t1.env
         print
         t2.env);
...
let mapunop f t = make_value t.env (f t.val0)
