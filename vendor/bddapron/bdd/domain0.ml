...
module O = struct
  ...
  let is_variable_unconstrained env abs var =
    ...
    Array.fold_left (fun res id -> res && not (Cudd.Bdd.is_var_in id abs)) true tid
  ;;
  ...
  module Asssub = struct
    let sort tid tbdd =
      let tidbdd =
        Array.init (Array.length tid) (fun i ->
          Array.unsafe_get tid i, Array.unsafe_get tbdd i)
      in
      let man = Cudd.Bdd.manager tbdd.(0) in
      Array.sort
        (fun (id1, _) (id2, _) ->
           Cudd.Man.level_of_var man id1 - Cudd.Man.level_of_var man id2)
        tidbdd;
      let ntid = Array.init (Array.length tid) (fun i -> fst tidbdd.(i))
      and ntbdd = Array.init (Array.length tbdd) (fun i -> snd tidbdd.(i)) in
      ntid, ntbdd
    ;;
    ...
    let is_equal ta tb =
      try
        for i = 0 to Array.length ta - 1 do
          ...
          and b = Array.unsafe_get tb i in
    ...
    let post bdd tid tf =
      ...
      let rec parcours i tf =
        ...
        if length = 1
        then
          if Cudd.Bdd.is_true f
          then Cudd.Bdd.ithvar manager id
          else if Cudd.Bdd.is_false f
          then Cudd.Bdd.dnot (Cudd.Bdd.ithvar manager id)
          else Cudd.Bdd.dtrue manager
        else (
          let tf = Array.sub tf 1 (length - 1) in
          if Cudd.Bdd.is_true f
          then Cudd.Bdd.dand (Cudd.Bdd.ithvar manager id) (parcours (i + 1) tf)
          else if Cudd.Bdd.is_false f
          then
            Cudd.Bdd.dand
            ...
            then parcours (i + 1) tf
              ...
              Cudd.Bdd.ite
                ...
                (parcours (i + 1) tf)))
      in
      parcours 0 (Array.map (fun f -> Cudd.Bdd.constrain f bdd) tf)
    ;;
  ...
  let relation_supp_compose_of_lvarlexpr env lvar (lexpr : 'a Expr0.t list)
    ...
    let tbdd =
      Array.init (Cudd.Man.get_bddvar_nb manager) (fun i -> Cudd.Bdd.ithvar manager i)
    in
    List.iter2
      (fun var expr ->
  ...
  let forget_list env abs lvar = Expr0.O.Bool.exist env lvar abs

  let apply_change abs change =
    let nabs =
      match change.Env.intro with
      | None -> abs
      | Some perm -> Cudd.Bdd.permute abs perm
    in
    let nnabs =
      match change.Env.remove with
      | None -> nabs
      | Some (supp, perm) ->
