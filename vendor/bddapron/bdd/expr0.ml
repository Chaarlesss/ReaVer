...
type 'a t =
  [ `Bool of 'a Cudd.Bdd.t (** Boolean *)
  | `Bint of 'a Int.t (** Bounded integer *)
  | `Benum of 'a Enum.t (** Enumerated *)
  ]
...
module O = struct
  ...
  let register_of_var (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (var : 'a)
    : 'd Cudd.Bdd.t array
    =
  ...
  let bddvar (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (var : 'a) : 'd expr =
    ...
    | #Env.typ as typ ->
      (try
         ...
         match typ with
         | `Bool -> `Bool reg.(0)
         | `Bint (b, n) -> `Bint { Int.signed = b; Int.reg }
         | `Benum s -> `Benum { Enum.typ = env.symbol.marshal s; Enum.reg }
       with
       | Not_found -> `Benum (Enum.of_label env var))
  ...
  module Expr = struct
    ...
    let term_of_vint (var : 'a) (reg : 'd Int.t) (minterm : Cudd.Man.tbool array)
      : 'a term
      =
      let lcode = Reg.Minterm.map (Reg.Minterm.to_int ~signed:reg.Int.signed) minterm in
    ...
    let term_of_venum
      ...
      Reg.Minterm.iter
        (fun valminterm ->
           ...
           if Enum.mem_typcode env typ code
           then (
      ...
      then Tcst false
        ...
        if length = Array.length tlabels then Tcst true else Tatom (Tenum (var, !labels)))
    ;;
    ...
    let term_of_idcondb (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (idcondb : int * bool)
      : 'a term
      =
      let idcond, b = idcondb in
      if mem_id env idcond
      then (
    ...
    let conjunction_of_minterm
      ...
      try
        ...
        PMappe.iter
          (fun var tid ->
             ...
             | _ ->
               ...
               if not (Reg.Minterm.is_indet minterm)
               then (
                 let term =
                   match bddvar env var with
                   | `Bint bddint -> term_of_vint var bddint minterm
        ...
        Array.iteri
          (fun id (v : Cudd.Man.tbool) ->
             if v <> Cudd.Man.Top && not (mem_id env id)
             then (
    ...
    let dnf_of_bdd (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (bdd : 'd Cudd.Bdd.t)
      : 'a term Normalform.dnf
      =
      if Cudd.Bdd.is_true bdd
      then Normalform.Dtrue
      else if Cudd.Bdd.is_false bdd
      then Normalform.Disjunction []
      else (
        try
          let res = ref [] in
          Cudd.Bdd.iter_cube
    ...
    let print_term
      ...
      | Tatom atom ->
        (match atom with
         | Tbool (var, s) ->
           fprintf fmt "%s%a" (if s then "" else "not ") env.symbol.print var
         | Tenum (var, labels) ->
           if List.tl labels = []
           then
             fprintf fmt "%a = %a" env.symbol.print var env.symbol.print (List.hd labels)
           else
             fprintf
               fmt
               "%a in {%a}"
               env.symbol.print
               var
               (Print.list ~first:"@[<h>" ~sep:",@," ~last:"@]" env.symbol.print)
               labels
         | Tint (var, codes) ->
           if List.tl codes = []
           then fprintf fmt "%a = %i" env.symbol.print var (List.hd codes)
           else
             fprintf
               fmt
               "%a in {%a}"
               env.symbol.print
               var
               (Print.list ~first:"@[<h>" ~sep:",@," ~last:"@]" pp_print_int)
               codes)
      | Texternal idb ->
        let print =
    ...
    let print_conjunction
          ...
          env
          fmt
          (conjunction : 'a term Normalform.conjunction)
      =
    ...
    let print_dnf ?print_external_idcondb env fmt (disjunction : 'a term Normalform.dnf) =
      Normalform.print_dnf (print_term ?print_external_idcondb env) fmt disjunction
    ;;
  end
  ...
  let print_bdd
    ...
    =
    if Cudd.Bdd.is_true bdd
    then pp_print_string fmt "true"
    else if Cudd.Bdd.is_false bdd
    then pp_print_string fmt "false"
    else (
      let size = Cudd.Bdd.nbpaths bdd in
      if size > float_of_int !Cudd.Man.print_limit
      then fprintf fmt "(%in,%gm)" (Cudd.Bdd.size bdd) size
      else (
        fprintf fmt "@[<hv>";
        ...
        Cudd.Bdd.iter_cube
          (fun cube ->
             ...
             if conjunction <> Normalform.Cfalse
             then (
  ...
  let print
    ...
    =
    match expr with
    | `Bool x -> print_bdd ?print_external_idcondb env fmt x
    | `Bint x -> Int.print_minterm (print_bdd ?print_external_idcondb env) fmt x
    | `Benum x -> Enum.print_minterm (print_bdd ?print_external_idcondb env) env fmt x
  ;;
  ...
  let cube_of_bdd env (bdd : 'a Cudd.Bdd.t) : 'a Cudd.Bdd.t =
    let map =
      ...
      let map =
        List.fold_left (fun map (id, b) -> PMappe.add id b map) (PMappe.empty ( - )) list
      in
      ref map
    in
    let res = ref (Cudd.Bdd.dtrue env.cudd) in
    let process id b =
      if not (mem_id env id)
      then (
  ...
  let mapmonop (monop : 'a Cudd.Bdd.t -> 'a Cudd.Bdd.t) (expr : 'a expr) : 'a expr =
    match expr with
    | `Bool x -> `Bool (monop x)
    | `Bint x -> `Bint { Int.signed = x.Int.signed; Int.reg = Array.map monop x.Int.reg }
    | `Benum x -> `Benum { Enum.typ = x.Enum.typ; Enum.reg = Array.map monop x.Enum.reg }
  ;;
  ...
  let composition_of_lvarexpr
    ...
    let res =
      Array.init (Cudd.Man.get_bddvar_nb env.cudd) (fun i -> Cudd.Bdd.ithvar manager i)
    in
    List.iter
      (fun (var, expr) ->
  ...
  let varmap (expr : 'a expr) : 'a expr = mapmonop Cudd.Bdd.varmap expr
  ...
  let permute_list ?memo lexpr (permutation : int array) =
    match memo with
    | Some memo -> List.map (fun x -> permute ~memo x permutation) lexpr
    | None ->
  ...
  let compose_list ?memo (lexpr : 'a expr list) (composition : 'a Cudd.Bdd.t array)
    : 'a expr list
    =
    match memo with
    | Some memo -> List.map (fun x -> compose ~memo x composition) lexpr
    | None ->
      let hash = Cudd.Hash.create 1 in
      let memo = Cudd.Memo.Hash hash in
  ...
  module Bool = struct
    ...
    let var (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (var : 'a) =
      ...
      | _ ->
        failwith
          (Print.sprintf
             "Formula.Bool.var: variable %a of type %a instead of Boolean"
             env.symbol.print
             var
             (Env.print_typ env.symbol.print)
             (Env.typ_of_var env var))
    ;;
    ...
    let exist env (lvar : 'a list) expr = Cudd.Bdd.exist (bddsupport env lvar) expr
    let forall env (lvar : 'a list) expr = Cudd.Bdd.forall (bddsupport env lvar) expr
  ...
  module Bint = struct
    ...
    let of_int (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) typ cst =
      match typ with
      | `Bint (sgn, size) -> Int.of_int env.cudd sgn size cst
      | _ -> failwith ""
    ;;
    ...
    let var (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (var : 'a) =
      ...
      | _ ->
        failwith
          (Print.sprintf
             "Formula.Bint.var: variable %a of type %a instead of bounded integer"
             env.symbol.print
             var
             (Env.print_typ env.symbol.print)
             (Env.typ_of_var env var))
  ...
  module Benum = struct
    ...
    let var (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (var : 'a) =
      ...
      | _ ->
        failwith
          (Print.sprintf
             "Formula.Bint.var: variable %a of type %a instead of enumerated type"
             env.symbol.print
             var
             (Env.print_typ env.symbol.print)
             (Env.typ_of_var env var))
  ...
  let ite (bdd : 'a Cudd.Bdd.t) (e1 : 'a expr) (e2 : 'a expr) : 'a expr =
    ...
    | _ -> failwith "Bddexpr.ite: typing error"
  ;;

  let eq (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (e1 : 'd expr) (e2 : 'd expr)
    : 'd Cudd.Bdd.t
    =
    match e1, e2 with
    ...
    | _ -> failwith "Bddexpr.eq: typing error"
  ;;
  ...
  let support_cond cudd (expr : 'd expr) : 'd Cudd.Bdd.t =
    ...
    | `Bint x ->
      if x.Int.reg = [||] then Cudd.Bdd.dtrue cudd else Cudd.Bdd.vectorsupport x.Int.reg
    | `Benum x ->
      if x.Enum.reg = [||] then Cudd.Bdd.dtrue cudd else Cudd.Bdd.vectorsupport x.Enum.reg
  ;;
...
*)
  let texpr_of_tbdd (oldtexpr : 'a expr array) (tbdd : 'a Cudd.Bdd.t array)
    : 'a expr array
    =
    let index = ref 0 in
    Array.map
      (fun oldexpr ->
         ...
         | `Bint x ->
           let res =
             { x with Int.reg = Array.mapi (fun i bdd -> tbdd.(!index + i)) x.Int.reg }
           in
           index := !index + Array.length x.Int.reg;
           `Bint res
         | `Benum x ->
           let res =
             { x with Enum.reg = Array.mapi (fun i bdd -> tbdd.(!index + i)) x.Enum.reg }
           in
