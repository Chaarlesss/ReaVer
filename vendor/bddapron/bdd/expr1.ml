...
module O = struct
  ...
  let ite e1 e2 e3 = Env.mapterop Expr0.O.ite e1 e2 e3
  ...
  module Bool = struct
    ...
    let to_expr (e : ('a, 'b, 'c) t) = make_value e.env (`Bool e.val0)
    ...
    let exist (lvar : 'a list) e =
      ...
      make_value e.env (Cudd.Bdd.exist (Expr0.O.bddsupport e.env lvar) e.val0)
    ;;

    let forall (lvar : 'a list) e =
      Env.check_lvar e.env lvar;
      make_value e.env (Cudd.Bdd.forall (Expr0.O.bddsupport e.env lvar) e.val0)
    ;;
    ...
    let substitute ?memo e lvarexpr = of_expr (substitute ?memo (to_expr e) lvarexpr)
    let print fmt (x : ('a, 'b, 'c) t) = Expr0.O.print_bdd x.env fmt x.val0
  end
  ...
  module Bint = struct
    ...
    let to_expr (e : ('a, 'b, 'c) t) = make_value e.env (`Bint e.val0)
    let extend_environment e nenv = Env.extend_environment Int.permute e nenv

    let of_int env typ cst =
      match typ with
      | `Bint (sgn, size) -> make_value env (Int.of_int env.cudd sgn size cst)
    ;;

    let var env (var : 'a) = make_value env (Expr0.O.Bint.var env var)
  ...
  module Benum = struct
    ...
    let to_expr (e : ('a, 'b, 'c) t) = make_value e.env (`Benum e.val0)
    let extend_environment e nenv = Env.extend_environment Enum.permute e nenv
    let var env (var : 'a) = make_value env (Expr0.O.Benum.var env var)
    let ite e1 e2 e3 = Env.mapterop Enum.ite e1 e2 e3
    let eq e1 e2 = Env.mapbinope Enum.equal e1 e2
    let eq_label e label = make_value e.env (Enum.equal_label e.env e.val0 label)
  ...
  let var env (var : 'a) : ('a, 'b, 'c) expr = make_value env (Expr0.O.var env var)
  let make = Env.make_value
  let of_expr0 = Env.make_value
  let get_env = Env.get_env
  let to_expr0 = Env.get_val0
...
*)
    ...
    let to_lexpr e = List.map (fun expr0 -> Env.make_value e.env expr0) e.val0
    let extend_environment e nenv = Env.extend_environment Expr0.O.permute_list e nenv

    let print ?first ?sep ?last fmt x =
      Print.list ?first ?sep ?last (Expr0.O.print x.env) fmt x.val0
    ;;
  end
end
