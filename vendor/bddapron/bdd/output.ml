...
type 'a bdd =
  { cond : int PSette.t ref (** Reachable conditions *)
...
let make_bdd ~cond =
  { cond; bdef = PMappe.empty ( - ); bhash = Hashhe.create 23; blastid = -1 }
;;

let signid_of_bdd (db : 'a bdd) (bdd : 'a Cudd.Bdd.t) : bool * int =
...
let id_of_vdd (db : 'a vdd) (vdd : 'a Cudd.Vdd.t) : int =
  let rec iter (vdd : 'a Cudd.Vdd.t) : int =
    try Hashhe.find db.vhash vdd with
    | Not_found ->
      let mdef =
        match Cudd.Vdd.inspect vdd with
...
(** ADD node *)
type anode =
  | AIte of int * int * int (** RIte(idcond,idnodeThen,idnodeElse) *)
  | ACst of float
...
let id_of_add (db : add) (add : Cudd.Add.t) : int =
  let rec iter (add : Cudd.Add.t) : int =
    try Hashhe.find db.ahash add with
    | Not_found ->
      let mdef =
        match Cudd.Add.inspect add with
