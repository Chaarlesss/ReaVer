...
module O = struct
  ...
  let check_value permute t nenv =
    if Env.is_eq t.env nenv && t.env.bddindex0 = nenv.bddindex0
    then t.val0
    else if Env.is_leq t.env nenv && t.env.bddindex0 = nenv.bddindex0
    then permute t.val0 (Env.permutation12 t.env nenv)
    else
      failwith
        (Print.sprintf
           "Bdd.Domain1: the environment of the argument is not a subenvironment of the \
            expected environment@. t.env=%a@.nenv=%a@."
           Env.print
           t.env
           Env.print
           nenv)
  ...
  let check_lvalue permute lt nenv = List.map (fun t -> check_value permute t nenv) lt
  ...
  let assign_lexpr ?relational ?nodependency (t : ('a, 'b, 'c) t) lvar lexpr =
    ...
    then t
      ...
      make_value
        t.env
        (Domain0.O.assign_lexpr ?relational ?nodependency t.env t.val0 lvar lexpr0))
  ;;
