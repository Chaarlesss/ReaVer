...
let mem_typcode env (typ : 'b) (code : int) : bool = code <= maxcode_of_typ env typ

let findcode (label : 'a) (tlabels : 'a array) : int =
  let i = ref 0 in
  while !i < Array.length tlabels && tlabels.(!i) <> label do
    incr i
  done;
  if !i = Array.length tlabels then raise Not_found else !i
;;
...
let label_of_typcode (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (typ : 'a) (code : int) : 'a =
  ...
  if code < Array.length t
  then t.(code)
  else
    failwith
      (Print.sprintf
         "Bddenum.label_of_typcode: no label for typ=%a, code=%i"
         env.symbol.print
         typ
         code)
;;
...
let of_label (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (label : 'a) : 'd t =
  ...
  let code = findcode label labels in
  let t = { typ = env.symbol.marshal typ; reg = Reg.of_int env.cudd size code } in
  (*
...
let is_cst (x : 'e t) : bool = Reg.is_cst x.reg
let to_code (x : 'e t) : int = Reg.to_int ~signed:false x.reg
...
let equal_label (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (x : 'd t) (label : 'a)
  : 'd Cudd.Bdd.t
  =
...
let equal (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (x : 'd t) (y : 'd t) : 'd Cudd.Bdd.t =
  ...
  then
    failwith
      (sprintf "Bddenum.equal: applied between different types %s and %s" x.typ y.typ);
  Reg.equal env.cudd x.reg y.reg
;;

let ite bdd a b =
  if a.typ <> b.typ
  then failwith (Format.sprintf "Bddenum.ite: types %s and %s are different" a.typ b.typ);
  { typ = a.typ; reg = Reg.ite bdd a.reg b.reg }
;;
...
module Minterm = struct
  let iter
        ...
        (typ : 'a)
        (f : 'a -> unit)
        (minterm : Reg.Minterm.t)
    : unit
    =
    let maxcode = maxcode_of_typ env typ in
    Int.Minterm.iter
      ~signed:false
      (fun code -> if code <= maxcode then f (label_of_typcode env typ code))
      minterm
  ;;
...
let guard_of_label (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (x : 'd t) (label : 'a)
  : 'd Cudd.Bdd.t
  =
  Reg.guard_of_int env.cudd x.reg (code_of_label env label)
;;

let guardlabels (env : ('a, 'b, 'c, 'd, 'e) Env.O.t) (x : 'd t)
  : ('d Cudd.Bdd.t * 'a) list
  =
  ...
  let res =
    List.fold_left
      (fun res (bdd, code) ->
         if code <= maxcode then (bdd, label_of_typcode env typ code) :: res else res)
      []
      lguardints
  in
  List.rev res
...
let print f fmt t = fprintf fmt "{ @[<hv>typ=%s;@ reg=%a@] }" t.typ (Reg.print f) t.reg
...
let vectorcompose ?memo tab x = { x with reg = Reg.vectorcompose ?memo tab x.reg }
