...
let print_strategy_vertex
  ...
  fprintf
    ...
    man.print_vertex
    v.vertex
    (Print.list ~first:"@[<h>[" ~sep:"," ~last:"]@]" man.print_hedge)
    v.hedges
;;
...
let print_strategy
  ...
  Ilist.print
    ...
    (print_strategy_vertex man)
    fmt
    s
;;

let make_strategy_iteration ?(widening_start = 0) ?(widening_descend = 1) () =
  { widening_start
  ; widening_descend
  ; ascending_nb = 0
  ; descending_nb = 0
...
let make_strategy_vertex
  ...
  let hedges =
    PSette.fold
      ...
      []
  in
  let strategy_vertex = { vertex; hedges; widen } in
  strategy_vertex
;;
...
let make_strategy_default
  ...
  =
  let scfc = PSHGraph.scfc_multi vertex_dummy hedge_dummy ?priority graph sinit in
  let scfc =
    Ilist.map
      (make_strategy_iteration ~widening_start ~widening_descend)
      (fun flag _ vertex -> make_strategy_vertex ?priority graph flag vertex)
      scfc
  in
...
let print_stat_iteration fmt it = fprintf fmt "(%i,%b)" it.nb it.stable

let print_stat_iteration_ilist fmt ilist =
  Ilist.print print_stat_iteration (fun _ _ -> ()) fmt ilist
;;

let print_stat fmt stat =
  fprintf
    fmt
    "{ @[<hov>time=%f;@ ascending=%a;@ descending=%a;@] }"
    stat.time
...
let print_attr
      ...
      (attr : 'abstract attr)
  =
  fprintf fmt "{ @[<hov>reach=%a;@ empty=%b@] }" man.print_abstract attr.reach attr.empty
;;
...
let print_arc
      ...
      (arc : 'arc arc)
  =
  fprintf fmt "{ @[<hov>arc=%a;@ aempty=%b@] }" man.print_arc arc.arc arc.aempty
;;
...
let print_info
  ...
  fprintf
    fmt
    "{ @[<hov>itime=%f;@ iascending=%a;@ idescending=%a;@ iworkvertex=%a;@ \
     iworkhedge=%a@] }"
    !(info.itime)
    print_stat_iteration_ilist
    info.iascending
    print_stat_iteration_ilist
    info.idescending
    (PHashhe.print man.print_vertex (fun _ _ -> ()))
    info.iworkvertex
    (PHashhe.print man.print_hedge (fun _ _ -> ()))
    info.iworkhedge
;;
...
let print_workingsets
  ...
  fprintf
    fmt
    "@[<v>workvertex = %a@ workhedge = %a"
    (hashprint man.print_vertex)
    info.iworkvertex
    (hashprint man.print_hedge)
    info.iworkhedge;
  (match info.iinfodyn with
   | Some dyn -> fprintf fmt "@ addhedge = %a" (hashprint man.print_hedge) dyn.iaddhedge
   | None -> ());
  fprintf fmt "@]"
;;
...
let set2_of_strategy comparev (strategy : ('vertex, 'hedge) strategy)
  : 'vertex PSette.t * 'vertex PSette.t
  =
  let empty = PSette.empty comparev in
  Ilist.fold_left
    (fun (res, resw) _ _ v ->
       if v.widen then res, PSette.add v.vertex resw else PSette.add v.vertex res, resw)
    (empty, empty)
    strategy
;;
...
let dot_graph
  ...
  | Some dot_fmt ->
    let set, setw =
      match strategy with
      | None ->
        let e = PSette.empty comparev in
        e, e
    ...
    let fvertexstyle =
      ...
      let vertexstyle =
        ...
        then vertexstyle ^ ",style=filled,fillcolor=coral1"
        else if PSette.mem v setw
        then vertexstyle ^ ",style=filled,fillcolor=red1"
        else if PSette.mem v set
        then vertexstyle ^ ",style=filled,fillcolor=orange1"
        else vertexstyle
      in
      let vertexstyle =
        if PHashhe.mem info.iworkvertex v
        then vertexstyle ^ ",fontcolor=blue3"
        else vertexstyle
      in
      vertexstyle
    in
    let fhedgestyle =
      fun h ->
      let hedgestyle =
        if PHashhe.mem info.iworkhedge h
        then hedgestyle ^ ",fontcolor=blue3"
        else hedgestyle
      in
      hedgestyle
    in
    PSHGraph.print_dot
      ...
      (fun fmt vertex ->
         fprintf
           fmt
           "\"%s\""
           (Print.escaped ~linebreak:'n' (Print.sprintf "%a" man.dot_vertex vertex)))
      (fun fmt hedge ->
         fprintf
           fmt
           "\"%s\""
           (Print.escaped ~linebreak:'n' (Print.sprintf "%a" man.dot_hedge hedge)))
      (fun fmt vertex attr ->
         fprintf
           fmt
           "%s"
