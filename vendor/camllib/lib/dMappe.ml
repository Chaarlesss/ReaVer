...
let add x y t = { xy = Mappe.add x y t.xy; yx = Mappe.add y x t.yx }
let y_of_x x t = Mappe.find x t.xy
let x_of_y y t = Mappe.find y t.yx
let remove x t = { xy = Mappe.remove x t.xy; yx = Mappe.remove (y_of_x x t) t.yx }
...
let merge t1 t2 =
  { xy =
      Mappe.merge
        (fun y1 y2 ->
           if y1 = y2 then y1 else failwith "DMappe.merge: incompatible (x,y) bindings")
        t1.xy
        t2.xy
  ; yx =
      Mappe.merge
        (fun x1 x2 ->
           if x1 = x2 then x1 else failwith "DMappe.merge: incompatible (y,x) bindings")
        t1.yx
        t2.yx
  }
;;
...
let common t1 t2 =
  { xy =
      Mappe.common
        (fun y1 y2 ->
           if y1 = y2 then y1 else failwith "DMappe.common: incompatible (x,y) bindings")
        t1.xy
        t2.xy
  ; yx =
      Mappe.common
        (fun x1 x2 ->
           if x1 = x2 then x1 else failwith "DMappe.common: incompatible (y,x) bindings")
        t1.yx
        t2.yx
  }
;;
...
let intersetx t setx =
  { xy = Mappe.interset t.xy setx; yx = Mappe.filter (fun y x -> Sette.mem x setx) t.yx }
;;

let intersety t sety =
  { xy = Mappe.filter (fun x y -> Sette.mem y sety) t.xy; yx = Mappe.interset t.yx sety }
;;
...
let print
      ...
      t
  =
  Mappe.print ?first ?sep ?last ?firstbind ~sepbind ?lastbind px py fmt t.xy
;;
...
module Make (P : Param) = struct
  ...
  let add x y t = { xy = MappeX.add x y t.xy; yx = MappeY.add y x t.yx }
  let y_of_x x t = MappeX.find x t.xy
  let x_of_y y t = MappeY.find y t.yx
  let remove x t = { xy = MappeX.remove x t.xy; yx = MappeY.remove (y_of_x x t) t.yx }
  ...
  let merge t1 t2 =
    { xy =
        MappeX.merge
          (fun y1 y2 ->
             if MappeY.Setkey.Ord.compare y1 y2 = 0
             then y1
             else failwith "DMappe.merge: incompatible (x,y) bindings")
          t1.xy
          t2.xy
    ; yx =
        MappeY.merge
          (fun x1 x2 ->
             if MappeX.Setkey.Ord.compare x1 x2 = 0
             then x1
             else failwith "DMappe.merge: incompatible (y,x) bindings")
          t1.yx
          t2.yx
    }
  ;;
    ...
    { xy =
        MappeX.common
          (fun y1 y2 ->
             if MappeY.Setkey.Ord.compare y1 y2 = 0
             then y1
             else failwith "DMappe.common: incompatible (x,y) bindings")
          t1.xy
          t2.xy
    ; yx =
        MappeY.common
          (fun x1 x2 ->
             if MappeX.Setkey.Ord.compare x1 x2 = 0
             then x1
             else failwith "DMappe.common: incompatible (y,x) bindings")
          t1.yx
          t2.yx
    }
  ;;
  ...
  let equalx t1 t2 =
    t1 == t2
    || MappeX.equal (fun y1 y2 -> MappeY.Setkey.Ord.compare y1 y2 = 0) t1.xy t2.xy
  ;;

  let equaly t1 t2 =
    t1 == t2
    || MappeY.equal (fun x1 x2 -> MappeX.Setkey.Ord.compare x1 x2 = 0) t1.yx t2.yx
  ;;

  let subsetx t1 t2 =
    t1 == t2
    || MappeX.subset (fun y1 y2 -> MappeY.Setkey.Ord.compare y1 y2 = 0) t1.xy t2.xy
  ;;

  let subsety t1 t2 =
    t1 == t2
    || MappeY.subset (fun x1 x2 -> MappeX.Setkey.Ord.compare x1 x2 = 0) t1.yx t2.yx
  ;;
