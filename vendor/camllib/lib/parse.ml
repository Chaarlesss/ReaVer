...
let parse_lexbuf
  ...
  | Lex_error ->
    ...
    and endp = Lexing.lexeme_end_p lexbuf in
    Format.eprintf
      "Lexical error: %s, line %i, character %i-%i: '%s'.@."
      startp.Lexing.pos_fname
      startp.Lexing.pos_lnum
      (startp.Lexing.pos_cnum - startp.Lexing.pos_bol)
      (endp.Lexing.pos_cnum - endp.Lexing.pos_bol)
  ...
  | Failure s when s = "int_of_string" ->
    ...
    and endp = Lexing.lexeme_end_p lexbuf in
    Format.eprintf
      "Lexical error: %s, line %i, character %i-%i: '%s'.@.int_of_string failure.@."
      startp.Lexing.pos_fname
      startp.Lexing.pos_lnum
      (startp.Lexing.pos_cnum - startp.Lexing.pos_bol)
      (endp.Lexing.pos_cnum - endp.Lexing.pos_bol)
...
let parse_string
  ...
  lexbuf.Lexing.lex_curr_p
  <- { lexbuf.Lexing.lex_curr_p with Lexing.pos_fname = "string \"" ^ str ^ "\"" };
...
let parse_file
  ...
  lexbuf.Lexing.lex_curr_p
  <- { lexbuf.Lexing.lex_curr_p with Lexing.pos_fname = "file " ^ filename };
  let ast = parse_lexbuf ~lexer ~parser lexbuf in
  close_in input;
  ast
;;
