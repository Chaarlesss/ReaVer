...
let map f h =
  ...
  in
  { h with data = Array.map map_bucketlist h.data }
;;
...
let key_index h key = hash_param 10 100 key mod Array.length h.data

module Compare = struct
  let key_index compare h key = compare.hash key mod Array.length h.data
  ...
  let rec find_rec compare key = function
    ...
    | Cons (k, d, rest) -> if compare.equal key k then d else find_rec compare key rest
  ;;
  ...
  let find compare h key =
    ...
    | Empty -> raise Not_found
    | Cons (k1, d1, rest1) ->
      if compare.equal key k1
      then d1
      else (
        match rest1 with
        | Empty -> raise Not_found
        | Cons (k2, d2, rest2) ->
          if compare.equal key k2
          then d2
          else (
            match rest2 with
            | Empty -> raise Not_found
  ...
  let find_all compare h key =
    let rec find_in_bucket = function
      | Empty -> []
      | Cons (k, d, rest) ->
        if compare.equal k key then d :: find_in_bucket rest else find_in_bucket rest
    in
  ...
  let mem compare h key =
    let rec mem_in_bucket = function
      | Empty -> false
      | Cons (k, d, rest) -> compare.equal k key || mem_in_bucket rest
    in
    mem_in_bucket h.data.(key_index compare h key)
  ;;
end

let stdcompare = { hash; equal = ( = ) }
