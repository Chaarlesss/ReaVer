...
let create_compare cmpx cmpy size =
  { xy = PHashhe.create_compare cmpx size; yx = PHashhe.create_compare cmpy size }
;;

let create hashx eqx hashy eqy size =
  { xy = PHashhe.create hashx eqx size; yx = PHashhe.create hashy eqy size }
;;
...
let print
      ...
      t
  =
  PHashhe.print ~first ~sep ~last ~firstbind ~sepbind ~lastbind px py fmt t.xy
;;
