...
let compare_priority (p : 'a -> int) : 'a -> 'a -> int =
  fun x y ->
  let x = p x
  and y = p y in
  Stdlib.compare y x
...
let iter_vertex g f =
  Hashhe.iter
    (fun v vertex_n ->
       f v vertex_n.attrvertex ~pred:vertex_n.predhedge ~succ:vertex_n.succhedge)
    g.vertex
;;
...
let iter_hedge g f =
  Hashhe.iter
    (fun n hedge_n ->
       f n hedge_n.attrhedge ~pred:hedge_n.predvertex ~succ:hedge_n.succvertex)
    g.hedge
;;
...
let fold_vertex g f res =
  Hashhe.fold
    (fun v vertex_n res ->
       f v vertex_n.attrvertex ~pred:vertex_n.predhedge ~succ:vertex_n.succhedge res)
    g.vertex
    res
;;
...
let fold_hedge g f res =
  Hashhe.fold
    (fun n hedge_n res ->
       f n hedge_n.attrhedge ~pred:hedge_n.predvertex ~succ:hedge_n.succvertex res)
    g.hedge
    res
;;
...
let map g map_attrvertex map_attrhedge map_info =
  { vertex =
      Hashhe.map
        (fun v vertex_n ->
           { vertex_n with attrvertex = map_attrvertex v vertex_n.attrvertex })
        g.vertex
  ; hedge =
      Hashhe.map
        (fun h hedge_n -> { hedge_n with attrhedge = map_attrhedge h hedge_n.attrhedge })
        g.hedge
...
let copy copy_attrvertex copy_attrhedge copy_info g =
  { vertex =
      Hashhe.map
        (fun v vertex_n ->
           { vertex_n with attrvertex = copy_attrvertex v vertex_n.attrvertex })
        g.vertex
  ; hedge =
      Hashhe.map
        (fun h hedge_n -> { hedge_n with attrhedge = copy_attrhedge h hedge_n.attrhedge })
        g.hedge
...
module Compare = struct
  ...
  let attrvertex cmp g v = (vertex_n cmp g v).attrvertex
  let attrhedge cmp g h = (hedge_n cmp g h).attrhedge
  let is_vertex cmp g v = Hashhe.Compare.mem cmp.hashv g.vertex v
  let is_hedge cmp g h = Hashhe.Compare.mem cmp.hashh g.hedge h
  ...
  let succ_vertex cmp g v =
    ...
    Sette.iter
      (fun h ->
         ...
         Array.iter (fun v -> res := Sette.Compare.add cmp.comparev v !res) succvertex)
      succhedge;
    !res
  ;;
  ...
  let pred_vertex cmp g v =
    ...
    Sette.iter
      (fun h ->
         ...
         Array.iter (fun v -> res := Sette.Compare.add cmp.comparev v !res) predvertex)
      predhedge;
    !res
  ;;

  let add_vertex cmp g v attrvertex =
    if is_vertex cmp g v
    then failwith "SHGraph.add_vertex: the vertex already exists (use replace_vertex)";
    Hashhe.Compare.add
      cmp.hashv
      g.vertex
      v
      { attrvertex; succhedge = Sette.empty; predhedge = Sette.empty }
  ;;

  let add_hedge cmp g h attrhedge ~pred ~succ =
    if is_hedge cmp g h
    then failwith "SHGraph.add_hedge: the hedge already exists (use replace_hedge)";
    (try
       Array.iter
         (fun v ->
     ...
     | Not_found ->
       failwith
         "SHGraph.add_hedge: origin vertex and/or destination hedge doesn't already exist");
    Hashhe.Compare.add
      cmp.hashh
      g.hedge
      h
      { attrhedge; predvertex = pred; succvertex = succ }
  ;;

  let replace_attrvertex cmp g v attrvertex =
    let vertex_n = Hashhe.Compare.find cmp.hashv g.vertex v in
    let vertex_n = { vertex_n with attrvertex } in
  ...
  let replace_attrhedge cmp g h attrhedge =
    ...
    let hedge_n = { hedge_n with attrhedge } in
  ...
  let add_dummy_forward cmp (vertex_dummy : 'a) (hedge_dummy : 'b) g (root : 'a Sette.t)
    : unit
    =
  ...
  let topological_sort cmp ?priority (g : ('a, 'b, 'c, 'd, 'e) t) (root : 'a) =
    let hash = Hashhe.create 23 in
    let rec visit res v =
      if Hashhe.Compare.mem cmp.hashv hash v
      then res
      else (
        ...
        | Some (Filter p) ->
          Sette.fold (fun h res -> if p h then explore h res else res) succhedges nres
        | Some (Priority p) ->
          let lhedges =
            Sette.fold (fun h res -> if p h >= 0 then h :: res else res) succhedges []
          in
          let lhedges = List.sort (compare_priority p) lhedges in
          List.fold_left (fun res h -> explore h res) nres lhedges)
    and explore (hedge : 'b) (res : 'a list) : 'a list =
      let hedge_n = hedge_n cmp g hedge in
      let cond = array_forall (Hashhe.Compare.mem cmp.hashv hash) hedge_n.predvertex in
      if cond then Array.fold_left visit res hedge_n.succvertex else res
    in
  ...
  let reachable cmp ?filter (g : ('a, 'b, 'c, 'd, 'e) t) (root : 'a) =
        ...
        Sette.iter
          (fun h ->
             ...
             then (
               let hedge_n = hedge_n cmp g h in
               let cond =
                 array_forall (Hashhe.Compare.mem cmp.hashv hashv) hedge_n.predvertex
               in
               if cond
               then (
                 Hashhe.Compare.add cmp.hashh hashh h ();
                 Array.iter visit hedge_n.succvertex)))
          (succhedge cmp g v))
    in
    visit root;
    let setv =
      ...
      reachable
        ...
        ?filter:
          (match filter with
           | None -> None
           | Some p ->
             Some (fun h -> if cmp.hashh.Hashhe.equal h hedge_dummy then true else p h))
        g
        vertex_dummy
    in
    rem_dummy cmp vertex_dummy hedge_dummy g;
  ...
  let cfc cmp ?priority g root =
    let num = ref (-1)
    and partition = ref []
    and pile = Stack.create ()
    and hash = Hashhe.map (fun _ _ -> min_int) g.vertex in
    let rec visit sommet =
      ...
      let head =
        match priority with
        | None -> Sette.fold compute_head succhedges !num
        | Some (Filter p) ->
          Sette.fold
            (fun h head -> if p h then compute_head h head else head)
            succhedges
            !num
        | Some (Priority p) ->
          let lhedges =
  ...
  let cfc_multi cmp ?priority vertex_dummy hedge_dummy g root =
    add_dummy_forward cmp vertex_dummy hedge_dummy g root;
    let res = cfc cmp ?priority:(map_priority cmp hedge_dummy priority) g vertex_dummy in
    rem_dummy cmp vertex_dummy hedge_dummy g;
    match res with
  ...
  let scfc cmp ?priority g root =
    ...
    let rec composante_aux partition h =
      ...
      then
        Array.iter
          (fun succ ->
             if Hashhe.Compare.find cmp.hashv hash succ = min_int
             then ignore (visit succ partition))
          hedge_n.succvertex
    and composante sommet =
      let partition = ref [] in
      let succhedges = succhedge cmp g sommet in
      (match priority with
       | None -> Sette.iter (composante_aux partition) succhedges
       | Some (Filter p) ->
         Sette.iter (fun h -> if p h then composante_aux partition h) succhedges
       | Some (Priority p) ->
         let lhedges =
           Sette.fold (fun h res -> if p h >= 0 then h :: res else res) succhedges []
         in
    ...
    and visit sommet partition =
      ...
      Hashhe.Compare.replace cmp.hashv hash sommet num_sommet;
      let head = ref !num
      and loop = ref false in
      let succhedges = succhedge cmp g sommet in
      (match priority with
       | None -> Sette.iter (compute_head partition head loop) succhedges
       | Some (Filter p) ->
         Sette.iter (fun h -> if p h then (compute_head partition head loop) h) succhedges
       | Some (Priority p) ->
         let lhedges =
           Sette.fold (fun h res -> if p h >= 0 then h :: res else res) succhedges []
         in
      ...
      then (
        ...
        else partition := Ilist.Atome sommet :: !partition);
      !head
    and compute_head partition head loop h =
      ...
      then
        Array.iter
          (fun succ ->
             let dfn = Hashhe.Compare.find cmp.hashv hash succ in
             let m = if dfn = min_int then visit succ partition else dfn in
             if m <= !head
             then (
               loop := true;
               head := m))
  ...
  let scfc_multi cmp vertex_dummy hedge_dummy ?priority g root =
    add_dummy_forward cmp vertex_dummy hedge_dummy g root;
    let res = scfc cmp ?priority:(map_priority cmp hedge_dummy priority) g vertex_dummy in
    rem_dummy cmp vertex_dummy hedge_dummy g;
    match res with
  ...
  let print
    ...
    let vertices =
      Hashhe.fold
        (fun v _ res -> Sette.Compare.add cmp.comparev v res)
        g.vertex
        Sette.empty
    in
    ...
    let hedges =
      Hashhe.fold
        (fun n _ res -> Sette.Compare.add cmp.compareh n res)
        g.hedge
        Sette.empty
    in
...
let stdcompare =
  ...
  { hashv = Hashhe.stdcompare; hashh = Hashhe.stdcompare; comparev = cmp; compareh = cmp }
;;

let create size info = { vertex = Hashhe.create size; hedge = Hashhe.create size; info }
...
(* Size *)
(* ======================================================================= *)
let size_vertex g = Hashhe.length g.vertex
let size_hedge g = Hashhe.length g.hedge

let size_edgevh g =
  let n = ref 0 in
  Hashhe.iter (fun _ vertex_n -> n := !n + Sette.cardinal vertex_n.succhedge) g.vertex;
  !n
;;

let size_edgehv g =
  let n = ref 0 in
  Hashhe.iter (fun _ hedge_n -> n := !n + Array.length hedge_n.succvertex) g.hedge;
  !n
;;

let size g = size_vertex g, size_hedge g, size_edgevh g, size_edgehv g
...
let is_empty g =
  try
    Hashhe.iter (fun _ _ -> raise Exit) g.vertex;
    true
  with
  | Exit -> false
;;
...
(* ======================================================================= *)
let add_vertex g v attrvertex = Compare.add_vertex stdcompare g v attrvertex
...
let replace_attrhedge g h attrhedge = Compare.replace_attrhedge stdcompare g h attrhedge
...
let reachable ?filter g root = Compare.reachable stdcompare ?filter g root
...
let cfc ?priority g root = Compare.cfc stdcompare ?priority g root
...
let scfc ?priority g root = Compare.scfc stdcompare ?priority g root
...
let print_dot
  ...
  if title <> "" then fprintf fmt "1073741823 [%s,label=\"%s\"];@ " titlestyle title;
  Hashhe.iter
    (fun vertex vertex_n ->
       fprintf
         fmt
  ...
  Hashhe.iter
    (fun hedge hedge_n ->
       Array.iter
         (fun pred -> fprintf fmt "%a -> %a;@ " print_vertex pred print_hedge hedge)
         hedge_n.predvertex;
       Array.iter
         (fun succ -> fprintf fmt "%a -> %a;@ " print_hedge hedge print_vertex succ)
         hedge_n.succvertex)
    g.hedge;
  fprintf fmt "@]@.}@.";
  ()
...
module Make (T : T) :
  ...
  let stdcompare =
    { hashv = { Hashhe.hash = HashV.Hash.hash; Hashhe.equal = HashV.Hash.equal }
    ; hashh = { Hashhe.hash = HashH.Hash.hash; Hashhe.equal = HashH.Hash.equal }
  ...
  let topological_sort_multi ?priority g root =
    Compare.topological_sort_multi
      stdcompare
      vertex_dummy
      hedge_dummy
      ?priority
