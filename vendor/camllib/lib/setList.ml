...
let rec add elt = function
  ...
  | x :: l as t ->
    let drp = compare x elt in
    if drp > 0 then elt :: t else if drp = 0 then t else x :: add elt l
;;
...
let rec remove elt = function
  ...
  | x :: l as t ->
    let drp = compare x elt in
    if drp < 0 then x :: remove elt l else if drp = 0 then l else t
;;
...
let rec inter ta tb =
  ...
  | xa :: la, xb :: lb ->
    let drp = compare xa xb in
    if drp > 0 then inter ta lb else if drp = 0 then xa :: inter la lb else inter la tb
;;
...
let rec diff ta tb =
  ...
  | xa :: la, xb :: lb ->
    let drp = compare xa xb in
    if drp > 0 then diff ta lb else if drp = 0 then diff la lb else xa :: diff la tb
;;
...
let rec subset ta tb =
  ...
  | xa :: la, xb :: lb ->
    let drp = compare xa xb in
    if drp > 0 then subset ta lb else if drp = 0 then subset la lb else false
;;
...
let compare =
  let rec cmp ta tb =
    ...
    | xa :: la, xb :: lb ->
      let drp = compare xa xb in
      if drp = 0 then cmp la lb else drp
  in
  cmp
;;
...
let partition f t =
  List.fold_right (fun x (a, b) -> if f x then x :: a, b else a, x :: b) t ([], [])
;;
...
module Make (Ord : Set.OrderedType) = struct
  ...
  let rec compare ta tb =
    ...
    | xa :: la, xb :: lb ->
      let drp = Ord.compare xa xb in
      if drp = 0 then compare la lb else drp
  ;;
