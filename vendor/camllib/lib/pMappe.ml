...
let make compare map = { compare; map }
...
let interset m1 s2 =
  ...
  make m1.compare (Mappe.Compare.interset m1.compare m1.map s2.PSette.set)
;;

let diffset m1 s2 =
  assert (m1.compare == s2.PSette.compare);
  make m1.compare (Mappe.Compare.diffset m1.compare m1.map s2.PSette.set)
;;
...
let mapfm12b foo cmp m1 m2 = foo m1.compare cmp m1.map m2.map
