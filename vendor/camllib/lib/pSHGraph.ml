...
let make compare graph = { compare; graph }
...
let succhedge t v =
  PSette.make t.compare.compareh (SHGraph.Compare.succhedge t.compare t.graph v)
;;

let predhedge t v =
  PSette.make t.compare.compareh (SHGraph.Compare.predhedge t.compare t.graph v)
;;
...
let succ_vertex t v =
  PSette.make t.compare.comparev (SHGraph.Compare.succ_vertex t.compare t.graph v)
;;

let pred_vertex t v =
  PSette.make t.compare.comparev (SHGraph.Compare.pred_vertex t.compare t.graph v)
;;

let add_vertex t v attrvertex = SHGraph.Compare.add_vertex t.compare t.graph v attrvertex
  ...
  Hashhe.iter
    (fun v vertex_n ->
       f
         v
         vertex_n.attrvertex
         ~pred:(PSette.make t.compare.compareh vertex_n.predhedge)
         ~succ:(PSette.make t.compare.compareh vertex_n.succhedge))
    t.graph.vertex
;;
...
let map t map_attrvertex map_attrhedge map_info =
  make t.compare (map t.graph map_attrvertex map_attrhedge map_info)
;;

let copy copy_attrvertex copy_attrhedge copy_info t =
  make t.compare (copy copy_attrvertex copy_attrhedge copy_info t.graph)
;;

let transpose copy_attrvertex copy_attrhedge copy_info t =
  make t.compare (transpose copy_attrvertex copy_attrhedge copy_info t.graph)
;;
...
let check_pset t root =
  if t.compare.comparev == root.PSette.compare
  then true
  else (
    Format.eprintf
      "@.PSHGraph: the comparison function for sets of vertices in the graph (%i) is not \
       physically the same as the comparison function for elements in the set (%i).@.See \
       the documentation of the module PSette.@."
      (Obj.magic t.compare.comparev)
      (Obj.magic root.PSette.compare);
    false)
;;

let topological_sort_multi vertex_dummy hedge_dummy ?priority t root =
  assert (check_pset t root);
  SHGraph.Compare.topological_sort_multi
    t.compare
    vertex_dummy
    hedge_dummy
    ?priority
...
let reachable ?filter t root =
  let setv, seth = SHGraph.Compare.reachable t.compare ?filter t.graph root in
  PSette.make t.compare.comparev setv, PSette.make t.compare.compareh seth
;;

let reachable_multi vertex_dummy hedge_dummy ?filter t root =
  assert (check_pset t root);
  let setv, seth =
    SHGraph.Compare.reachable_multi
      t.compare
      vertex_dummy
      hedge_dummy
      ?filter
...
let cfc ?priority t root = SHGraph.Compare.cfc t.compare ?priority t.graph root

let cfc_multi vertex_dummy hedge_dummy ?priority t root =
  assert (check_pset t root);
  SHGraph.Compare.cfc_multi
    t.compare
    vertex_dummy
    hedge_dummy
    ?priority
...
let scfc ?priority t root = SHGraph.Compare.scfc t.compare ?priority t.graph root

let scfc_multi vertex_dummy hedge_dummy ?priority t root =
  assert (check_pset t root);
  SHGraph.Compare.scfc_multi
    t.compare
    vertex_dummy
    hedge_dummy
    ?priority
