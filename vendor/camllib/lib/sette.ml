...
module Compare = struct
  ...
  let rec join (compare : 'a -> 'a -> int) l v r =
    ...
    | Node (ll, lv, lr, lh), Node (rl, rv, rr, rh) ->
      if lh > rh + 2
      then bal ll lv (join compare lr v r)
      else if rh > lh + 2
      then bal (join compare l v rl) rv rr
      else create l v r
  ;;
  ...
  let rec split (compare : 'a -> 'a -> int) x = function
    ...
    | Node (l, v, r, _) ->
      ...
      then (
        ...
        ll, pres, join compare rl v r)
      else (
        let lr, pres, rr = split compare x r in
        join compare l v lr, pres, rr)
  ;;
  ...
  let rec remove (compare : 'a -> 'a -> int) x = function
    ...
    | Node (l, v, r, _) ->
      ...
      then bal (remove compare x l) v r
    ...
    let rec inter s1 s2 =
      ...
      | Node (l1, v1, r1, _), t2 ->
        (match split compare v1 t2 with
         | l2, false, r2 -> concat compare (inter l1 l2) (inter r1 r2)
         | l2, true, r2 -> join compare (inter l1 l2) v1 (inter r1 r2))
    in
    inter s1 s2
  ;;
  ...
  let diff (compare : 'a -> 'a -> int) s1 s2 =
    let rec diff s1 s2 =
      ...
      | Node (l1, v1, r1, _), t2 ->
        (match split compare v1 t2 with
         | l2, false, r2 -> join compare (diff l1 l2) v1 (diff r1 r2)
         | l2, true, r2 -> concat compare (diff l1 l2) (diff r1 r2))
    in
    diff s1 s2
  ;;
  ...
  let subset (compare : 'a -> 'a -> int) s1 s2 =
    let rec subset s1 s2 =
      ...
      | Node (l1, v1, r1, _), (Node (l2, v2, r2, _) as t2) ->
        ...
        then subset (Node (l1, v1, Empty, 0)) l2 && subset r1 t2
