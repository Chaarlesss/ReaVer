...
module Compare = struct
  let rec add (compare : 'a -> 'a -> int) x data = function
    ...
    | Node (l, v, d, r, h) ->
      ...
      then bal (add compare x data l) v d r
      else bal l v d (add compare x data r)
  ;;
  ...
  let rec join (compare : 'a -> 'a -> int) l x d r =
    ...
    | Node (ll, lx, ld, lr, lh), Node (rl, rx, rd, rr, rh) ->
      if lh > rh + 2
      then bal ll lx ld (join compare lr x d r)
      else if rh > lh + 2
      then bal (join compare l x d rl) rx rd rr
      else create l x d r
  ;;
  ...
  *)
  let rec split (compare : 'a -> 'a -> int) key = function
    ...
    | Node (l, x, d, r, _) ->
      ...
      then (
        let ll, pres, rl = split compare key l in
        ll, pres, join compare rl x d r)
      else (
        let lr, pres, rr = split compare key r in
        join compare l x d lr, pres, rr)
  ;;
  ...
  let rec remove (compare : 'a -> 'a -> int) x = function
    ...
    | Node (l, v, d, r, h) ->
      ...
      then bal (remove compare x l) v d r
      else bal l v d (remove compare x r)
  ;;
    ...
    let rec common m1 m2 =
      ...
      | Node (l1, x1, d1, r1, _), t2 ->
        (match split compare x1 t2 with
         | l2, None, r2 -> concat compare (common l1 l2) (common r1 r2)
         | l2, Some d2, r2 -> join compare (common l1 l2) x1 (f d1 d2) (common r1 r2))
    in
    common m1 m2
  ;;
  ...
  let commoni (compare : 'a -> 'a -> int) f m1 m2 =
    let rec common m1 m2 =
      ...
      | Node (l1, x1, d1, r1, _), t2 ->
        (match split compare x1 t2 with
         | l2, None, r2 -> concat compare (common l1 l2) (common r1 r2)
         | l2, Some d2, r2 -> join compare (common l1 l2) x1 (f x1 d1 d2) (common r1 r2))
    in
    common m1 m2
  ;;
    ...
    let rec interset m1 s2 =
      ...
      | Node (l1, x1, d1, r1, _), t2 ->
        (match Sette.Compare.split compare x1 t2 with
         | l2, false, r2 -> concat compare (interset l1 l2) (interset r1 r2)
         | l2, true, r2 -> join compare (interset l1 l2) x1 d1 (interset r1 r2))
    in
    interset m1 s2
  ;;
  ...
  let diffset (compare : 'a -> 'a -> int) m1 s2 =
    let rec diffset m1 s2 =
      ...
      | Node (l1, x1, d1, r1, _), t2 ->
        (match Sette.Compare.split compare x1 t2 with
         | l2, false, r2 -> join compare (diffset l1 l2) x1 d1 (diffset r1 r2)
         | l2, true, r2 -> concat compare (diffset l1 l2) (diffset r1 r2))
    in
    diffset m1 s2
  ;;
  ...
  type ('a, 'b) enumeration =
    | End
    | More of 'a * 'b * ('a, 'b) t * ('a, 'b) enumeration
  ...
  let equal (compare : 'a -> 'a -> int) cmp m1 m2 =
    let rec equal_aux e1 e2 =
      ...
      | _, End -> false
      | More (v1, d1, r1, e1), More (v2, d2, r2, e2) ->
        compare v1 v2 = 0 && cmp d1 d2 && equal_aux (cons_enum r1 e1) (cons_enum r2 e2)
    in
  ...
  let equali (compare : 'a -> 'a -> int) cmp m1 m2 =
    let rec equal_aux e1 e2 =
      ...
      | _, End -> false
      | More (v1, d1, r1, e1), More (v2, d2, r2, e2) ->
        compare v1 v2 = 0 && cmp v1 d1 d2 && equal_aux (cons_enum r1 e1) (cons_enum r2 e2)
    in
    m1 == Obj.magic m2 || equal_aux (cons_enum m1 End) (cons_enum m2 End)
  ;;
  ...
  let subset (compare : 'a -> 'a -> int) cmp m1 m2 =
    let rec subset_aux m1 m2 =
      ...
      | Node (l1, v1, d1, r1, _), (Node (l2, v2, d2, r2, _) as t2) ->
        let c = compare v1 v2 in
        if c = 0
        then cmp d1 d2 && subset_aux l1 l2 && subset_aux r1 r2
        else if c < 0
        then subset_aux (Node (l1, v1, d1, Empty, 0)) l2 && subset_aux r1 t2
        else subset_aux (Node (Empty, v1, d1, r1, 0)) r2 && subset_aux l1 t2
    in
  ...
  let subseti (compare : 'a -> 'a -> int) cmp m1 m2 =
    let rec subset_aux m1 m2 =
      ...
      | Node (l1, v1, d1, r1, _), (Node (l2, v2, d2, r2, _) as t2) ->
        let c = compare v1 v2 in
        if c = 0
        then cmp v1 d1 d2 && subset_aux l1 l2 && subset_aux r1 r2
        else if c < 0
        then subset_aux (Node (l1, v1, d1, Empty, 0)) l2 && subset_aux r1 t2
        else subset_aux (Node (Empty, v1, d1, r1, 0)) r2 && subset_aux l1 t2
    in
