...
let rec mem elt = function
  ...
  | (x, _) :: l ->
    ...
    if drp > 0 then false else if drp = 0 then true else mem elt l
;;
...
let rec mult elt = function
  ...
  | (x, n) :: l ->
    let drp = compare x elt in
    if drp > 0 then 0 else if drp = 0 then n else mult elt l
;;
...
let rec add ((elt, m) as em) = function
  ...
  | ((x, n) as xn) :: l as t ->
    let drp = compare x elt in
    if drp > 0 then em :: t else if drp = 0 then (x, n + m) :: l else xn :: add em l
;;
...
let rec remove ((elt, m) as em) = function
  ...
  | ((x, n) as xn) :: l as t ->
    ...
    then if n <= m then l else (x, n - m) :: l
    else t
;;
...
let rec union ta tb =
  ...
  | ((xa, na) as xna) :: la, ((xb, nb) as xnb) :: lb ->
    ...
    if drp > 0
    then xnb :: union ta lb
    else if drp = 0
    then (xa, na + nb) :: union la lb
...
let rec diff ta tb =
  ...
  | ((xa, na) as xna) :: la, (xb, nb) :: lb ->
    ...
    then if na > nb then (xa, na - nb) :: diff la lb else diff la lb
    else xna :: diff la tb
;;

let union_set ta tb =
  let rec union_set ta tb =
    match ta, tb with
    ...
    | ((xa, na) as xna) :: la, xb :: lb ->
      let drp = compare xa xb in
      if drp > 0
      then (xb, 1) :: union_set ta lb
      else if drp = 0
      then (xa, na + 1) :: union_set la lb
...
let inter_set ta tb =
  let rec inter_set ta tb =
    match ta, tb with
    | [], _ | _, [] -> []
    | ((xa, na) as xna) :: la, xb :: lb ->
      let drp = compare xa xb in
      if drp > 0
      then inter_set ta lb
      else if drp = 0
      then xna :: inter_set la lb
...
let rec subset ta tb =
  ...
  | (xa, na) :: la, (xb, nb) :: lb ->
    let drp = compare xa xb in
    if drp > 0 then subset ta lb else if drp = 0 && na <= nb then subset la lb else false
;;
...
let partition p l =
  let rec part yes no = function
    ...
    | (x, n) :: l -> if p x then part ((x, n) :: yes) no l else part yes ((x, n) :: no) l
  in
  part [] [] l
;;
  ...
  | (x, n) :: l ->
    List.fold_left
      (fun ((set, mult) as res) (x, n) ->
         if n < mult
         then SetList.singleton x, n
         else if n = mult
         then SetList.add x set, mult
         else res)
      (SetList.singleton x, n)
      l
;;
...
let compare =
  let rec cmp ta tb =
    ...
    | (xa, na) :: la, (xb, nb) :: lb ->
      let drp = compare xa xb in
      if drp = 0 then if na = nb then cmp la lb else na - nb else drp
  in
  cmp
;;
...
module Make (Ord : Set.OrderedType) = struct
  ...
  let rec mult elt = function
    ...
    | (x, n) :: l ->
      let drp = Ord.compare x elt in
      if drp > 0 then raise Not_found else if drp = 0 then n else mult elt l
  ;;
  ...
  let rec inter ta tb =
    ...
    | (xa, na) :: la, (xb, nb) :: lb ->
      ...
      then (xa, Stdlib.min na nb) :: inter la lb
      else inter la tb
  ;;
