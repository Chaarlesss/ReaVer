...
let add name =
  try Hashtbl.find tbl_string_symbol name with
  | Not_found ->
    incr counter;
...
let exists name = Hashtbl.mem tbl_string_symbol name
let of_string name = Hashtbl.find tbl_string_symbol name
let to_string symbol = Hash.find tbl_symbol_string symbol
let print fmt symbol = Format.pp_print_string fmt (to_string symbol)

let print_set fmt set =
  if Set.is_empty set
  then pp_print_string fmt "empty"
  else (
    let min = Set.min_elt set in
    fprintf fmt "{ @[%s" (to_string min);
    Set.iter (fun x -> fprintf fmt ";@ %s" (to_string x)) (Set.remove min set);
    fprintf fmt "@] }")
;;
    ...
    Map.iter
      (fun t x ->
         ...
         then (
           ...
           isfirst := false)
         else fprintf fmt ";@ (%s,%a)" (to_string t) print x)
      map;
    fprintf fmt "@] }")
;;
