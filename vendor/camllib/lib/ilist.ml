...
let depth (b, ll) =
  let rec depth maxdepth ll =
    match ll with
    | [] -> maxdepth
    | Atome _ :: ll -> depth maxdepth ll
    | List (_, ll2) :: ll -> depth (max maxdepth (1 + depth 1 ll2)) ll
  in
  depth 1 ll
;;

let append ~combine (b1, ll1) (b2, ll2) = combine b1 b2, ll1 @ ll2

let rev_append ~(combine : 'b -> 'b -> 'b) (b1, ll1) (b2, ll2) =
  let rec rev_append nb ll1 ll2 =
    match ll1 with
    | [] -> nb, ll2
    | (Atome a as x1) :: ll1 -> rev_append nb ll1 (x1 :: ll2)
    | List (b, ll) :: ll1 -> rev_append nb ll1 (List (rev_append b ll []) :: ll2)
  in
...
let map fb fa ilist =
  let rec parcours flag b = function
    | [] -> []
    | Atome a :: ll -> Atome (fa flag b a) :: parcours false b ll
    | List (b2, ll2) :: ll ->
      let b3 = fb b2 in
...
let map' fa fb ilist =
  let rec parcours flag = function
    | [] -> []
    | Atome a :: ll -> Atome (fa flag a) :: parcours false ll
    | List (b, l2) :: ll ->
  ...
  in
  let b, ll = ilist in
  fb true (b, ll), parcours false ll
;;
...
let of_list b list =
  let rilist = List.fold_left (fun res a -> Atome a :: res) [] list in
  b, List.rev rilist
;;

let to_list ilist =
  let list = fold_left (fun res flag b elt -> elt :: res) [] ilist in
  List.rev list
;;
      ...
      let nres =
        ...
        | List (b, ll) ->
          ...
          then (
            ...
            List (b, ll2) :: res)
          else fold_left (fun res flag b a -> Atome a :: res) res (b, ll)
      in
