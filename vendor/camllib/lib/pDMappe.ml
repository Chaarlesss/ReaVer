...
let add x y t = { xy = PMappe.add x y t.xy; yx = PMappe.add y x t.yx }
let y_of_x x t = PMappe.find x t.xy
let x_of_y y t = PMappe.find y t.yx
let removex x t = { xy = PMappe.remove x t.xy; yx = PMappe.remove (y_of_x x t) t.yx }
let removey y t = { xy = PMappe.remove (x_of_y y t) t.xy; yx = PMappe.remove y t.yx }
let remove = removex
...
let merge t1 t2 =
  { xy =
      PMappe.merge
        (fun y1 y2 ->
           if y1 = y2 then y1 else failwith "DPMappe.merge: incompatible (x,y) bindings")
        t1.xy
        t2.xy
  ; yx =
      PMappe.merge
        (fun x1 x2 ->
           if x1 = x2 then x1 else failwith "DPMappe.merge: incompatible (y,x) bindings")
        t1.yx
        t2.yx
  }
;;
...
let common t1 t2 =
  { xy =
      PMappe.common
        (fun y1 y2 ->
           if y1 = y2 then y1 else failwith "DPMappe.common: incompatible (x,y) bindings")
        t1.xy
        t2.xy
  ; yx =
      PMappe.common
        (fun x1 x2 ->
           if x1 = x2 then x1 else failwith "DPMappe.common: incompatible (y,x) bindings")
        t1.yx
        t2.yx
  }
;;
...
let print
      ...
      t
  =
  PMappe.print ?first ?sep ?last ?firstbind ~sepbind ?lastbind px py fmt t.xy
;;
