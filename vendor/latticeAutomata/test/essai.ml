...
module Finite = struct
  ...
  let print_regexp (title : string) fmt (auto : unit A.t) : unit =
    fprintf
      ...
      "%s: @[%a@]@."
      title
      (Regexp.print print_letter)
      (Regexp.simplify (PL.is_eq lattice) (A.to_regexp auto));
    fprintf fmt "automaton:@.@[%a@]@." (A.print print_letter) auto
  ;;

  let print_dot (title : string) fmt (auto : unit A.t) : unit =
    let info = LAutomaton.Graph.info auto in
    let title =
      sprintf "%s\\ndet=%b,min=%b" title info.LAutomaton.det info.LAutomaton.min
    in
    A.print_dot ~title print_letter fmt auto
  ;;

  let letters (l : char list) : unit PL.t =
    List.fold_left (fun res c -> Alphabet.Map.add c () res) Alphabet.Map.empty l
  ;;
  ...
  let complementary fmt (auto : unit A.t) : unit =
    Format.fprintf
      fmt
      "auto @[%a@] @. compl @[%a@] @."
      (A.print print_letter)
      auto
      (A.print print_letter)
      (A.complementary auto)
  ;;
  ...
  let itself fmt (auto : unit A.t) : unit =
    ...
    print_dot
      "itself concatenation normalized"
      fmt
      (A.minimise (A.Language.concat auto auto));
      ...
      A.Language.of_regexp
        ...
        (Regexp.Exp
           (Regexp.Concat
              ...
              ; Regexp.Plus
                  (Regexp.Concat
                     [ Regexp.Letter (letter 'a'); Regexp.Letter (letter 'b') ])
              ]))
    in
      ...
      A.of_regexp
        ...
        (Regexp.Exp
           (Regexp.Concat
              ...
              ; Regexp.Star
                  (Regexp.Concat
                     [ Regexp.Letter (letter 'd'); Regexp.Letter (letter 'd') ])
              ; Regexp.Letter (letters [ 'd'; 'e' ])
              ; Regexp.Letter (letter 'e')
              ; Regexp.Letter (PL.sep lattice)
              ; Regexp.Plus
                  (Regexp.Concat
                     [ Regexp.Letter (letters [ 'e'; 'f' ]); Regexp.Letter (letter 'g') ])
...
module Int = struct
  module L = struct
    ...
    let widening (a1, b1) (a2, b2) =
      ...
      if is_bottom (a1, b1)
      then a2, b2
      else (if a2 < a1 then min_int else a1), if b2 > b1 then max_int else b1
    ;;
  end
  ...
  let print_interval fmt ((a, b) as elt) =
    if L.is_bottom elt then pp_print_string fmt "bot" else fprintf fmt "[%i,%i]" a b
  ;;

  let print_letter fmt (letter : (int * int) PL.t) =
    PL.print pp_print_char print_interval fmt letter
  ;;
  ...
  let letters (l : (char * L.t) list) : L.t Alphabet.Map.t =
    List.fold_left (fun res (c, elt) -> Alphabet.Map.add c elt res) Alphabet.Map.empty l
  ;;
  ...
  let main () =
    ...
    let word1 =
      A.Language.cons_right
        word1
        (letters [ 'a', (-30, -30); 'b', (-20, -20); 'c', (-10, -10) ])
    in
