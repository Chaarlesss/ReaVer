...
module type S = sig
  ...
  module LAuto : LAutomaton.S with module A = PL.A and module PL = PL

  type 'a t = 'a LAuto.t ref

  val print : (Format.formatter -> 'a PL.t -> unit) -> Format.formatter -> 'a t -> unit
  ...
  val is_prefix : projection:('a -> PL.A.t * 'b -> 'b) -> 'a list -> 'b t -> int -> bool
  ...
  module type Operations = sig
    ...
    val pop_partitioned : ?cond:'a PL.t -> 'a t -> int -> ('a t * 'a) PL.A.Map.t
    val drop : 'a t -> int -> 'a t
    val queue : 'a t -> int -> 'a PL.t -> 'a t
    val firstout : ?cond:'a PL.t -> 'a t -> int -> 'a PL.t
    val dequeue : ?cond:'a PL.t -> 'a t -> int -> 'a t * 'a PL.t
    val dequeue_partitioned : ?cond:'a PL.t -> 'a t -> int -> ('a t * 'a) PL.A.Map.t
  end
...
module Make (LAuto : LAutomaton.S) = struct
  ...
  let check_dim auto i msg =
    ...
    then
      failwith
        (Format.sprintf
           "%s: the dimension %i is out of range of the dimensionality %i of the \
            decision automaton "
           msg
           i
           dim)
  ...
  let print_dot ?margin ?titlestyle ?vertexstyle ?edgestyle ?title f fmt auto =
    LAuto.print_dot ?margin ?titlestyle ?vertexstyle ?edgestyle ?title f fmt !auto
  ;;

  let print_regexp f fmt auto =
    Regexp.print f fmt (Regexp.simplify ( = ) (LAuto.to_regexp (LAuto.determinise !auto)))
  ;;
  ...
  let get_size auto = LAuto.nb_states !auto
  let f_determinise auto = ref (LAuto.determinise !auto)
  let f_minimise auto = ref (LAuto.minimise !auto)
  let determinise auto = auto := LAuto.determinise !auto
  let canonicalize auto = auto := LAuto.minimise !auto
  let minimise = canonicalize
  let complementary auto = ref (LAuto.complementary !auto)

  let is_prefix ~projection w auto i =
    check_dim auto i "is_prefix";
    LAuto.is_prefix ~projection w !auto 0
  ;;
  ...
  let bottom ~lattice ~partition dim = ref (LAuto.bottom ~lattice ~partition dim)
  let top ~lattice ~partition dim = ref (LAuto.top ~lattice ~partition dim)
  let allempty ~lattice ~partition dim = ref (LAuto.allempty ~lattice ~partition dim)
