...
let equal (letter_equal : 'a -> 'a -> bool) (e1 : 'a t) (e2 : 'a t) : bool =
  match e1, e2 with
  | Empty, Empty -> true
...
let simplify_union (letter_equal : 'a -> 'a -> bool) (lexp : 'a exp list) : 'a exp list =
  ...
  let rec subsume (res : 'a exp list) (lexp : 'a exp list) : 'a exp list =
    ...
    | [] -> res
    | x :: rest ->
      if List.exists (is_subsumed x) res || List.exists (is_subsumed x) rest
      then subsume res rest
