...
module type S = sig
  ...
  val project : project:('a -> A.t * 'b -> 'b) -> 'a -> 'b lattice -> 'b t -> 'b t
  val merge : empty:'b -> merge:(A.t * 'a -> 'b -> 'b) -> 'a t -> 'b
...
module Make (A : Alphabet) = struct
  ...
  module SetAA = Sette.Make (struct
      ...
      let compare (x1, y1) (x2, y2) =
        ...
        if res = 0 then A.Set.Ord.compare y1 y2 else res
      ;;
    end)
  ...
  let is_sep pletter =
    ...
    match map with
    | Mappe.Node (Mappe.Empty, x, data, Mappe.Empty, _) -> A.Set.Ord.compare x A.sep = 0
    | _ -> false
  ;;
  ...
  let normalize lattice p =
    A.Map.fold
      (fun letter elt res ->
         if lattice.is_bottom letter elt then A.Map.remove letter res else res)
      p
      p
  ;;
  ...
  let meet lattice pelt1 pelt2 =
    if pelt1 == pelt2
    then pelt1
    else normalize lattice (A.Map.commoni lattice.meet pelt1 pelt2)
  ;;

  let join lattice pelt1 pelt2 =
    if pelt1 == pelt2 then pelt1 else A.Map.mergei lattice.join pelt1 pelt2
  ;;
  ...
  let widening lattice ~partition pelt1 pelt2 =
    ...
    then pelt1
      ...
      A.Map.mergei
        (fun l elt1 elt2 ->
           lattice.meet l (lattice.widening l elt1 elt2) (A.Map.find l partition))
        pelt1
        pelt2
  ;;
  ...
  let project
    ...
    =
    A.Map.fold
      (fun (letter : A.t) (abs : 'b) (res : 'b t) ->
         let (inter : 'b) = project elt (letter, abs) in
         if not (lattice.is_bottom letter inter) then A.Map.add letter inter res else res)
      p
      A.Map.empty
  ;;

  let merge ~(empty : 'b) ~(merge : A.t * 'a -> 'b -> 'b) (p : 'a t) : 'b =
    A.Map.fold (fun letter elt res -> merge (letter, elt) res) p empty
  ;;
  ...
  let rec lcm
    ...
    then (
      ...
      let res = lcm ~coherent:false ~meet (A.Map.diffset p1 set) (A.Map.diffset p2 set) in
      A.Map.fold (fun x elt res -> MapAA.add (x, x) elt res) common res)
    else (
        ...
        (fun l1 elt1 ->
           A.Map.iter
             (fun l2 elt2 ->
                ...
                match elt with
                | None -> ()
                | Some elt -> res := MapAA.add (l1, l2) elt !res)
             p2)
        p1;
      !res)
  ;;
      ...
      MapAA.fold
        (fun (l1, l2) elt map ->
           try
             ...
             map
           with
           | Not_found -> A.Map.add l2 (ref elt) map)
        lcm
        A.Map.empty
    in
