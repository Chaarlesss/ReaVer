...
module Make (LAuto : LAutomaton.S) = struct
  ...
  let print f fmt auto = Print.array (LAuto.print f) fmt auto
  ...
  let get_size auto =
    let f_fold g res = res + LAuto.nb_states g in
    Array.fold_right f_fold auto 0
  ;;
  ...
  let complementary auto = Array.map LAuto.complementary auto

  let is_prefix ~projection w auto i =
    check_dim auto i "is_prefix";
    LAuto.is_prefix ~projection w auto.(i) 0
  ;;
  ...
  let is_bottom auto = Array.fold_left (fun res a -> res || LAuto.is_bottom a) false auto
  ...
  let widening_shape (abstract : 'a LAuto.t -> 'a LAuto.t) auto1 auto2 =
    assert (Array.length auto1 = Array.length auto2);
    Array.init (Array.length auto1) (fun i ->
      LAuto.widening_shape abstract auto1.(i) auto2.(i))
  ;;
    ...
    let pop_partitioned ?cond auto i =
      ...
      PL.A.Map.mapi
        (fun letter elt ->
           ...
           resauto.(i)
           <- T.deriv ~cond:(PL.A.Map.add letter elt PL.A.Map.empty) resauto.(i) 0;
           resauto, elt)
        mapA_L
    ;;
    ...
    let dequeue_partitioned ?cond auto i =
      ...
      PL.A.Map.mapi
        (fun letter elt ->
           let resauto = Array.copy auto in
           resauto.(i)
           <- T.deriv_dual ~cond:(PL.A.Map.add letter elt PL.A.Map.empty) resauto.(i) 0;
           resauto, elt)
        mapA_L
    ;;
  end
