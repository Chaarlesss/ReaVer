...
let fixedpoint env doman image s_start =
  ...
  while not (Bddapron.Domain0.is_eq doman !s !s_next) do
    ...
    Log.debug3_o logger (BddapronUtil.print_abstract env doman) "s_next = " !s_next
  done;
  !s_next
;;
...
let fixedpoint2 env cond init image =
  ...
  while not (Bddapron.Expr0.Bool.is_eq env cond !s !s_next) do
    ...
    Log.debug3_o logger (BddapronUtil.print_boolexpr env cond) "s_next = " !s_next
  done;
  !s_next
;;
  ...
  let sres =
    ...
    | `Backward ->
      Bddapron.Domain0.meet_condition
        ...
        env
        cond
        (Bddapron.Domain0.substitute_lexpr doman env cond s vars exprs None)
        g
  in
...
let check_reach ?(dir = `Forward) env cond doman g equs ivars phi1 phi2 =
  ...
  let s_res =
    image
      ...
      equs
      ivars
      (BddapronUtil.boolexpr_to_abstract env cond doman phi1)
  in
  Log.debug3_o logger (BddapronUtil.print_abstract env doman) "s_res = " s_res;
  let res =
    not
      (Bddapron.Domain0.is_bottom
         doman
         (Bddapron.Domain0.meet_condition doman env cond s_res phi2))
  in
...
let pre_assertion env cond doman g equs phi1 phi2 =
  ...
  let s_pre = Bddapron.Domain0.substitute_lexpr doman env cond s vars exprs None in
  Log.debug3_o logger (BddapronUtil.print_abstract env doman) "s_pre = " s_pre;
  let ass =
    Bddapron.Expr0.Bool.tdrestrict
      (BddapronUtil.abstract_to_boolexpr env cond doman s_pre)
      (Bddapron.Expr0.Bool.dand env cond phi1 g)
  in
...
let bool_reach ?(dir = `Forward) env cond doman g bequs in_vars phi =
  let s = BddapronUtil.boolexpr_to_abstract env cond doman phi in
  let s_reach = fixedpoint env doman (image ~dir env cond doman g bequs in_vars) s in
...
let check_bool_reach ?(dir = `Forward) env cond doman g bequs in_vars phi1 phi2 =
  ...
  let s_res =
    Bddapron.Domain0.meet_condition
      ...
      env
      cond
      (image ~dir env cond doman g bequs in_vars s)
      phi2
  in
...
let get_freach env cond phi1 ass2 fexprlist =
  ...
  List.fold_left
    (fun faccu fe -> Bddapron.Expr0.Bool.dand env cond faccu fe)
...
let bool_image2 get_unprimed_var env cond fexprlist assertion forget_supp s =
  ...
  let fexpr =
    List.fold_left
      (fun faccu feq -> Bddapron.Expr0.Bool.dand env cond faccu feq)
      sass
      fexprlist
  in
...
let bool_pre_assertion2 get_primed_var env cond sinv dinv equs assertion invars =
  ...
  let ass =
    Bddapron.Expr0.Bool.tdrestrict
      s_pre
      (Bddapron.Expr0.Bool.dand env cond sinv assertion)
  in
  Log.debug3_o logger (BddapronUtil.print_boolexpr env cond) "pre-ass = " ass;
  ass
;;
...
let bool_reach2
  ...
  Log.debug3_o
    logger
    (Util.list_print env.Bdd.Env.symbol.Bdd.Env.print)
    "invars = "
    invars;
  let svars, _ = List.split bequs in
  let fnext, forget_vars =
    match dir with
    | `Forward -> bool_image2 get_unprimed_var, List.append svars invars
    | `Backward ->
      ( bool_preimage2 get_primed_var invars
      , List.append (List.map get_primed_var svars) invars )
  in
  Log.debug3_o
    logger
    (Util.list_print env.Bdd.Env.symbol.Bdd.Env.print)
    ...
    forget_vars;
  let forget_supp =
    Cudd.Bdd.dand cond.Bdd.Cond.supp (BddapronUtil.supp_of_vars env forget_vars)
  in
  ...
  fixedpoint2 env cond s_start (fnext env cond fexprlist assertion forget_supp)
;;
...
let is_id_bequs2 get_primed_var get_unprimed_var env cond g bequs phi =
  let phivars = Bddapron.Expr0.support env cond (Bddapron.Expr0.Bool.to_expr phi) in
  let inveq, ninveq = List.partition (fun (v, _) -> PSette.mem v phivars) bequs in
  ...
  let forget_supp =
    Cudd.Bdd.dand cond.Bdd.Cond.supp (BddapronUtil.supp_of_vars env forget_vars)
  in
  let post = bool_image2 get_unprimed_var env cond fexprlist g forget_supp phi in
  let res =
    Bddapron.Expr0.Bool.is_eq env cond post (Cudd.Bdd.exist cond.Bdd.Cond.supp phi)
  in
  Log.debug2_o
    logger
    (BddapronUtil.print_equations env cond)
    "is_id_bequs2: inveq = "
    inveq;
  Log.debug2_o
    logger
    ...
    "is_id_bequs2: ninveq = "
    ninveq;
  Log.debug2_o logger (BddapronUtil.print_boolexpr env cond) "is_id_bequs2: phi  = " phi;
  Log.debug2_o logger (BddapronUtil.print_boolexpr env cond) "is_id_bequs2: post = " post;
