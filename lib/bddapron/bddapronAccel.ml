...
let acc ?(dir = `Forward) ~trans_apronman env cond doman g equs i_num_vars s =
  ...
  Log.debug3_o logger (BddapronUtil.print_equations env cond) "acc: equs = " equs;
  Log.debug3_o logger (BddapronUtil.print_vars env) "acc: i_num_vars = " i_num_vars;
  let sgb =
    Bddapron.Domain0.meet_condition
      ...
      (BddapronUtil.boolexpr_remove_cond
         (BddapronUtil.supp_of_vars env (BddapronUtil.boolvars_of_env env))
         g)
  in
  let sres =
    if not (Bddapron.Domain0.is_bottom doman sgb)
    then (
      ...
      let sacclist =
        List.map
          (fun (sb, sn) ->
             ...
             let sn1 =
               { Apron.Abstract1.abstract0 = sn; Apron.Abstract1.env = apronenv }
             in
             let sn2 =
               ApronAccel.acc
                 ~dir
                 ~trans_man:trans_apronman
                 gn
                 eqs
  ...
  in
  Log.debug3_o logger (BddapronUtil.print_abstract env doman) "acc: post_oplus(s) = " sres;
  sres
;;
...
let get_transition_type_actions env cond i_num_vars vars apronactions =
  let eqs =
    Util.array_map2
      ...
      apronactions
  in
  ApronAccel.get_transition_type eqs (BddapronUtil.vars_to_apronvars env i_num_vars)
;;
...
let get_transition_type_numequs env cond i_num_vars numequs =
  try
    ...
    ApronAccel.get_transition_type eqs (BddapronUtil.vars_to_apronvars env i_num_vars)
  with
  | BddapronUtil.NotLinearAction _ -> ApronAccel.NonAcc
;;
...
let exists_acc_action env cond i_num_vars vars apronactions =
  Util.array_exists2 (is_acc_action env cond i_num_vars) vars apronactions
;;
