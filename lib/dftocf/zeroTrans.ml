...
type kind_t =
  | Continuous of Env.var_t
  | Discrete of Env.var_t
  | DiscCont of Env.var_t * Env.var_t

type guardexpr_t =
  | GContinuous of Env.boolexpr_t
...
type zspec_t =
  ...
  ; qvars : kind_t
    (* q-variables (for location encoding) for 
...
(******************************************************************************)
let get_epsilon env =
  Bddapron.Expr0.Apron.cst env.Env.env env.Env.cond (Apron.Coeff.s_of_float Env.epsilon)
;;
...
let replace_zeros_boolexpr
      (trans_zero : zero_trans_t)
      (env : Env.t)
      zero_defs
      (expr : Env.boolexpr_t)
  : Env.boolexpr_t
  =
  let zero_defs =
    List.map (fun (v, z) -> v, Bddapron.Expr0.Bool.to_expr (trans_zero env z)) zero_defs
  in
...
let print_zdef env fmt zdef = Bddapron.Expr0.Apron.print env.Env.env env.Env.cond fmt zdef
...
let print_zguardmap env fmt zguardmap =
  Util.list_print
    (fun fmt (zvar, g) ->
       Format.pp_print_string fmt zvar;
...
let apron_expr env expr =
  Cudd.Mtbdd.cst_u
    env.Env.cuddman
    (Cudd.Mtbdd.unique env.Env.env.Bdd.Env.ext.Bddapron.Env.table expr)
;;

let apron_ite env c e1 e2 = Bddapron.Expr0.Apron.ite env.Env.env env.Env.cond c e1 e2
let benum_var env q = Bddapron.Expr0.Benum.var env.Env.env env.Env.cond q
let benum_label env l = Bdd.Enum.of_label env.Env.env l

let benum_eq_label env q label =
  Bddapron.Expr0.Benum.eq_label
    env.Env.env
    env.Env.cond
    (Bddapron.Expr0.Benum.var env.Env.env env.Env.cond q)
    label
;;
...
let bool_closed_not env doman e =
  BddapronUtil.boolexpr_topoclose env.Env.env env.Env.cond doman (bool_not env e)
;;

let bool_init env einit e =
  Bddapron.Expr0.Bool.ite
    env.Env.env
    env.Env.cond
...
let bool_ite env c e1 e2 = Bddapron.Expr0.Bool.ite env.Env.env env.Env.cond c e1 e2
let benum_ite env c e1 e2 = Bddapron.Expr0.Benum.ite env.Env.env env.Env.cond c e1 e2
...
let get_purecontset env disc_equs cont_equs =
  ...
  List.fold_right
    (fun v s ->
       let dexpr =
         match List.assoc v disc_equs with
         | `Apron expr -> expr
         | _ -> assert false
       in
       ...
       let rec check i =
         if i < Array.length guardleaves
         then (
           let g, l = guardleaves.(i) in
           if
             (has_init
              && Bddapron.Expr0.Bool.is_true
                   env.Env.env
                   env.Env.cond
                   (Bddapron.Expr0.Bool.eq
                      env.Env.env
                      env.Env.cond
                      g
                      (bool_var env Env.init_var)))
             || Bddapron.Apronexpr.equal
                  Env.symbol
                  l
                  (Bddapron.Apronexpr.var
                     Env.symbol
                     (Bddapron.Env.typ_of_var env.Env.env)
                     v)
           then check (i + 1)
           else false)
         else true
       in
       if check 0 then PSette.add v s else s)
    conteqvars
...
let guardexprs env zspec =
  ...
  let guardexpr_cont q =
    ...
    | AtZero -> bool_and env (trans_zero env eq_zero zspec.zdef) (bool_var env q)
    | Contact ->
      bool_and env (trans_zero env eq_zero zspec.zdef) (benum_eq_label env q l_ready)
    | Crossing ->
      bool_and
        env
    ...
    (match zspec.cont_sem with
     | AtZero ->
       bool_or
         ...
         (bool_and
            env
            (bool_not env (bool_var env q))
            (trans_zero env geq_zero zspec.zdef))
         (bool_and env (bool_var env q) (trans_zero env leq_zero zspec.zdef))
     | Contact ->
       bool_or
         env
         (bool_and
         ...
         (bool_and
            env
            (benum_eq_labels env q [ l_below; l_ready ])
      ...
      let qexpr =
        bool_init
          ...
          (bool_ite
             env
             (bool_var env q)
             (bool_ite env (bool_var env qb1) (bool_t env) (bool_not env guard))
             (bool_ite
                env
                (bool_var env qb2)
                (trans_zero env leq_zero zspec.zdef)
      ...
      let qexpr =
        benum_init
          ...
          (benum_ite
             env
             (bool_var env qb0)
             (benum_label env l_above)
             (benum_label env l_below))
          (benum_ite
             env
             (benum_eq_label env q l_above)
             (benum_ite
                env
                (bool_and env (bool_var env qb1) (trans_zero env leq_zero zspec.zdef))
                (* eq *)
                (benum_label env l_below)
                (benum_label env l_above))
             (benum_ite
                ...
                (benum_ite
                   env
                   (bool_and env (bool_var env qb2) (trans_zero env lt_zero zspec.zdef))
                   (benum_label env l_ready)
                   (benum_label env l_below))
                (* l_ready *)
                (benum_ite
                   env
                   (bool_and env (bool_var env qb3) guard)
                   (benum_ite
                      env
                      (bool_var env qb4)
                      (benum_label env l_above)
        ...
        benum_init
          ...
          (benum_ite
             ...
             (benum_ite
                ...
                (benum_ite
                   env
                   (bool_and env (bool_var env qb2) (trans_zero env geq_zero zspec.zdef))
                   (*eq*)
                   (benum_label env l_ready)
                   (benum_label env l_below))
                (benum_ite
...
let get_zero_specs env dfprog cont_sem disc_sem zvars =
  ...
  let comp_spec zvar =
    ...
    let all_cont = PSette.is_empty (PSette.diff (PSette.diff supp purecontset) iset) in
    ...
    let qvars =
      if all_cont
      then Continuous (Env.get_q_var zvar true)
      else if no_cont
      then Discrete (Env.get_q_var zvar false)
      else DiscCont (Env.get_q_var zvar false, Env.get_q_var zvar true)
    in
    let zspec = { zvar; disc_sem; cont_sem; zdef; qvars } in
    zspec
  in
...
let extend_environment env param zspecs =
  ...
  Bddapron.Env.add_typ_with env.Env.env q_state3 (`Benum [| l_above; l_below; l_ready |]);
  ...
  let gen_variables_for_zspec zspec =
    let new_disc_q_varlist, new_cont_q_varlist =
      match zspec.qvars with
      | Discrete q -> [ q ], []
      | Continuous q -> [], [ q ]
      | DiscCont (q1, q2) -> [ q1 ], [ q2 ]
    in
    let new_q_varstyp, qivars_cnt =
      match zspec.qvars with
      | Discrete q -> [ q, `Bool ], 1
      | Continuous q ->
        (match zspec.cont_sem with
    ...
    new_q_varstyp, new_qi_varstyp, new_disc_q_varlist, new_cont_q_varlist
  in
  (* iterate through zero-crossing specs *)
  let q_varstyp, qi_varstyp, disc_q_varlist, cont_q_varlist =
    List.fold_right
      (fun zspec (q_varstyp, qi_varstyp, disc_q_varlist, cont_q_varlist) ->
         let new_q_varstyp, new_qi_varstyp, new_disc_q_varlist, new_cont_q_varlist =
           gen_variables_for_zspec zspec
         in
  ...
  in
  (* variable lists *)
  let new_i_q_varstyp = List.map (fun (v, t) -> Env.get_newinput env v, t) q_varstyp in
  let primed_q_varstyp =
    List.map (fun (v, t) -> BddapronUtil.get_primed_var env.Env.env v, t) q_varstyp
  in
  ...
  (* add the variables *)
  let varstyp =
    List.concat [ q_varstyp; primed_q_varstyp; new_i_q_varstyp; qi_varstyp ]
  in
  ...
  let newenv =
    ...
    ; Env.b_vars =
        List.concat
          [ env.Env.b_vars; q_varlist; new_i_q_varlist; primed_q_varlist; qi_varlist ]
...
let translate_cont_eqs env continter f =
  let translate_cont_eq (v, e) =
    ...
    , Bddapron.Expr0.ite
        ...
        e
        (`Apron
            (Bddapron.Expr0.Apron.cst env.Env.env env.Env.cond (Apron.Coeff.s_of_int 0)))
    )
  in
...
let translate env dfprog disc_sem cont_sem =
  ...
  let doman =
    Bddapron.Domain0.man_of_bdd
      (Bddapron.Domain0.make_bdd (Polka.manager_alloc_strict ()))
  in
  (* translates the given zero-crossing spec *)
  let do_zero_trans (gexprlist, zguardmap, q_equations, staycond, continter) zspec =
    Log.debug_o logger (print_zspec env) "translating zero-crossing: " zspec;
    ...
    Log.debug_o logger (Env.print_equations env) "q_equations: " new_q_equations;
    Log.debug_o logger (Env.print_boolexpr env) "staying condition: " new_staycond;
    ( gexprs :: gexprlist
  ...
  let _, zguardmap, q_equations, staycond, continter =
    List.fold_left do_zero_trans ([], [], [], bool_t env, bool_t env) zspecs
  in
  ...
  let new_disc_equs =
    BddapronUtil.simplify_equs
      env.Env.env
      env.Env.cond
      (List.concat [ f_no_z; translate_disc_eqs env zguardmap f_z; q_equations ])
  ...
  Log.debug_o
    logger
    (Env.print_equations env)
    "translated discrete equations: "
    new_disc_equs;
  let staycond = Cudd.Bdd.tdrestrict staycond env.Env.cond.Bdd.Cond.careset in
  Log.debug_o logger (Env.print_boolexpr env) "staying condition: " staycond;
  let cfprog =
    Program.make_cfprog
      (Cfg.make
         env
