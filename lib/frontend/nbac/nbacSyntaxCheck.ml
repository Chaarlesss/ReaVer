...
let typ_of_label decl s =
  ...
  in
  if found then typ else raise Not_found
;;

let typ_of_var decl s =
  try Mappe.find s decl.Program.state with
  | Not_found ->
    (try Mappe.find s decl.Program.input with
     | Not_found ->
       (try Mappe.find s decl.Program.local with
        | Not_found -> typ_of_label decl s))
;;
  ...
  let rec parcours = function
    | Cst cst ->
      (match cst with
       ...
       | `Apron _ -> `Real)
    | Ref s ->
      (try typ_of_var decl s with
       | Not_found ->
         raise
           (NbacParseError
              (sprintf "Error: in expression, undeclared variable or label %s" s)))
    | Unop (op, e) ->
      (match op with
       | `Not -> `Bool
       | `Up -> `Bool
    ...
    | Cst cst ->
      ...
      then
        raise
          (NbacParseError
             (sprintf
                "constant %s:%s has not the expected type %s"
                (string_of_cst cst)
                (string_of_typ t)
                (string_of_typ typ)))
    | Ref var ->
      ...
      then
        raise
          (NbacParseError
             (sprintf
                "symbol %s:%s has not the expected type %s"
                var
                (string_of_typ t)
                (string_of_typ typ)))
    | Unop (op, e) ->
...
let check_definition_transition prog =
  ...
  and setsymbol2 = Mappe.maptoset prog.jump in
  if not (Sette.subset setsymbol1 setsymbol2)
  then (
    let symbol = Sette.choose (Sette.diff setsymbol1 setsymbol2) in
    raise
      (NbacParseError
         (sprintf
            "Error: declared state variable %s is not given a transition function"
            symbol)))
  else if not (Sette.subset setsymbol2 setsymbol1)
  then (
    let symbol = Sette.choose (Sette.diff setsymbol2 setsymbol1) in
    raise
      (NbacParseError
         (sprintf
            "transition function specified for the undeclared state variable %s"
            symbol)));
  check_typequations prog.jump;
  check_typequations prog.flow;
  (* Local variables and definitions *)
  let setsymbol1 = Mappe.maptoset decl.Program.local
  and setsymbol2 = Mappe.maptoset prog.defs in
  if not (Sette.subset setsymbol1 setsymbol2)
  then (
    let symbol = Sette.choose (Sette.diff setsymbol1 setsymbol2) in
    raise
      (NbacParseError
         (sprintf "Error: declared local variable %s is not given a definition" symbol)))
  else if not (Sette.subset setsymbol2 setsymbol1)
  then (
    let symbol = Sette.choose (Sette.diff setsymbol2 setsymbol1) in
    raise
      (NbacParseError
         (sprintf "definition specified for the undeclared local variable %s" symbol)));
  ...
  then (
    ...
    let support expr =
      let rec parcours supp = function
        | Cst _ -> supp
        | Ref nom -> if Sette.mem nom locals then Sette.add nom supp else supp
    ...
    (* vertices *)
    Mappe.iter (fun symbol _ -> g := FGraph1.add_vertex !g symbol ()) prog.defs;
    (* arcs *)
    Mappe.iter
      (fun symbol expr ->
         let supp = support expr in
         if Sette.mem symbol supp
         then
           failwith
             ("Syntax.sort_definitions: the definition of "
              ^ symbol
              ^ " is self recursive !");
         Sette.iter (fun var -> g := FGraph1.add_edge !g (symbol, var) ()) supp)
      prog.defs;
    (* topological sort *)
    let sroot = ref Sette.empty in
    FGraph1.iter_vertex !g (fun vertex _ _ ->
      if Sette.is_empty (FGraph1.pred !g vertex) then sroot := Sette.add vertex !sroot;
      ());
    if Sette.is_empty !sroot
    then failwith "Syntax.sort_definitions: some definitions are mutually recursive !")
;;
