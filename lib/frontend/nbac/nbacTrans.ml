...
let logger =
  { Log.fmt = Format.std_formatter; Log.module_name = "NbacTrans"; Log.level = Log.Debug }
;;
...
let sort_eqs eqs statevars inputvars labels =
  ...
  let rec var_set expr : NbacExpr.symtype Sette.t =
    ...
    | NbacExpr.If (e1, e2, e3) ->
      Sette.union (Sette.union (var_set e1) (var_set e2)) (var_set e3)
    | NbacExpr.Excl le ->
      List.fold_left (fun lle ee -> Sette.union (var_set ee) lle) Sette.empty le
    | NbacExpr.In (e, le) ->
      List.fold_left (fun lle ee -> Sette.union (var_set ee) lle) (var_set e) le
    | _ -> Sette.empty
  in
  ...
  let eqs_vars2 =
    Mappe.mapi
      (fun a b -> Sette.diff b (Sette.union (Sette.union statevars inputvars) labels))
      eqs_vars1
  in
  (*  Log.debug3_o logger (fun fmt vars -> Mappe.print Format.pp_print_string
      (Sette.print Format.pp_print_string) fmt vars)
  ...
  let rec sort eqs_varset =
    ...
    if Mappe.is_empty indep && not (Mappe.is_empty dep)
    then raise (Failure "definitions contain circular dependencies");
    let indepset = Mappe.maptoset indep in
    let reddep = Mappe.mapi (fun a b -> Sette.diff b indepset) dep in
    if Mappe.is_empty reddep
    then Util.mappe2list (Mappe.interset eqs indepset)
  ...
  let rec traverse
    ...
    | NbacExpr.Unop (op, e) ->
      (match op with
       ...
       | `Up ->
         ...
         let zvar =
           try Env.find_zero_var zs zexpr with
           | Not_found -> Env.get_newzero_var env
         in
       ...
       | `Apron (unop, typ, round) ->
         (match unop with
          ...
          | Apron.Texpr0.Cast ->
            let ee, zs = tr_apronexpr e zeros in
            `Apron (Bddapron.Expr0.Apron.cast env0 cond0 ~typ ~round ee), zs
          | Apron.Texpr0.Sqrt ->
            let ee, zs = tr_apronexpr e zeros in
            `Apron (Bddapron.Expr0.Apron.sqrt env0 cond0 ~typ ~round ee), zs))
       ...
       | `Bool bbinop ->
         ...
         (match bbinop with
          ...
          | NbacExpr.NEQ ->
            ...
            ( `Bool
                (Bddapron.Expr0.Bool.dnot
                   env0
                   cond0
                   (Bddapron.Expr0.eq env0 cond0 ee1 ee2))
            , zs )
          | NbacExpr.GT ->
            let ee1, zs = traverse e1 treqs zeros in
            let ee2, zs = traverse e2 treqs zs in
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Bool (app_apron_bbinop Bddapron.Expr0.Apron.sup eee1 eee2), zs
             | `Bint eee1, `Bint eee2 ->
               `Bool (app_bint_bbinop Bddapron.Expr0.Bint.sup eee1 eee2), zs
          ...
          | NbacExpr.GEQ ->
            ...
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Bool (app_apron_bbinop Bddapron.Expr0.Apron.supeq eee1 eee2), zs
             | `Bint eee1, `Bint eee2 ->
               `Bool (app_bint_bbinop Bddapron.Expr0.Bint.supeq eee1 eee2), zs
             | _ -> failwith "type mismatch in comparison operator")
          | NbacExpr.LEQ ->
            let ee1, zs = traverse e1 treqs zeros in
            let ee2, zs = traverse e2 treqs zs in
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Bool (app_apron_bbinop Bddapron.Expr0.Apron.supeq eee2 eee1), zs
             | `Bint eee1, `Bint eee2 ->
               `Bool (app_bint_bbinop Bddapron.Expr0.Bint.supeq eee2 eee1), zs
             | _ -> failwith "type mismatch in comparison operator")
          | NbacExpr.LT ->
            let ee1, zs = traverse e1 treqs zeros in
            let ee2, zs = traverse e2 treqs zs in
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Bool (app_apron_bbinop Bddapron.Expr0.Apron.sup eee2 eee1), zs
             | `Bint eee1, `Bint eee2 ->
               `Bool (app_bint_bbinop Bddapron.Expr0.Bint.sup eee2 eee1), zs
             | _ -> failwith "type mismatch in comparison operator"))
       | `Apron (binop, typ, round) ->
         ...
         (match binop with
          | Apron.Texpr0.Add ->
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Apron (app_apron_binop Bddapron.Expr0.Apron.add eee1 eee2), zs
             | `Bint eee1, `Bint eee2 ->
               `Bint (app_bint_binop Bddapron.Expr0.Bint.add eee1 eee2), zs
             | _ -> failwith "type mismatch in arithmetic operator")
          | Apron.Texpr0.Sub ->
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Apron (app_apron_binop Bddapron.Expr0.Apron.sub eee1 eee2), zs
             | `Bint eee1, `Bint eee2 ->
               `Bint (app_bint_binop Bddapron.Expr0.Bint.sub eee1 eee2), zs
             | _ -> failwith "type mismatch in arithmetic operator")
          | Apron.Texpr0.Mul ->
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Apron (app_apron_binop Bddapron.Expr0.Apron.mul eee1 eee2), zs
             | `Bint eee1, `Bint eee2 ->
               `Bint (app_bint_binop Bddapron.Expr0.Bint.mul eee1 eee2), zs
             | _ -> failwith "type mismatch in arithmetic operator")
          | Apron.Texpr0.Div ->
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Apron (app_apron_binop Bddapron.Expr0.Apron.div eee1 eee2), zs
             | `Bint eee1, `Bint eee2 ->
          ...
          | Apron.Texpr0.Mod ->
            (match ee1, ee2 with
             | `Apron eee1, `Apron eee2 ->
               `Apron (app_apron_binop Bddapron.Expr0.Apron.gmod eee1 eee2), zs
             | `Bint eee1, `Bint eee2 ->
    ...
    | NbacExpr.Excl le ->
      ...
      let rec atmost1true trlrest e =
        ...
        | hd :: tl ->
          if hd = e
          then Bddapron.Expr0.Bool.dand env0 cond0 hd (allfalse tl)
          else
            Bddapron.Expr0.Bool.dand
              env0
              cond0
              (Bddapron.Expr0.Bool.dnot env0 cond0 hd)
              (atmost1true tl e)
      in
      (* (e_1 & -...) or (-e1 & e2 & -...) or ...(-... & e_n) *)
      let rec disj trl trlrest =
        ...
        | [] -> allfalse trl
        | hd :: tl ->
          Bddapron.Expr0.Bool.dor env0 cond0 (atmost1true trlist hd) (disj trl tl)
      in
...
let translate_prog prog env =
  ...
  let trdefs, zeros0 =
    List.fold_left
      ...
      (sort_eqs
         ...
         labels)
  in
  Log.debug3_o logger (Env.print_zero_defs env) "zero_defs0: " zeros0;
  ...
  let jump, zeros1 =
    List.fold_right
      ...
      ([], zeros0)
  in
  Log.debug3_o logger (Env.print_zero_defs env) "zero_defs1: " zeros1;
  let flow, zeros =
    List.fold_right
      (fun (v, e) (eqs, zzs) ->
         let ee, zs = translate_expr env e trdefs zzs in
      ...
      ([], zeros1)
  in
  Log.debug3_o logger (Env.print_zero_defs env) "zero_defs: " zeros;
...
let parse ?(is_file = true) input =
  let prog =
    if is_file && input <> ""
    then Parse.parse_file ~lexer:NbacLexer.lexBac ~parser:NbacParser.prog input
    else if (not is_file) && input <> ""
    then
      Parse.parse_lexbuf
        ~lexer:NbacLexer.lexBac
        ~parser:NbacParser.prog
  ...
  let default_df2cf, default_strat =
    if Mappe.is_empty prog.NbacExpr.flow
    then default_df2cf_d, default_strat_d
