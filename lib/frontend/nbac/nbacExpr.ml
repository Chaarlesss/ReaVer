...
let print_unop fmt (op : unop) : unit =
  pp_print_string
    ...
    (match op with
     | `Not -> "not"
     | `Up -> "up"
     | `Apron (unop, typ, round) -> Apron.Texpr0.print_sprint_unop unop typ round)
;;
...
let print_binop fmt (op : binop) =
  ...
  | `Apron (op, typ, round) ->
    pp_print_string fmt (Apron.Texpr0.print_sprint_binop op typ round)
;;
...
let print_expr (fmt : Format.formatter) (expr : 'ref expr) =
  ...
  let rec print_expr ?(specialif = false) fmt (expr : 'a expr) =
    match expr with
    ...
    | In (expr, lexpr) ->
      ...
      fprintf
        ...
        expr
        (if par1 then ")" else "")
        (Print.list ~first:"@[<hov>" ~sep:",@," ~last:"@]" (fun fmt expr ->
           let prec1 = precedence_of_expr expr in
           let par1 = prec1 <= prec in
           fprintf
             fmt
...
let is_true = function
  | Cst (`Bool true) | Unop (`Not, Cst (`Bool false)) -> true
  | _ -> false
;;

let is_false = function
  | Cst (`Bool false) | Unop (`Not, Cst (`Bool true)) -> true
  | _ -> false
;;
...
let rec support ?filter res expr =
  ...
  | In (e, le) -> List.fold_left (support ?filter) (support ?filter res e) le
  | Excl e ->
    (match e with
     | [] -> res
     | hd :: tl -> List.fold_left (support ?filter) (support ?filter res hd) tl)
;;

let print_declarations (fmt : Format.formatter) (decl : Program.declaration_t) : unit =
  (* Type definitions *)
  if PMappe.is_empty decl.Program.typdef
  then
    PMappe.print
      ~first:"typedef@.   @[<v>"
      ~sep:"@ "
  ...
  Mappe.print
    ...
    decl.Program.state;
  (* Input variables *)
  if decl.Program.input <> Mappe.empty
  then
    Mappe.print
      ~first:"input@.   @[<v>"
      ~sep:"@ "
      ...
      decl.Program.input;
  (* Local variables *)
  if decl.Program.local <> Mappe.empty
  then
    Mappe.print
      ~first:"local@.   @[<v>"
      ~sep:"@ "
...
let print_definitions (fmt : Format.formatter) (defs : (string, 'a expr) Mappe.t) : unit =
  if defs <> Mappe.empty
  then
    Mappe.print
      ~first:"definition@.  @[<v>"
...
let print (fmt : Format.formatter) (prog : prog) =
  print_declarations fmt prog.decl;
  print_definitions fmt prog.defs;
  print_transitions fmt prog.jump prog.flow;
  fprintf fmt "assertion %a;@." print_expr prog.assertion;
  fprintf fmt "initial %a;@." print_expr prog.initial;
  fprintf fmt "final %a;@." print_expr prog.final;
  ()
;;
