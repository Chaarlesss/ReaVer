...
let flattenp
  ...
  =
  let rec rev_flatten
            (res : ('a, 'b) Ilist.el list)
            (cdepth : int)
            (parent : 'a)
            (ilist : ('a, 'b) Ilist.el list)
    : ('a, 'b) Ilist.el list
    =
    match ilist with
    | [] -> res
    | x :: l ->
      let nres =
        ...
        | Ilist.Atome _ -> x :: res
        | Ilist.List (b, l2) ->
          if depth <= cdepth && flatten ~parent (b, l2)
          then rev_flatten res (cdepth + 1) b l2
          else (
...
let make_strategy_vertex
  ...
  in
  let strategy_vertex =
    { FixpointType.vertex; FixpointType.hedges; FixpointType.widen }
  in
  strategy_vertex
;;
...
let ilist_map
  ...
  let rec parcours flag = function
    | [] -> []
    | Ilist.Atome a :: l -> Ilist.Atome (fa flag a) :: parcours false l
    | Ilist.List (b, l2) :: l ->
...
let make_strategy_widening_start
      ...
      ~(flatten :
         parent:FixpointType.strategy_iteration
  ...
  let scfc : ('vertex, 'hedge) Fixpoint.strategy =
    ilist_map
      (fun level0 l ->
         if level0
         then FixpointType.make_strategy_iteration ~widening_start:0 ()
         else FixpointType.make_strategy_iteration ~widening_start:(widening_start l) ())
