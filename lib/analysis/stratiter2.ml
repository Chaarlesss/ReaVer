...
*)
type strategy_leaf_t =
  Env.var_t BddapronUtil.boolexpr_t * ApronUtil.linconss_t * ApronUtil.equs_t
...
type stratcfg_t =
  ( int
    , int
    , int * Env.var_t BddapronUtil.boolexpr_t
    , ApronUtil.linconss_t * ApronUtil.equs_t
    , info_t )
    PSHGraph.t
...
let print_strategies env fmt strats =
  ...
  MtbddUtil.print_base_mtbdd
    env.Env.env
    env.Env.cond
    (print_strategies_leaf env)
    fmt
    strats
;;
...
let print_strategy_elt env fmt (t_elt, s_elt) =
  ...
  Format.pp_print_string fmt " --> ";
  MtbddUtil.print_base_mtbdd env.Env.env env.Env.cond (print_strategy_leaf env) fmt s_elt
;;

let print_strategy env template fmt (strat : strategy_t) =
  Util.array_iter2
    (fun t_elt s_elt -> print_strategy_elt env fmt (t_elt, s_elt))
...
let print_values env template fmt (s : value_t) =
  Util.array_iter2
    (fun t_elt s_elt ->
       ...
       Template.print_elt fmt t_elt;
       Format.pp_print_string fmt " --> ";
       MtbddUtil.print_base_mtbdd env.Env.env env.Env.cond print_value_leaf fmt s_elt)
    (Array.of_list template)
    s
;;
...
let benum_careset env =
  List.fold_left
    (fun res v ->
       ...
       | `Benum typname ->
         let careset =
           BddapronUtil.get_enum_careset_for_var env.Env.env env.Env.cond v typname
         in
...
let build_strategies strat_table env doman assertion equs =
  ...
  let ga =
    BddapronUtil.numequs_to_guardedactions env.Env.env env.Env.cond ~assertion doman fn
  in
  let benum_careset = benum_careset env in
  let ga =
    List.map (fun (b, g, a) -> Bddapron.Expr0.Bool.tdrestrict b benum_careset, g, a) ga
  in
  let ga =
    List.filter
      (fun (b, g, _) ->
  ...
  in
  let pexprs =
    BddapronUtil.generate_boolencoding
      env.Env.env
      env.Env.cond
      env.Env.p_vars
      (List.length ga)
    ...
    List.map2
      (fun (b, g, a) pexpr ->
         let a =
           Util.array_map2
             (fun v a ->
                v, BddapronUtil.apronaction_to_linexpr env.Env.env env.Env.cond a)
             apronvars
             a
         in
  ...
  let bcompl =
    Bddapron.Expr0.Bool.dnot
      ...
      env.Env.cond
      (List.fold_left
         (fun res (b, _) -> Bddapron.Expr0.Bool.dor env.Env.env env.Env.cond res b)
         (Bddapron.Expr0.Bool.dfalse env.Env.env env.Env.cond)
         ga)
  in
  let bcompl = Bddapron.Expr0.Bool.tdrestrict bcompl benum_careset in
  if Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond bcompl
  then
    MtbddUtil.bddleafarr_to_mtbdd (make_leaf strat_table) env.Env.env (Array.of_list ga)
  else
    MtbddUtil.bddleafarr_to_mtbdd
      (make_leaf strat_table)
      env.Env.env
      (Array.of_list ((bcompl, (ApronUtil.linconss_false env.Env.apronenv, [||])) :: ga))
;;
...
let initial_strategy_value env template value_table strat_table strats initial =
  let strat =
    Array.map
      (fun _ ->
         let id_equs =
           ApronUtil.get_id_equations_for
             env.Env.apronenv
             (BddapronUtil.vars_to_apronvars env.Env.env env.Env.ns_vars)
         in
         MtbddUtil.bddleafarr_to_mtbdd
           (make_leaf strat_table)
           env.Env.env
           [| ( initial
              , ( Bddapron.Expr0.Bool.dtrue env.Env.env env.Env.cond
                , ApronUtil.linconss_true env.Env.apronenv
                , id_equs ) )
            ; ( Bddapron.Expr0.Bool.dnot env.Env.env env.Env.cond initial
              , ( Bddapron.Expr0.Bool.dfalse env.Env.env env.Env.cond
                , ApronUtil.linconss_false env.Env.apronenv
                , id_equs ) )
           |])
      (Array.of_list template)
  in
...
let simplify_strats strats b = Cudd.Mtbdd.tdrestrict strats b
...
let compute_bool_inv env s =
  ...
  Array.fold_left
    (fun inv (b, c) ->
       if not (Apron.Lincons1.is_unsat c)
       then Bddapron.Expr0.Bool.dor env.Env.env env.Env.cond inv b
       else inv)
    (Bddapron.Expr0.Bool.dfalse env.Env.env env.Env.cond)
    bc
;;
...
let get_value_leaf env value_elt boolexpr =
  ...
  let leaf =
    Array.fold_left (fun res (b, v) -> v :: res) [] (Cudd.Mtbdd.guardleafs value)
  in
...
let get_strategies_leaf env strats_table strats boolexpr =
  ...
  let leaf =
    Array.fold_left
      (fun res (b, (g, a)) ->
         if ApronUtil.linconss_is_unsat g then res else (b, g, a) :: res)
      []
      (Cudd.Mtbdd.guardleafs strat)
  in
  assert (List.length leaf = 1);
  (* TODO: sometimes this fails -> check why *)
  List.hd leaf
;;

let update_strategy_elt env strat_table strat boolexpr leaf =
  Cudd.Mtbdd.ite boolexpr (Cudd.Mtbdd.cst env.Env.cuddman strat_table leaf) strat
;;
...
let smt_build_value_elt env t_elt s_elt =
  ...
  in
  let res =
    BddapronSmt.mtbdd_to_smt !val_mtbddmap leaf_to_smt env.Env.env env.Env.cond s_elt
  in
...
let smt_build_strats env strats =
  ...
  in
  let res =
    BddapronSmt.mtbdd_to_smt !nequ_mtbddmap leaf_to_smt env.Env.env env.Env.cond strats
  in
...
let smt_model_filter_vars vars m =
  ...
  List.iter
    (fun v ->
       try Hashtbl.add res v (Hashtbl.find m v) with
       | Not_found -> ())
    vars;
  res
;;
  ...
  BddapronSmt.smt_and
    (List.map
       (fun (v, x) ->
          match x with
          | `Bool x ->
            if Bddapron.Expr0.Bool.is_true env.Env.env env.Env.cond x
            then YicesSmt.Bool v
  ...
  BddapronSmt.smt_and
    (List.map
       (fun (v, x) ->
          match x with
          | YicesSmt.VBool x ->
            if x then YicesSmt.Bool v else YicesSmt.Not (YicesSmt.Bool v)
...
let strategy_improve_elt
  ...
  while !sat1 do
    ...
    | Some mm1 ->
      ...
      while !sat2 do
        ...
        let bpmodel2 =
          smt_model_filter_vars
            (BddapronSmt.vars_to_smtvars env.Env.env env.Env.cond bpvars)
            !m
        in
        let bprime = YicesSmt.instantiate_formula bpmodel2 nff in
        (*         Log.debug3 logger ("smt-bprime: "^(Smt.string_of_formula bprime));*)
        let bprimeexpr =
          BddapronSmt.smt_to_boolexpr ~unprime:true env.Env.env env.Env.cond bprime
        in
        let bisupp =
          BddapronUtil.supp_of_vars
            env.Env.env
            (List.append env.Env.bi_vars env.Env.p_vars)
        in
        ...
        strat_elt := update_strategy_elt env strat_table !strat_elt bprimeexpr leaf;
        let blocked_g =
          smt_build_boolexpr
            env
            (Bddapron.Expr0.Bool.dnot
               env.Env.env
               env.Env.cond
               (BddapronSmt.smt_to_boolexpr env.Env.env env.Env.cond bprime))
        in
        TimeMeas.stop tm_smt_mod;
        ...
        assertion_ids
        := BddapronSmt.smt_assert_retractable_ctx ctx1 blocked_g :: !assertion_ids;
        TimeMeas.stop tm_smt_ctx;
        TimeMeas.start tm_smt;
  ...
  List.iter
    (fun id ->
       let _ = BddapronSmt.smt_retract_ctx ctx1 id in
       ())
    !assertion_ids;
  TimeMeas.stop tm_smt_ctx;
...
let strategy_improve_elt2
  ...
  while !sat1 do
    ...
    | Some mm1 ->
      ...
      let bprimeexpr =
        BddapronSmt.model_to_boolexpr ~unprime:true env.Env.env env.Env.cond bprimemodel
      in
      let bisupp =
        BddapronUtil.supp_of_vars env.Env.env (List.append env.Env.bi_vars env.Env.p_vars)
      in
      let bprimeexpr = BddapronUtil.boolexpr_forget_supp bisupp bprimeexpr in
      (*      Log.debug2_o logger (BddapronUtil.print_boolexpr env.Env.env 
          env.Env.cond) "b'-boolexpr: " bprimeexpr;*)
...
(* Log.debug3 logger ("smt-value0: "^(Smt.string_of_formula smtv0)); *)
  ...
  let strategy_improve_elt, ff0 =
    match param with
    | MicroIter ->
      strategy_improve_elt, BddapronSmt.smt_and [ smtstrats; smtbequs; smtenum; smtv0 ]
    | NoMicroGenIter ->
      strategy_improve_elt4, BddapronSmt.smt_and [ smtstrats; smtbequs; smtenum; smtv0 ]
    ...
    Util.list_fold_lefti
      (fun i f t_elt ->
         ...
         YicesSmt.Or
           ...
           , YicesSmt.And
               ( YicesSmt.Eq
                   ( YicesSmt.Real template_row_var
                   , YicesSmt.Const
...
*)
      let i =
        match Hashtbl.find mm1 template_row_var with
      ...
      (*      Log.debug2_o logger (BddapronUtil.print_boolexpr env.Env.env 
          env.Env.cond) "b'-boolexpr: " bprimeexpr;*)
      strat.(i) <- update_strategy_elt env strat_table strat.(i) bprimeexpr leaf;
      (*
(* boolean encoding of template rows *)
  ...
  while !sat1 do
    ...
    let check_model, m1 =
      ...
      | model :: tl ->
        ...
        mmms := tl;
        true, m
      | [] -> false, BddapronSmt.smt_compute_model_ctx env.Env.env env.Env.cond ctx1
    in
    ...
    | Some mm1 ->
      ...
      if not check_model then models := mm1 :: !models else tryimpsucc := !tryimpsucc + 1;
      ...
      Log.debug2_o
        logger
        (BddapronSmt.smt_print_model env.Env.env env.Env.cond)
        "smt-bp-model: "
        bpmodel;
      let bpexpr = BddapronSmt.model_to_boolexpr env.Env.env env.Env.cond bpmodel in
      (*        Log.debug3_o logger (BddapronUtil.print_boolexpr env.Env.env 
...
let stratcfg_split_loc env cfg locid inv1 inv2 =
  ...
  *)
  PSette.iter
    (fun pe ->
       ...
       then (
         PSHGraph.add_hedge
           cfg
           (stratcfg_next_arcid cfg)
           (PSHGraph.attrhedge cfg pe)
           parr
           [| loc1 |];
         PSHGraph.add_hedge
           cfg
  ...
  PSette.iter (fun e -> PSHGraph.remove_hedge cfg e) (PSHGraph.predhedge cfg locid);
  ...
  | locid :: _ ->
    ...
    let inv2 =
      Bddapron.Expr0.Bool.dand
        ...
        env.Env.cond
        inv
        (Bddapron.Expr0.Bool.dnot env.Env.env env.Env.cond b)
    in
    let inv3 =
      Bddapron.Expr0.Bool.dand
        env.Env.env
        env.Env.cond
        b
        (Bddapron.Expr0.Bool.dnot env.Env.env env.Env.cond inv)
    in
    Log.debug3_o logger (BddapronUtil.print_boolexpr env.Env.env env.Env.cond) "expr = " b;
    Log.debug3_o
      logger
      (BddapronUtil.print_boolexpr env.Env.env env.Env.cond)
      "inv = "
    ...
    if
      not
        (Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond inv1
         || Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond inv2)
    then stratcfg_split_loc env cfg locid inv1 inv2;
    if not (Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond inv3)
    then stratcfg_split_locs env cfg inv3
;;
...
let stratcfg_add_arc env cfg bsrc btgt arc =
  ...
  List.iter
    (fun srcid ->
       List.iter
         (fun tgtid ->
            if srcid <> tgtid || List.length srcids = 1
            then (
              let i, _, _ = arc in
              Log.debug3_o logger Format.pp_print_int "add arc for i = " i;
              ...
              Log.debug3
                ...
                ("add arc("
                 ...
                 ^ " --> "
                 ^ string_of_int tgtid);
              PSHGraph.add_hedge cfg arcid arc [| srcid |] [| tgtid |]))
         tgtids)
    srcids
;;
...
let stratcfg_print_dot env fmt cfg print_arcs =
  ...
  PSHGraph.print_dot
    ...
    (fun fmt arcid arc ->
       if print_arcs
       then print_arc env fmt arcid arc
...
let update_value_elt env value_table mtbdd boolexpr leaf =
  Cudd.Mtbdd.ite boolexpr (Cudd.Mtbdd.cst env.Env.cuddman value_table leaf) mtbdd
;;

let add_suffix s1 s2 v = v ^ "_" ^ string_of_int (-s1) ^ "_" ^ string_of_int s2

let add_suffix_primed s1 s2 v =
  v ^ "_" ^ string_of_int (-s1) ^ "_" ^ string_of_int s2 ^ "'"
;;

let add_suffix2 s1 s2 s3 v =
  v ^ "_" ^ string_of_int (-s1) ^ "_" ^ string_of_int (-s2) ^ "_" ^ string_of_int s3
;;

let add_suffix_primed2 s1 s2 s3 v =
  v ^ "_" ^ string_of_int (-s1) ^ "_" ^ string_of_int (-s2) ^ "_" ^ string_of_int s3 ^ "'"
;;
...
let strat_to_lp_conss env doman initial template assertion strat s =
    ...
    (fun i strat_elt ->
       ...
       Array.iter
         (fun (btgt, (bsrc, g, equs)) ->
            ...
            let bsrc = Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond srcinv bsrc in
            if
              (not (Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond bsrc))
              && not (Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond btgt)
            then (
              let gass = Bddapron.Expr0.Bool.tdrestrict assertion bsrc in
              let gass =
                BddapronUtil.boolexpr_to_linconss
                  env.Env.env
                  env.Env.cond
                  doman
                  env.Env.b_vars
  ...
  PSHGraph.iter_vertex stratcfg (fun locid inv ~pred ~succ:_ ->
    if PSette.cardinal pred = 0 && Apron.Lincons1.is_unsat (get_value_leaf env s.(0) inv)
    then
  ...
  let f, vars =
    PSHGraph.fold_hedge
      ...
      (fun _ (i, g, equs) ~pred ~succ (f, vars) ->
         ...
         let ftr =
           List.append
             (ApronLp.make_conss_conss (* transition *)
                ~rename:(add_suffix2 pred.(0) succ.(0) i)
                g)
             (List.map
                (ApronLp.make_conss_equ
         ...
         let svars =
           (*  bound variables *)
           let sv = Apron.Var.of_string (add_suffix pred.(0) i (Apron.Var.to_string v)) in
         ...
         , Sette.union
             (Sette.add
                (Apron.Var.of_string (add_suffix succ.(0) i (Apron.Var.to_string v)))
                svars)
             vars ))
      ([], Sette.empty)
  in
  let f =
    Util.array_fold_lefti (* old value constraints *)
      (fun i f s_elt ->
         Array.fold_left
           (fun f (b, cons) ->
              if ApronUtil.lincons_is_false cons || ApronUtil.lincons_is_true cons
              then f
              else (
                ...
                List.fold_left
                  (fun f locid ->
                     ...
                     ApronLp.make_conss_lequ
                       ...
                       (Apron.Var.of_string boundvar_prefix, expr)
                     :: f)
                  f
                  locids))
           f
...
let compute_strategy_value env doman value_table template initial assertion strat s =
  ...
  let lp_constrs, vars, stratcfg =
    strat_to_lp_conss env doman initial template assertion strat s
  in
...
*)
  ...
  PSHGraph.iter_vertex stratcfg (fun locid b ~pred:_ ~succ:_ ->
    Log.debug2_o logger (BddapronUtil.print_boolexpr env.Env.env env.Env.cond) "b = " b;
    List.iteri
      (fun i t_elt ->
         let v = Apron.Var.of_string (add_suffix locid i boundvar_prefix) in
         let x =
           if Util.array_mem v bounded_vars
           then sol v
               ...
               (Array.map
                  (fun (b, n) ->
                     ...
                     , Apron.Abstract1.abstract0
                         (Apron.Abstract1.of_lincons_array apronman env.Env.apronenv narr)
                     ))
                  (Cudd.Mtbdd.guardleafs s_elt)))))
...
let analyze_pow param template env doman cfprog =
  ...
  let strat0, s0 =
    initial_strategy_value
      env
      template
      value_table
      strat_table
      strats
      cfprog.Program.c_init
  in
...
module PowLognum (Dom : Template.TEMPLATE_T) = struct
  ...
  let print_result env final anres fmt () =
    let print_res_loc locid s res =
      ...
      if res
      then Format.pp_print_string strfmt ": reach = "
  ...
  let result_to_bddapron env anres () =
    Util.list2mappe
      (List.map (fun (locid, s) -> locid, Dom.to_boollinconsslist env s) anres)
  ;;
  ...
  let is_safe env final anres =
    let rec check = function
      ...
      | (_, s) :: reslist ->
        ...
        if res then check reslist else false
    in
    check anres
  ;;
