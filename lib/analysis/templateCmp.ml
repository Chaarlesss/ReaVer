...
let template_comparison_stat
  ...
  let locmap = Cfg.get_locmap env cfg1 cfg2 in
  (* l1 -> [l2...] *)
  ...
  let get_bool_space_size (anres : Analysis.bddapron_res_t) =
    let reach =
      Mappe.fold
        (fun _ s res ->
           if Util.list_is_empty s
           then res
           else List.fold_left (fun res (b, _) -> Cudd.Bdd.dor res b) res s)
        anres
        (Cudd.Bdd.dfalse env.Env.cuddman)
    in
  ...
  in
  let get_max n t =
    if Apron.Abstract1.is_bottom apronman n
    then Apron.Scalar.of_infty (-1)
    else if Apron.Abstract1.is_top apronman n
    then Apron.Scalar.of_infty 1
    else (
    ...
    List.fold_right
      (fun t stats ->
         ...
         let incdev max1 max2 =
           if Apron.Scalar.is_infty max1 = 0 && Apron.Scalar.is_infty max2 = 0
           then (
             let m1 = scalar_to_float max1 in
             let m2 = scalar_to_float max2 in
             if Apron.Scalar.equal_int max2 0
             then abs_float (m1 -. m2)
        ...
        Format.pp_print_int fmt cmp) "" (max1,max2,cmp);*)
         ...
         else { stats with eq = stats.eq + 1; eqinf = stats.eqinf + incinf max1 })
      templates
      stats
  in
  let convexify_num s =
  ...
  let stats =
    Mappe.fold
      (fun v s1 stats ->
         ...
         let templates =
           List.append global_template (get_local_template local_templates v)
         in
...
let print_template_comparison_stat fmt stats =
  ...
  Format.pp_print_string fmt ("reachable bool space size 1 = " ^ string_of_int stats.b1);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("reachable bool space size 2 = " ^ string_of_int stats.b2);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("better bounds = " ^ string_of_int stats.lt);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("equal  bounds = " ^ string_of_int stats.eq);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("infinite equal bounds = " ^ string_of_int stats.eqinf);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("worse  bounds = " ^ string_of_int stats.gt);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("infinite worse bounds 1 = " ^ string_of_int stats.gtinf1);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("infinite worse bounds 2 = " ^ string_of_int stats.gtinf2);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("deviation of worse bounds = " ^ string_of_float stats.gtdev);
  Format.pp_print_newline fmt ();
  ...
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("         total bounds = " ^ string_of_int total);
  Format.pp_print_newline fmt ();
  Format.pp_print_string fmt ("total finite bounds 2 = " ^ string_of_int totfinite);
  Format.pp_print_newline fmt ();
  Format.pp_print_string
    fmt
    ("        better bounds = "
     ^ string_of_int stats.lt
     ^ " ("
  ...
  Format.pp_print_string
    ...
    ("         worse bounds = "
     ...
     ^ ")"
     ^ " dev = "
     ^ string_of_float (stats.gtdev /. float_of_int worse));
  Format.pp_print_newline fmt ();
  Format.pp_print_string
    fmt
    ("          lost bounds = "
