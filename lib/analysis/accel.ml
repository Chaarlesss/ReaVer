...
module Acc (Dom : Domain.T) = struct
  ...
  let make_fp_manager env cfg initial trfct =
    ...
    ; Fixpoint.is_leq = (fun vertex s1 s2 -> Dom.is_leq env s1 s2)
  ...
  let make_strategy_acc ws wd aws cfg sinit =
    let widening_start cfg l =
      ...
      let is_accel h =
        ...
        | _ -> false
      in
      let exists_accel hset = PSette.fold (fun h res -> res || is_accel h) hset false in
    ...
    in *)
      let rec all_accel l =
        ...
        | Ilist.Atome v :: tl ->
          if
            exists_accel (PSHGraph.predhedge cfg v)
            && exists_accel (PSHGraph.succhedge cfg v)
          then all_accel tl
          else false
    ...
    in
    let flatten ~parent (b, _) =
      b.FixpointType.widening_start <= 0 || parent.FixpointType.widening_start > 0
    in
    let filter arcid =
  ...
  let arc_normal env dir assertion ass f s =
    ...
    let s =
      ...
      | `Backward -> Dom.meet_condition env (Dom.substitute_lexpr env s f) g
    in
  ...
  let trfct env dir assertion cfg arcid tabs =
    ...
    let s =
      match PSHGraph.attrhedge cfg arcid with
      | Arc.Normal (ass, f) ->
        Log.debug logger ("processing normal arc (" ^ string_of_int arcid ^ ")...");
        arc_normal env dir assertion ass f s
      | Arc.Accel (ass, f) ->
        Log.debug logger ("processing accelerable arc (" ^ string_of_int arcid ^ ")...");
        let g =
      ...
      | Arc.Bool (ass, f) ->
        Log.debug logger ("processing Boolean arc (" ^ string_of_int arcid ^ ")...");
        arc_normal
          env
          dir
          assertion
          ...
          (List.append
             f
             (BddapronUtil.get_id_equs_for env.Env.env env.Env.cond env.Env.ns_vars))
          s
      | Arc.BoolNacc (ass, f) ->
        Log.debug
          logger
      ...
      | Arc.Nonacc (ass, f) ->
        ...
        arc_normal env dir assertion ass f s
      | Arc.Id ->
        Log.debug logger ("processing identity arc (" ^ string_of_int arcid ^ ")...");
        s
      | _ -> assert false
    in
  ...
  let result_to_bddapron env anres () = Mappe.map (Dom.to_boollinconsslist env) anres
  ...
  let analyze param env cf =
    ...
    let cfg2 =
      ...
      | `Forward -> cfg
      | `Backward ->
        PSHGraph.transpose (fun v inv -> inv) (fun a arc -> arc) (fun info -> info) cfg
    in
    let get_init_state =
      match param.a_dir with
    ...
    in
    let fpman =
      make_fp_manager env cfg2 get_init_state (trfct env param.a_dir assertion cfg2)
    in
    let sinit =
      match param.a_dir with
    ...
    in
    let strategy = make_strategy_acc param.a_ws param.a_wd param.a_aws cfg2 sinit in
