...
let extract_num_equs env cfg initial assertion =
  ...
  let extract_num_equs_arc make_arc check_actions (arcid, ass, f, sloc, dloc) =
    ...
    let dinvfb = Bddapron.Expr0.Bool.substitute env.Env.env env.Env.cond dinv fb in
    ...
    if not (Cudd.Bdd.is_false g)
    then (
      let fn =
        BddapronUtil.simplify_equs env.Env.env env.Env.cond (Env.get_num_equs env f) g
      in
      ...
      let fnset = MtbddUtil.arraymtbdd_to_arrsetmtbdd compare tabarrset numarr in
      Log.debug3_o
        logger
        (MtbddUtil.print_arrset_mtbdd
           env.Env.env
           env.Env.cond
           (MtbddUtil.print_action env.Env.env))
        "fnset: "
        fnset;
      let fnite =
        Cudd.Mtbdd.ite
          g
          fnset
          (MtbddUtil.arrsetmtbdd_empty ~compare_array:compare tabarrset env.Env.cuddman)
      in
      Log.debug3_o
        logger
        (MtbddUtil.print_arrset_mtbdd
      ...
      let aflist =
        Array.fold_left
          (fun arclist (gg, arrset) ->
             if PSette.is_empty arrset
             then arclist
             else (
               let ass2 =
                 Cudd.Bdd.tdrestrict (Cudd.Bdd.dand g gg) env.Env.cond.Bdd.Cond.careset
               in
               ...
               List.fold_left
                 (fun arclist ass2 ->
                    PSette.fold
                      (fun arr arclist ->
                         if sloc <> dloc || check_actions vars (Array.to_list arr)
                         then (
                           let f2 =
                             List.map2
                               (fun v a ->
                                  ( v
                                  , `Apron
                                      (BddapronUtil.apronexpr_to_apronexprDD
                                         env.Env.env
                                         a) ))
                               vars
                               (Array.to_list arr)
      ...
      List.iter
        (fun arc ->
           let _ = Cfg.add_arc env cfg sloc dloc arc in
           ())
        aflist);
  ...
  (* extract num_tr_fcts *)
  PSette.iter
    (extract_num_equs_arc (fun (a, f) -> Arc.Normal (a, f)) not_is_identity_actions)
    (Cfg.get_arcs2 cfg (fun arc ->
       match arc with
       | Arc.Normal (a, f) -> Some (a, f)
       | _ -> None));
  PSette.iter
    (extract_num_equs_arc (fun (a, f) -> Arc.Flow (a, f)) not_is_zero_actions)
    (Cfg.get_arcs2 cfg (fun arc ->
       match arc with
       | Arc.Flow (a, f) -> Some (a, f)
       | _ -> None));
  ...
  let unreach, _ =
    PSHGraph.reachable_multi
      ...
      Cfg.arcid_dummy
      cfg
      (Cfg.get_locidset_by_inv env cfg initial)
  in
...
let print_locmap fmt locmap =
  List.iter
    (fun (loc, locs) ->
       ...
       Util.list_print Format.pp_print_int fmt locs)
    locmap
;;
...
let boolrefine env doman cfg initial assertion anres =
  ...
  let locmap =
    List.fold_left
      (fun locmap (v, s) ->
         if Bddapron.Domain0.is_bottom doman s
         then (
           ...
           let cfg, locs =
             List.fold_left
               (fun (cfg, locs) (b, _) ->
                  let newinv = Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond inv b in
                  let cfg, newv = Cfg.copy_location_with env cfg v newinv assertion in
                  cfg, newv :: locs)
               (cfg, [])
               slist
           in
...
let rec find_in_locmap locmap v =
  ...
  | (mv, locs) :: locmaptl ->
    if List.mem v locs
    then mv, List.filter (fun x -> x <> v) locs
  ...
  PSHGraph.fold_vertex
    ...
    (fun v inv ~pred ~succ (cfg, locmap) ->
       ...
       then cfg, locmap
         ...
         let cfg, newlocs =
           PSette.fold
             (fun (_, ass, f) (cfg, locs) ->
                let cfg, newv = Cfg.copy_location_with env cfg v inv assertion in
...
let reduce_to_numcfg anres env doman cfg initial assertion =
  let numcfg =
    PSHGraph.copy (fun v inv -> inv) (fun a arc -> arc) (fun info -> info) cfg
  in
...
let scalar_to_numinf c =
  if Apron.Scalar.is_infty c > 0
  then Maxstratutil.Num.InfGmpRatio.Infty
  else if Apron.Scalar.is_infty c < 0
  then Maxstratutil.Num.InfGmpRatio.Neginfty
  else if Apron.Scalar.equal_int c 1
  then Maxstratutil.Num.InfGmpRatio.one
  else if Apron.Scalar.equal_int c (-1)
  then Maxstratutil.Num.InfGmpRatio.minus_one
  else
    Maxstratutil.Num.InfGmpRatio.Num
      (Maxstratutil.Num.GmpRatio.from_string (Apron.Scalar.to_string c))
;;

let scalar_to_num c =
  if Apron.Scalar.equal_int c 1
  then Maxstratutil.Num.GmpRatio.one
  else if Apron.Scalar.equal_int c (-1)
  then Maxstratutil.Num.GmpRatio.minus_one
  else Maxstratutil.Num.GmpRatio.from_string (Apron.Scalar.to_string c)
;;

let coeff_to_num c =
  (*  Log.debug3_o logger (Apron.Coeff.print) "coeff_to_num: " c; *)
  if Apron.Coeff.is_zero c
  then Maxstratutil.Num.GmpRatio.zero
  else (
    match c with
...
(* converts APRON coefficients into STRATIMP coefficients *)
let num_to_coeff c =
  Apron.Coeff.s_of_mpqf (Mpqf.of_string (Maxstratutil.Num.GmpRatio.to_string c))
;;
...
let linexpr1_to_expr env linexpr =
  ...
  let constterm =
    if Apron.Coeff.is_zero const then [] else [ MyHa.expr_const (coeff_to_num const) ]
  in
  let terms =
    List.fold_left
      (fun terms var ->
         ...
         if Apron.Coeff.is_zero c
         then terms
         else if Apron.Coeff.equal_int c 1
         then MyHa.expr_var var :: terms
         else
           MyHa.expr_mul [ MyHa.expr_const (coeff_to_num c); MyHa.expr_var var ] :: terms)
      constterm
      vars
  in
  let res =
    if Util.list_is_empty terms then MyHa.expr_const MyHa.zero else MyHa.expr_add terms
  in
...
let linexpr1_to_neg_expr env linexpr =
  ...
  let const = Apron.Linexpr1.get_cst linexpr in
  let constterm =
    if Apron.Coeff.is_zero const
    then []
    else [ MyHa.expr_const (coeff_to_num (Apron.Coeff.neg const)) ]
  in
  let terms =
    List.fold_left
      (fun terms var ->
         ...
         then MyHa.expr_var var :: terms
           ...
           MyHa.expr_mul
             [ MyHa.expr_const (coeff_to_num (Apron.Coeff.neg c)); MyHa.expr_var var ]
           :: terms)
      constterm
      vars
  in
...
let lincons1_to_cons env lincons =
  ...
  | Apron.Lincons0.EQ -> MyHa.atom_eq expr (MyHa.expr_const MyHa.zero)
  | Apron.Lincons0.SUPEQ -> MyHa.atom_geq expr (MyHa.expr_const MyHa.zero)
...
let linconss_to_pform env linconss =
  MyHa.pform_and
    (Array.to_list
       (Array.mapi
          (fun i _ ->
             MyHa.pform_atom (lincons1_to_cons env (Apron.Lincons1.array_get linconss i)))
...
let equs_to_assignment env f =
  ...
  List.map
    (fun (v, e) -> Apron.Var.to_string v, linexpr1_to_expr env e)
    (Array.to_list numf)
;;
...
let linexpr1_to_diffinequexpr env linexpr : MyHa.DiffIneq.expr =
  ...
  let constterm =
    if Apron.Coeff.is_zero const
    then []
    else [ MyHa.DiffIneq.expr_const (coeff_to_num const) ]
  in
  let terms =
    List.fold_left
      (fun terms var ->
         ...
         then MyHa.DiffIneq.expr_var (MyHa.DiffIneqVar.Var var) :: terms
         else
           MyHa.DiffIneq.expr_mul
             [ MyHa.DiffIneq.expr_const (coeff_to_num c)
             ; MyHa.DiffIneq.expr_var (MyHa.DiffIneqVar.Var var)
             ]
           :: terms)
      constterm
      vars
  in
  let res =
    if Util.list_is_empty terms
    then MyHa.DiffIneq.expr_const MyHa.DiffIneq.zero
    else MyHa.DiffIneq.expr_add terms
  in
  (*  Log.debug3_o logger (Format.pp_print_string) "linexpr1_to_expr: expr=" 
    (MyHa.string_of_expr res); *)
  res
...
let lincons1_to_atom env lincons =
  let expr = linexpr1_to_diffinequexpr env (Apron.Lincons1.get_linexpr1 lincons) in
  match Apron.Lincons1.get_typ lincons with
  | Apron.Lincons0.EQ -> MyHa.DiffIneq.atom_eq expr (MyHa.DiffIneq.expr_const MyHa.zero)
...
let boolexpr_to_atomlist env doman boolexpr : MyHa.DiffIneq.atom list =
  ...
  Array.to_list
    (Array.mapi
       (fun i _ -> lincons1_to_atom env (Apron.Lincons1.array_get linconss i))
       linconss.Apron.Lincons1.lincons0_array)
;;
  ...
  List.map
    (fun (v, e) ->
       MyHa.DiffIneq.atom_eq
         (MyHa.DiffIneq.expr_var (MyHa.DiffIneqVar.DiffVar (Apron.Var.to_string v)))
  ...
  let ha1 =
    PSHGraph.fold_vertex
      ...
      (fun locid _ ~pred ~succ ha ->
         ...
         if PSette.is_empty flowarcs
         then MyHa.add_ev (string_of_int locid, [ MyHa.DiffIneq.atom_false ]) ha
         else
           PSette.fold
             (fun (_, ass, f) ha ->
                let inv = Loc.get_inv (Cfg.get_loc cfg locid) in
                let staycond = boolexpr_to_atomlist env doman (Cudd.Bdd.dand ass inv) in
    ...
    PSHGraph.fold_hedge
      ...
      (fun h arc ~pred ~succ ha ->
         ...
         | _ ->
           ...
           let trans =
             ( boolexpr_to_pform env doman ass2
             , equs_to_assignment
                 env
                 (BddapronUtil.simplify_equs env.Env.env env.Env.cond f ass) )
           in
           MyHa.add_trans (string_of_int pred.(0), trans, string_of_int succ.(0)) ha)
      ha1
  in
...
let get_local_template local_templates loc =
  try List.assoc loc local_templates with
  | Not_found -> []
;;
    ...
    (fun loc ->
       let locarr =
         List.map
           (fun cons ->
              let value =
                Maxstrat.Stratimp.get
                  var_ass
                  (Maxstrat.Stratimp.V, string_of_int loc, linexpr1_to_neg_expr env cons)
              in
              match value with
       ...
       Log.debug2_o
         logger
         (Util.list_print Apron.Lincons1.print)
         ("result(loc=" ^ string_of_int loc ^ ") = ")
         locarr;
       let locearr =
         { Apron.Lincons1.lincons0_array =
             Array.of_list (List.map (fun c -> Apron.Lincons1.get_lincons0 c) locarr)
         ; array_env = env.Env.apronenv
         }
       in
       (* loc -> bool x num *)
       loc, Apron.Abstract1.of_lincons_array apronman env.Env.apronenv locearr)
...
let anres_to_varass env doman global_template local_templates anres =
  ...
  let make_varass f sloc s template varass =
    List.fold_left
      (fun varass t ->
         ...
         (*        Log.debug3 logger 
          ("max = "^(Maxstratutil.Num.InfGmpRatio.to_string max)); *)
         List.fold_left (fun varass v -> ((v, sloc, tex), max) :: varass) varass vars)
      varass
      template
  in
  let loc_to_varass loc s =
    ...
    let template = List.append global_template (get_local_template local_templates loc) in
    if Apron.Abstract1.is_top apronman s
    then make_varass (fun s t -> Maxstratutil.Num.InfGmpRatio.Infty) sloc s template []
    else if Apron.Abstract1.is_bottom apronman s
    then make_varass (fun s t -> Maxstratutil.Num.InfGmpRatio.Neginfty) sloc s template []
           ...
           then (
             ...
             | l :: ltl, (sb, _) :: _ ->
               let ns =
                 ...
                 with
                 | Not_found -> Bddapron.Domain0.bottom doman env.Env.env
               in
               iter slist ltl (Bddapron.Domain0.join doman ns news) (i + 1)
             | _ -> assert false)
         in
         let news = iter slist locs (Bddapron.Domain0.bottom doman env.Env.env) 0 in
         Log.debug3_o
           logger
...
let numanres_to_anres2 env doman cfg numanres =
  List.map
    (fun loc ->
       ...
       let s =
         try
           Bddapron.Domain0.meet_condition
             doman
             env.Env.env
             env.Env.cond
             (Bddapron.Domain0.of_apron
                doman
                env.Env.env
                (Apron.Abstract1.abstract0 (List.assoc loc numanres)))
             inv
         with
         | Not_found -> Bddapron.Domain0.bottom doman env.Env.env
       in
         ...
         let rec iter numres slist locs i =
           ...
           then (
             ...
             | l :: ltl, (sb, sn) :: _ ->
               ...
               let abs =
                 { Apron.Abstract1.abstract0 = sn
                 ; Apron.Abstract1.env = env.Env.apronenv
                 }
               in
...
let max_solve env doman cfg initloc global_template local_templates anres =
  ...
  Log.debug2_o
    logger
    (Util.list_print Format.pp_print_string)
    "global templates: "
    (List.map MyHa.string_of_expr globaltmp);
  let localtmps loc =
    template_convert env (get_local_template local_templates (int_of_string loc))
  in
  let var_ass0 = anres_to_varass env doman global_template local_templates anres in
  (*  let var_ass0 = [] in *)
  let var_ass =
    Maxstrat.Stratimp.solve_with_initial_var_ass ha globaltmp localtmps var_ass0
  in
...
let analyze_num template oldanres locmap env doman cfg initial =
  ...
  if PSette.cardinal initlocidset > 1
  then raise (NotSupported "multiple initial locations");
  ...
  if Util.list_is_empty oldanres
  then numanres_to_anres2 env doman cfg newnumanres
...
module Num (Dom : Template.TEMPLATE_T) = struct
...
module Lognum (Dom : Template.TEMPLATE_T) = struct
  type analysisparam_t = unit
  ...
  let make_fp_manager env initial trfct =
    ...
    ; Fixpoint.canonical = (fun vertex s -> Dom.canonicalize env s)
    ; Fixpoint.is_bottom = (fun vertex s -> Dom.is_bottom env s)
    ; Fixpoint.is_leq = (fun vertex s1 s2 -> bool_is_leq env s1 s2)
    ; Fixpoint.join = (fun vertex s1 s2 -> Dom.join env s1 s2)
    ; Fixpoint.join_list =
        (fun vertex slist ->
          List.fold_left (fun res x -> Dom.join env res x) (Dom.bottom env) slist)
  ...
  let fpout_to_anres env cfg output =
    ...
    PSette.fold (fun v res -> (v, PSHGraph.attrvertex output v) :: res) locs []
  ;;

  (* fixpoint iteration strategy for standard analysis *)
  let make_strategy_std cfg sinit =
    Fixpoint.make_strategy_default
      ~vertex_dummy:Cfg.locid_dummy
      ~hedge_dummy:Cfg.arcid_dummy
      cfg
      sinit
  ;;
    ...
    let rec anres_is_eq_loc a1 a2 =
      ...
      | (v, s1) :: ra1 ->
        (try
         ...
         with
         | Not_found -> Dom.is_bottom env s1)
    in
  ...
  let analyze (param : analysisparam_t) env cf =
    ...
    let anres_prev =
      ref
        (List.map
           (fun loc -> loc, Dom.bottom env)
           (*    (fun loc -> (loc,get_initial_state env initial cfg loc)) *)
           (PSette.elements (Cfg.get_locidset cfg)))
    in
    let anres =
      ref
        (analyze_propagate_bounded env cfg assertion (get_initial_state env initial cfg))
    in
    Log.debug2_o
      logger
      (fun fmt s -> print_result env final s fmt ())
      "anres_prev = "
      !anres_prev;
    Log.debug2_o logger (fun fmt s -> print_result env final s fmt ()) "anres = " !anres;
    let cnt = ref 0 in
    while not (anres_is_eq env !anres !anres_prev) do
      let numcfg, locmap = reduce_to_numcfg !anres env doman cfg initial assertion in
      ...
      let get_init_state locid =
        try List.assoc locid !anres with
        | Not_found -> Dom.bottom env
      in
      anres_prev := !anres;
