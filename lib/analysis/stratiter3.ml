...
type open_strats_t =
  (Cfg.locid_t * int, Cfg.arcid_t PSette.t * Cfg.arcid_t PSette.t) Mappe.t
...
let print_value_elt fmt arr = Util.array_print Template.print_bound fmt arr
...
let print_open_strats fmt (open_strats : open_strats_t) =
  Mappe.print
    (fun fmt (l, i) ->
       ...
       Format.pp_print_string fmt "]")
    (fun fmt (x, y) ->
       PSette.print Format.pp_print_int fmt x;
       PSette.print Format.pp_print_int fmt y)
    fmt
    open_strats
;;
...
let open_strats_add_new_loc cfg (open_strats : open_strats_t) slocs locid i =
  let old, used =
    try Mappe.find (locid, i) open_strats with
    | Not_found -> PSette.empty compare, PSette.empty compare
  in
  ...
  in
  let open_strats =
    Mappe.add (locid, i) (PSette.diff (PSette.union old newpreds) used, used) open_strats
  in
  PSette.fold
    (fun succ (open_strats : open_strats_t) ->
       let old, used =
         try Mappe.find (succ, i) open_strats with
         | Not_found -> PSette.empty compare, PSette.empty compare
       in
       ...
       Mappe.add
         (succ, i)
         (PSette.diff (PSette.union newpreds old) used, used)
         open_strats)
    (PSHGraph.succ_vertex cfg locid)
    open_strats
;;
...
let initial_strategy_value env template cfg initial =
  ...
  let s =
    Mappe.add
      initlocid
      (Array.map
         (fun _ -> Apron.Coeff.Scalar (Apron.Scalar.of_infty (-1)))
         (Array.of_list template))
      Mappe.empty
  in
  let open_strats =
    Util.array_fold_lefti
      (fun i open_strats _ ->
         open_strats_add_new_loc cfg open_strats (Sette.singleton initlocid) initlocid i)
      Mappe.empty
      (Array.of_list template)
  in
  strat, s, open_strats
...
let get_gequs env doman cfg assertion arcid =
  ...
  let g =
    BddapronUtil.boolexpr_to_linconss env.Env.env env.Env.cond doman env.Env.b_vars ass
  in
  g, nequs
;;
...
let strategy_improve_loci
  ...
  while (not !updated) && not (PSette.is_empty !open_arcs) do
    ...
    let smtvg0 =
      smt_build_gt_value_elt
        env
        (List.nth template i)
        (try (Mappe.find locid s).(i) with
         | Not_found -> Apron.Coeff.Scalar (Apron.Scalar.of_infty 1))
    in
    ...
    | Some _ ->
      ...
      open_strats := Mappe.add (locid, i) (opena, PSette.add arcid used) !open_strats;
      open_strats
      := open_strats_add_new_loc
           cfg
           !open_strats
           (Sette.add locid (Mappe.maptoset s))
           locid
           i;
      let sarr =
        try Mappe.find locid !strat with
        | Not_found -> Array.make (List.length template) 0
      in
      sarr.(i) <- arcid;
...
let strategy_improve_loci2
      ...
      (open_arcs : Cfg.arcid_t PSette.t)
  =
  if not (PSette.is_empty open_arcs)
  then (
    TimeMeas.start tm_smt_conv;
    let f =
      PSette.fold
    ...
    | Some m ->
      let arcid =
        match Hashtbl.find m arcid_var with
      ...
      Log.debug
        ...
        ("improved: ["
         ...
         ^ ")");
      let opena, used = Mappe.find (locid, i) open_strats in
      let open_strats = Mappe.add (locid, i) (opena, PSette.add arcid used) open_strats in
      let open_strats =
        open_strats_add_new_loc
          cfg
          open_strats
...
let strategy_improve
  ...
  let updated, strat, open_strats =
    List.fold_left
      ...
      (false, strat, open_strats)
      (Mappe.bindings open_strats)
  in
...
let add_suffix_primed3 s1 s2 s3 s4 v =
  v
  ^ "_"
  ^ string_of_int (-s1)
  ^ "_"
         ...
         Util.array_fold_lefti
           (fun i (f, vars) arcid ->
              ...
              let ftr =
                List.append
                  (ApronLp.make_conss_conss (* transition *)
                     ~rename:(add_suffix3 pred.(0) tgtid arcid i)
                     g)
                  (List.map
              ...
              let svars =
                (*  bound variables *)
                let sv =
                  Apron.Var.of_string (add_suffix pred.(0) i (Apron.Var.to_string v))
                in
              ...
              , Sette.union
                  (Sette.add
                     (Apron.Var.of_string (add_suffix tgtid i (Apron.Var.to_string v)))
                     svars)
                  vars ))
           (f, vars)
           arcids)
      ([], Sette.empty)
      (Mappe.bindings strat)
  in
  let f =
    List.fold_left
      (fun f (locid, sarr) ->
         (* old value constraints *)
         Util.array_fold_lefti
           (fun i f bound ->
              match bound with
              | Apron.Coeff.Scalar bb ->
                if Apron.Scalar.is_infty bb = 0
                then (
...
let compute_strategy_value
      env
      doman
      (cfg : Cfg.t)
      (template : Template.template_t)
      initial
      assertion
      strat
      s
  ...
  let lp_constrs, vars = strat_to_lpconss env doman cfg template assertion strat s in
...
let values_to_abstract env doman cfg template values =
  ...
  List.map
    (fun locid ->
         ...
         , Bddapron.Domain0.of_bddapron
             ...
             [ ( inv
               , Apron.Abstract1.abstract0
                   (Apron.Abstract1.of_lincons_array
                      apronman
                      env.Env.apronenv
       ...
       with
       | Not_found -> locid, Bddapron.Domain0.bottom doman env.Env.env)
...
let analyze_num ~smt_disjuncts template env doman cfprog =
  let strat0, s0, os0 =
    initial_strategy_value env template cfprog.Program.c_cfg cfprog.Program.c_init
  in
  let strategy_improve_loci =
    if smt_disjuncts then strategy_improve_loci2 else strategy_improve_loci
  in
