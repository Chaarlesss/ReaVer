...
module Std (Dom : Domain.T) = struct
  ...
  let make_fp_manager env cfg initial trfct =
    ...
    ; Fixpoint.widening =
        (fun vertex s1 s2 ->
          ...
          Dom.forget_list
            env
            (Dom.meet_condition env (Dom.widening env s1 s2) dinv)
            env.Env.i_vars)
  ...
  let analyze param env cf =
    ...
    in
    let fpman =
      make_fp_manager env cfg2 get_init_state (trfct env param.s_dir assertion cfg2)
    in
    let sinit =
      match param.s_dir with
...
module Bool = struct
  ...
  let refine_loc env (id, s) ~refine_bool locid loc =
    if not (Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond s)
    then
      if id = locid
      then Some (Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond s loc)
      else None
    else None
  ;;
  ...
  let analyze param env cf =
    ...
    let s =
      BddapronAnalysis.bool_reach2
        ...
        env.Env.cond
        fb
        (List.append env.Env.n_vars env.Env.bi_vars)
        s_start
        cf.Program.c_ass
    in
...
module Hyb (Dom : Domain.T) = struct
  ...
  let trfct env assertion cfg hedge tabs =
    ...
    let s = tabs.(0) in
    let s =
      match PSHGraph.attrhedge cfg hedge with
      | Arc.Normal (ass, f) ->
        Log.debug logger ("processing jump arc (" ^ string_of_int hedge ^ ")...");
      ...
      | Arc.Flow (stay, f) ->
        Log.debug logger ("processing flow arc (" ^ string_of_int hedge ^ ")...");
        let stay =
          Bddapron.Expr0.Bool.dand
            env.Env.env
            env.Env.cond
  ...
  let is_safe env final anres =
    let rec check = function
      | [] -> true
      | (_, s) :: reslist ->
        if Dom.is_bottom env (Dom.meet_condition env s final)
        then check reslist
        else false
    in
    check (Util.mappe2list anres)
  ...
  let analyze param env cf =
    ...
    let fpman = make_fp_manager env cfg get_init_state (trfct env assertion cfg) in
