...
let mappe2keylist (m : ('a, 'b) Mappe.t) : 'a list = Mappe.fold (fun a b c -> a :: c) m []
...
let mappe2list (m : ('a, 'b) Mappe.t) = Mappe.fold (fun a b c -> (a, b) :: c) m []
...
let array2sette (bb : 'b array) =
  Array.fold_left (fun (aa : 'b Sette.t) (b : 'b) -> Sette.add b aa) Sette.empty bb
;;
...
let list_filter2 filter a b =
  List.fold_left2
    (fun (aa, bb) a b -> if filter a b then a :: aa, b :: bb else aa, bb)
    ([], [])
    a
    b
;;
...
let array_union a b =
  Array.of_list (Sette.elements (Sette.union (array2sette a) (array2sette b)))
;;

(* concats two arrays, keeping only the values present in both (removes duplicates) *)
let array_inter a b =
  Array.of_list (Sette.elements (Sette.inter (array2sette a) (array2sette b)))
;;

(* converts a list to a set *)
let list2sette (bb : 'b list) =
  List.fold_left (fun (aa : 'b Sette.t) (b : 'b) -> Sette.add b aa) Sette.empty bb
;;
...
let psette2list (aa : 'a PSette.t) =
  PSette.fold (fun (a : 'a) (bb : 'a list) -> a :: bb) aa []
;;
...
let sette2eqclasses (s : 'a Sette.t) (eqrel : 'a -> 'a -> bool) =
  Sette.fold
    (fun e cls ->
       ...
       if Sette.is_empty samecls
       then Sette.add (Sette.add e Sette.empty) cls
       else (
...
let psette2eqclasses
  ...
  PSette.fold
    (fun e cls ->
       ...
       if PSette.is_empty samecls
       then PSette.add (PSette.add e (PSette.empty compare)) cls
       else (
         let cl = PSette.choose samecls in
         PSette.add (PSette.add e cl) (PSette.remove cl cls)))
    s
    (PSette.empty PSette.compare)
;;
...
let list_inter l1 l2 = List.filter (fun e -> List.mem e l2) l1
...
let list_equal equal l1 l2 =
  let rec comp l1 l2 =
    match l1, l2 with
    | hd1 :: tl1, hd2 :: tl2 -> if equal hd1 hd2 then comp tl1 tl2 else false
    | [], [] -> true
    | _ -> assert false
  in
  if List.length l1 <> List.length l2 then false else comp l1 l2
;;

(* computes the product of two lists *)
let list_product combine l1 l2 =
  List.fold_right
    (fun e1 res -> List.append res (List.map (fun e2 -> combine e1 e2) l2))
    l1
    []
;;
...
let array_mem e l =
  let len = Array.length l in
  let rec check i =
    if i >= len then false else if l.(i) = e then true else check (i + 1)
  in
  check 0
;;
...
let array_diff l1 l2 =
  ...
  let rec build i l =
    ...
    then l
      ...
      let e = l1.(i) in
      if not (array_mem e l2) then build (i - 1) (e :: l) else build (i - 1) l)
  in
...
let array_exists p l =
  let len = Array.length l in
  let rec check i = if i >= len then false else if p l.(i) then true else check (i + 1) in
  check 0
;;
...
let array_exists2 p l1 l2 =
  ...
  assert (Array.length l2 = len);
  let rec check i =
    if i >= len then false else if p l1.(i) l2.(i) then true else check (i + 1)
  in
  check 0
;;
...
let array_print ?(csep = ";") ?(copen = "[") ?(cclose = "]") pp fmt l =
  ...
  Array.iter
    (fun e ->
       if not !start then Format.pp_print_string fmt csep else start := false;
       pp fmt e)
    l;
...
let print_fixed fmt len s =
  ...
  if slen > len
  then Format.pp_print_string fmt (String.sub s 0 len)
  else (
    Format.pp_print_string fmt s;
    for i = slen to len - 1 do
      Format.pp_print_string fmt " "
    done)
;;

let hashtbl_to_list x = Hashtbl.fold (fun a b res -> (a, b) :: res) x []
