...
let logger =
  { Log.fmt = Format.std_formatter; Log.module_name = "RelAbstr"; Log.level = Log.Debug3 }
;;
...
(* Helpers *)
(******************************************************************************)
let rec check_coeffs env e vdim dims ivars j (has_v_only, is_reset, has_inputs) =
  if j >= dims
  then has_v_only, is_reset, has_inputs
  else (
    ...
    | false, false, false ->
      check_coeffs env e vdim dims ivars (j + 1) (false, false, has_inputs)
    | true, false, false ->
      check_coeffs env e vdim dims ivars (j + 1) (has_v_only, false, has_inputs)
    | _, false, true ->
      check_coeffs env e vdim dims ivars (j + 1) (has_v_only, is_reset, true)
    | _ -> check_coeffs env e vdim dims ivars (j + 1) (has_v_only, is_reset, has_inputs))
;;
...
let get_transition_type_equ (v, e1) ivars =
  ...
  match check_coeffs env e vdim dims ivars 0 (true, true, false), has_nonzero_cst with
...
let transform_equ env (v, g, a) =
  ...
  match
    get_transition_type_equ
      (BddapronUtil.var_to_apronvar env v, a)
      (BddapronUtil.vars_to_apronvars env theenv.Env.ni_vars)
  with
  | Zero -> v, Bddapron.Expr0.Bool.dtrue env cond, Bddapron.Expr0.var env cond v
  | RealEigen1 (c, _) ->
    let newinvar = Env.get_newinput theenv v in
    let gg =
      (* c>0: *)
      if Apron.Coeff.cmp c (Apron.Coeff.s_of_int 0) >= 0
      then (* cx+Txi+d>0 /\ xx>=x /\ x'=xx *)
        (
        let gpos =
          Bddapron.Expr0.Bool.dand
            env
            cond
            (BddapronUtil.lincons_to_boolexpr
               env
               cond
    ...
    | Some (arcid, ass, f) ->
             ...
             | `Apron e ->
               ...
               , Array.fold_left
                   (fun ee (g, a) ->
                      ...
                      let a =
                        BddapronUtil.apronaction_to_linexpr env.Env.env env.Env.cond a
                      in
      ...
      let ass =
        Bddapron.Expr0.Bool.dand
          ...
          env.Env.cond
          ass
          (Bddapron.Expr0.Bool.substitute_by_var
             env.Env.env
             env.Env.cond
             ass
