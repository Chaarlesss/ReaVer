...
let logger =
  { Log.fmt = Format.std_formatter; Log.module_name = "TransGen"; Log.level = Log.Debug3 }
;;
...
let initfinal env cf _ =
  ...
  let split_constraints e =
    ...
    BddapronUtil.boolexpr_to_numconvex_list env.Env.env env.Env.cond e
  in
  (* split into initial and other locations *)
  let initial = split_constraints initial in
  (*(Cudd.Bdd.dand initial inv) in*)
  let final = split_constraints final in
  (*(Cudd.Bdd.dand final inv) in*)
  Log.debug2_o
    logger
    (Util.list_print (BddapronUtil.print_boolexpr env.Env.env env.Env.cond))
    "initial="
    initial;
  Log.debug2_o
    logger
    ...
    final;
  let exprs =
    PartitionUtil.compute_partition_expr_list env inv (List.append initial final)
  in
  Log.debug2_o
    logger
    (Util.list_print (BddapronUtil.print_boolexpr env.Env.env env.Env.cond))
  ...
  let notinitlocs = Cfg.get_locidset_by_inv env cfg (Cudd.Bdd.dnot cf.Program.c_init) in
  let finallocs = Cfg.get_locidset_by_inv env cfg cf.Program.c_final in
  let notfinallocs = Cfg.get_locidset_by_inv env cfg (Cudd.Bdd.dnot cf.Program.c_final) in
...
let manual exprlist env cf _ =
  ...
  let iflocs =
    ...
    then PSette.empty compare
    else
      Cfg.get_locidset_by_inv
        env
        cfg
  ...
  Log.debug2_o
    ...
    "exprlist="
    exprlist;
  PartitionUtil.partition_all_by env cfg iflocs cf.Program.c_ass exprlist;
         ...
         (fun v ->
              ...
              List.map
                (fun label ->
                   Bddapron.Expr0.Benum.eq_label
                     env.Env.env
                     env.Env.cond
                     (Bddapron.Expr0.Benum.var env.Env.env env.Env.cond v)
                     label)
            ...
            | `Bint (sign, len) ->
              let smin, max =
                if sign
                then
                  ( int_of_float (1. -. (2. ** (float_of_int len -. 1.)))
                  , int_of_float ((2. ** (float_of_int len -. 1.)) -. 1.) )
                else 1, int_of_float ((2. ** float_of_int len) -. 1.)
              in
              let build i max list =
                Bddapron.Expr0.Bint.eq_int
                  env.Env.env
                  env.Env.cond
                  (Bddapron.Expr0.Bint.var env.Env.env env.Env.cond v)
                  i
                :: list
              in
              build smin max []
    ...
    then
      PSette.iter
        (fun arcid ->
           ...
           match arc with
           | Arc.Flow (ass, f) ->
             let f = BddapronUtil.simplify_equs env.Env.env env.Env.cond f sinv in
             ...
             Log.debug_o
               logger
               (MtbddUtil.print_array_mtbdd
                  env.Env.env
                  env.Env.cond
                  (Bddapron.Apronexpr.print Env.symbol))
               "numarr="
               numarr;
             let ass2, f2 =
               Array.fold_left
                 (fun (ass2, f2) (g, arr) ->
                    ...
                    then ass2, f2
                      ...
                      , BddapronUtil.simplify_equs env.Env.env env.Env.cond f notg )))
                 (ass, f)
                 (Cudd.Mtbdd.guardleafs numarr)
             in
             PSHGraph.replace_attrhedge cfg arcid (Arc.Flow (ass2, f2))
           | _ ->
             ...
             let preass =
               BddapronAnalysis.bool_pre_assertion2
                 get_primed_var
                 env.Env.env
                 env.Env.cond
                 sinvb
                 ...
                 env.Env.i_vars
             in
             let ass2 = Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond ass preass in
             Log.debug3_o
               logger
               (BddapronUtil.print_boolexpr env.Env.env env.Env.cond)
               "preass: "
               ass2;
             if Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond ass2
             ...
             then (
               ...
               Log.debug3_o
                 logger
                 (BddapronUtil.print_equations env.Env.env env.Env.cond)
                 "eqs: "
                 f;
               let f2 =
                 if not (Bddapron.Expr0.Bool.is_cst env.Env.env env.Env.cond ass2)
                 then BddapronUtil.simplify_equs env.Env.env env.Env.cond f ass2
                 else f
               in
               (* remove identity self-loops  *)
               if
                 BddapronAnalysis.is_id_equs2
                   get_primed_var
                   get_unprimed_var
                   env.Env.env
                   env.Env.cond
               ...
               then PSHGraph.remove_hedge cfg arcid
                 ...
                 PSHGraph.replace_attrhedge cfg arcid (Arc.replace_ass_equs arc (ass2, f)))
             else
               PSHGraph.replace_attrhedge cfg arcid (Arc.replace_ass_equs arc (ass2, f)))
        succs
  in
  PSHGraph.iter_vertex cfg (fun loc inv ~pred ~succ ->
    if not (PSette.is_empty succ) then refine_by_destloc_loc loc inv succ);
  Cfg.remove_unreachable_locs env cfg cf.Program.c_init;
  cf
;;
...
let boolbacksim env cf _ =
  ...
  let bool_post env f inv assertion =
    let fexprlist =
      BddapronUtil.get_fexprlist
        (BddapronUtil.get_primed_var env.Env.env)
        env.Env.env
        env.Env.cond
        (Env.get_bool_equs env f)
    in
    let forget_supp =
      Cudd.Bdd.dand
        env.Env.cond.Bdd.Cond.supp
        (BddapronUtil.supp_of_vars
           env.Env.env
           (List.append env.Env.bs_vars env.Env.bi_vars))
    in
    let phi =
      BddapronAnalysis.bool_image2
        ...
        inv
    in
    Log.debug2_o logger (Env.print_boolexpr env) "phi=" phi;
    PSette.add phi (PSette.empty compare)
  in
  PartitionUtil.partition_recsucc
    env
    cfg
    cf.Program.c_ass
    bool_post
    (Cfg.get_locidset_by_inv env cfg (Cudd.Bdd.dor cf.Program.c_init cf.Program.c_final));
  cf
;;
...
let remove_bool_inputs env cf _ =
  ...
  let remove_bool_inputs_arc (arcid, arc, sloc, dloc) =
    ...
    let f2 =
      if not (Bddapron.Expr0.Bool.is_cst env.Env.env env.Env.cond ass2)
      then BddapronUtil.simplify_equs env.Env.env env.Env.cond f1 ass2
      else f1
    in
  ...
  let split_arc (arcid, arc, sloc, dloc) =
    ...
    let aflist =
      Array.fold_left
        (fun af1 (g, _) ->
           let gass =
             BddapronUtil.simplify_boolexpr
               env.Env.env
               env.Env.cond
               (Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond g ass2)
               env.Env.cond.Bdd.Cond.careset
           in
           Log.debug3_o logger (Env.print_boolexpr env) "possibly non-convex g: " gass;
           let afn =
             List.fold_left
               (fun af2 g ->
