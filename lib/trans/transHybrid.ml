  ...
  let iflocs =
    Cfg.get_locidset_by_inv
      ...
      (Bddapron.Expr0.Bool.dor
         ...
         cf.Program.c_final)
  in
  PartitionUtil.partition_all_by
    env
    cfg
    iflocs
    cf.Program.c_ass
    (PSette.elements
       (PartitionUtil.nummodes1 vars env cf.Program.c_cont_equs cf.Program.c_ass));
  cf
;;
...
let modes_bool2 vars env cf _ =
  ...
  PartitionUtil.partition_all_by
    ...
    cf.Program.c_ass
    (PSette.elements
       (PartitionUtil.nummodes2 vars env cf.Program.c_cont_equs cf.Program.c_ass));
  cf
;;
...
let convex_staycond env cf _ =
  ...
  let convex_staycond_loc loc =
    ...
    let invs =
      BddapronUtil.boolexpr_to_numconvex_list
        env.Env.env
        env.Env.cond
        (Bddapron.Expr0.Bool.tdrestrict inv env.Env.cond.Bdd.Cond.careset)
    in
    Log.debug2_o logger (Util.list_print (Env.print_boolexpr env)) "invs=" invs;
    ...
    then (
      ...
      | Some (_, g, f) ->
        ...
        let suppvars =
          List.fold_left
            (fun supp (_, expr) ->
               PSette.union supp (Bddapron.Expr0.support env.Env.env env.Env.cond expr))
            (Bddapron.Expr0.support
               env.Env.env
               env.Env.cond
               (Bddapron.Expr0.Bool.to_expr g))
            f
        in
        let suppvars =
        ...
        let otherlocs =
          PSette.diff (Cfg.get_locidset cfg) (PSette.singleton compare loc)
        in
