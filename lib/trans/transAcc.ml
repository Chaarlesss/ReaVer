       ...
       | `Apron e ->
         Array.fold_left
           (fun res (_, a) ->
              res
              && not
                   (BddapronAccel.is_nonacc_action
                      env.Env.env
                      env.Env.cond
                      env.Env.ni_vars
                      v
...
let get_accel_preds env cfg loc =
  ...
  PSette.fold
    ...
    accelpreds
    (PSette.empty (fun (a1, _, _, _, _) (a2, _, _, _, _) ->
...
let inputize_tr_fct env (ass, f) inputize_num =
  let substb = List.map (fun v -> v, Env.get_newinput env v) env.Env.bs_vars in
         ...
         let neweq =
           ...
           | `Apron _ ->
             ...
             then
               ( v
               , `Apron
                   (Bddapron.Expr0.Apron.var
                      env.Env.env
                      env.Env.cond
                      (Env.get_newinput env v)) )
             else v, Bddapron.Expr0.substitute_by_var env.Env.env env.Env.cond expr substb
           | _ ->
             (* if not identity *)
             if not (BddapronUtil.is_id_or_const_equ env.Env.env env.Env.cond (v, expr))
             then (
               let rexpr = BddapronUtil.expr_forget_supp bsupp expr in
               match rexpr with
               | `Bool re ->
                 if Bddapron.Expr0.Bool.is_true env.Env.env env.Env.cond re
                 then (* replace true by top = a new unconstrained boolean input *)
                   ( v
          ...
          (fun af1 (g, a) ->
             ...
             let is_boolid =
               BddapronAnalysis.is_id_equs2
                 ...
                 env.Env.cond
                 assg2
                 (Env.get_bool_equs env f3)
                 inv
             in
             let is_numid =
               BddapronUtil.is_id_equs env.Env.env env.Env.cond (Env.get_num_equs env f3)
             in
             if
               (not (Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond assg2))
               && not (is_boolid && is_numid)
             then (
               let is_accel =
                 BddapronAccel.is_acc_actions
                   env.Env.env
                   env.Env.cond
                   env.Env.ni_vars
                   (Array.of_list vars)
               ...
               let newarc =
                 ...
                 then
                   if is_boolid
                   then Arc.Accel (assg2, Env.get_num_equs env f2)
                   else (
                     match decoupling_mode with
                     | DecoupleNone -> Arc.Nonacc (assg2, f2)
                     | _ -> Arc.BoolAccel (assg2, f2))
                 else if is_boolid
                 then Arc.Nonacc (assg2, f2)
                 else if is_numid
                 then Arc.Nonacc (assg2, f2)
                 else if
                   (match decoupling_mode with
                    | DecoupleBoolNacc -> true
                    | _ -> false)
                   && BddapronAccel.exists_acc_action
                        env.Env.env
                        env.Env.cond
                        env.Env.ni_vars
                        (Array.of_list vars)
                        a
                 then Arc.BoolNaccAccel (assg2, f2)
...
let flatten_loops env cf _ =
  ...
  let flatten_loops_loc loc inv pred succ loops =
    ...
    let laccs, lnaccs =
      let partfct arcid =
        match PSHGraph.attrhedge cfg arcid with
        | Arc.Accel (_, _) | Arc.BoolAccel (_, _) | Arc.BoolNaccAccel (_, _) -> true
        | _ -> false
      in
    ...
    in
    (* remove loops in loc ---------------------------------------------------*)
    let remove_loops () = PSette.iter (fun l -> PSHGraph.remove_hedge cfg l) loops in
        ...
        (fun v ->
           List.iter2
             (fun dv arc ->
                let _ =
                  if v <> dv
                  then Cfg.add_arc env cfg v dv arc
                  else (
    ...
    Log.debug2
      logger
      ("(accelerable/non-accelerable) loops = ("
       ^ string_of_int (List.length laccs)
       ^ ","
       ^ string_of_int (List.length lnaccs)
       ^ ")");
  ...
  PSHGraph.iter_vertex cfg (fun loc inv ~pred ~succ ->
    ...
    if PSette.cardinal loops > 0
    then flatten_loops_loc loc inv (PSette.diff pred loops) (PSette.diff succ loops) loops);
  cf
;;
...
let decouple_equs env cf _ =
  ...
  let decouple_tr_fcts_arc boolnacc (arcid, ass, f, sloc, dloc) =
    ...
    let fa1, fna1 = if boolnacc then split_acc_nonacc env f1 else split_num_bool env f1 in
    PSHGraph.replace_attrhedge cfg arcid (Arc.Accel (ass, fa1));
    if not (PSette.mem dloc !dlocs)
    then (
      ...
      let _ =
        ...
        then (
          ...
          let fna2 =
            List.append
              (BddapronUtil.get_id_equs_for env.Env.env env.Env.cond navars)
              fna1
          in
...
let inputize env cf _ =
  ...
  let inputize_arc boolnacc (arcid, ass, f, sloc, dloc) =
    ...
    Log.debug3_o
      logger
      (Util.list_print Format.pp_print_int)
      "accel_preds: "
    ...
    then
      ...
      then (
        ...
        let newarc1 =
          if boolnacc
          then (
            let a, f = inputize_tr_fct env (ass, f1) true in
            Arc.BoolNacc (a, f))
          else (
            let a, f = inputize_tr_fct env (ass, f1) false in
            Arc.Bool (a, f))
        in
        PSHGraph.remove_hedge cfg arcid;
        ...
        PSette.iter
          (fun (arcid, ass, f, _, _) ->
             PSHGraph.replace_attrhedge cfg arcid (Arc.Accel (inputize_guard env ass, f)))
          accelpreds)
