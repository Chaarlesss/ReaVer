    ...
    | e1 :: etl ->
      ...
      let s =
        PSette.fold
          (fun e2 s ->
             let ee1 =
               Cudd.Bdd.tdrestrict (Cudd.Bdd.dand e1 e2) env.Env.cond.Bdd.Cond.careset
             in
             let ee2 =
               Cudd.Bdd.tdrestrict (Cudd.Bdd.dand ne1 e2) env.Env.cond.Bdd.Cond.careset
             in
             if (not (Cudd.Bdd.is_false ee1)) && not (Cudd.Bdd.is_true ee1)
             then (
               let s = PSette.add ee1 s in
               if (not (Cudd.Bdd.is_false ee2)) && not (Cudd.Bdd.is_true ee2)
               then (
                 let s = PSette.add ee2 s in
                 PSette.remove e2 s)
               else s)
             else if (not (Cudd.Bdd.is_false ee2)) && not (Cudd.Bdd.is_true ee2)
             then PSette.add ee2 s
             else s)
          exprset
          exprset
      in
      compute s etl
  in
  PSette.elements (compute (PSette.singleton compare expr0) exprs)
;;
...
let check_partition_expr env inv expr =
  ...
  let expr2 =
    Bddapron.Expr0.Bool.tdrestrict
      (BddapronUtil.normalize_benumneg
         env.Env.env
         env.Env.cond
...
let partition_location_by env cfg loc partition_expr assertion =
  ...
  if check_partition_expr env inv partition_expr
  then (
    let _ = Cfg.split_location env cfg loc partition_expr assertion in
    ())
  else if not (Bddapron.Expr0.Bool.is_true env.Env.env env.Env.cond partition_expr)
  then
    Log.debug3_o
      logger
      (BddapronUtil.print_boolexpr env.Env.env env.Env.cond)
      "partition_expr invalid: "
      partition_expr
;;
...
let rec partition_disjoint_loc env cfg assertion loc exprlist =
  ...
  | e :: ee ->
    ...
    let newloc =
      if check_partition_expr env inv e
      then (
  ...
  PSette.iter
    (fun expr ->
       ...
       PSette.iter
         (fun loc ->
            let inv = PSHGraph.attrvertex cfg loc in
            if check_partition_expr env inv expr
            then (
              Log.debug_o logger (Env.print_boolexpr env) "partition location: " inv;
              Log.debug_o logger (Env.print_boolexpr env) "partition_expr = " expr;
...
let partition_locationsucc env cfg assertion loc heur =
  ...
  try
    let _, f =
      Arc.get_ass_equs
        env
        (PSHGraph.attrhedge cfg (PSette.choose (PSHGraph.succhedge cfg loc)))
    in
...
let partition_recsucc env cfg assertion heur iflocs =
  ...
  while not (PSette.is_empty !todoset) do
    ...
    Log.debug_o logger (PSette.print Format.pp_print_int) "todoset = " !todoset
  done
;;
...
let nummodes1 vars env f assertion =
  ...
  let numarrset = MtbddUtil.arraymtbdd_to_arrsetmtbdd compare tabarrset numarr in
  Log.debug3_o
    logger
  ...
  let boolguards = List.map (Cudd.Bdd.exist env.Env.cond.Bdd.Cond.supp) boolguards in
  ...
  Log.debug3_o
    logger
    (PSette.print ~sep:";" (BddapronUtil.print_boolexpr env.Env.env env.Env.cond))
    "guardset: "
    boolguards;
  boolguards
;;
...
let nummodes2 vars env f assertion =
  ...
  Log.debug3_o
    ...
    numarrset;
  let boolguards = MtbddUtil.get_guards_mtbdd numarrset in
  Log.debug3_o logger Format.pp_print_int "number of guards: " (List.length boolguards);
  let boolguards = Util.list2psette compare boolguards in
  Log.debug3_o
    logger
    (PSette.print (BddapronUtil.print_boolexpr env.Env.env env.Env.cond))
    "guardset: "
    boolguards;
  boolguards
;;
