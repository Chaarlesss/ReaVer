...
type bddaprondoman_t =
  ( Env.var_t
    , apronman_t
    , bddapronman_t
    , Env.var_t Bddapron.Domain0.t )
...
module type TEMPLATE_T =
  Domain.T
  with type doman_param_t = template_t
...
let create_interval_conss env =
  ...
  List.map (Apron.Parser.linexpr1_of_string env.Env.apronenv) (List.append vars nvars)
;;
...
let cons_true env =
  Apron.Lincons1.make (Apron.Linexpr1.make env.Env.apronenv) Apron.Lincons0.EQ
;;
...
let cons_instantiate env elt v =
  ...
  | Apron.Coeff.Scalar vv ->
    if Apron.Scalar.is_infty vv = 0
    then (
      let cons = Apron.Lincons1.make (Apron.Linexpr1.copy elt) Apron.Lincons0.SUPEQ in
      Apron.Lincons1.set_cst cons (Apron.Coeff.neg v);
      cons)
    else if Apron.Scalar.is_infty vv > 0
    then cons_false env
    else cons_true env
  | _ -> assert false
;;
    ...
    (fun i cons (v : bound_t) ->
       let cons =
         ...
         | Apron.Coeff.Scalar vv ->
           ...
           then (
             let cons =
               Apron.Lincons1.make (Apron.Linexpr1.copy cons) Apron.Lincons0.SUPEQ
             in
             Apron.Lincons1.set_cst cons (Apron.Coeff.neg v);
             cons)
           else if Apron.Scalar.is_infty vv > 0
...
let apron_approx_template apronenv apronman tt s =
  ...
  let calc_conss tt s =
    List.fold_right
      (fun t res ->
         ...
         if Apron.Scalar.is_infty max > 0
         then res
         else if Apron.Scalar.is_infty max = 0
         then (
  ...
  let res =
    if Apron.Abstract1.is_top apronman s || Apron.Abstract1.is_bottom apronman s
    then s
    else
...
let bddapron_approx_template env doman tt s =
  ...
  List.fold_left
    (fun res (sb, sn) ->
       let sn2 =
         { Apron.Abstract1.abstract0 = sn; Apron.Abstract1.env = env.Env.apronenv }
       in
...
module EmuProd = struct
  ...
  let make_man () : bddaprondoman_t =
    let bman : bddapronman_t = Bddapron.Domain0.make_bdd (Polka.manager_alloc_loose ()) in
  ...
  let convexify doman env s =
    ...
    Bddapron.Domain0.meet_condition
      ...
      env.Env.env
      env.Env.cond
      (Bddapron.Domain0.of_apron doman env.Env.env n)
      b
  ;;
  ...
  let assign_lexpr ?(doman = doman ()) env s equs =
    ...
    convexify
      doman
      env
      (Bddapron.Domain0.assign_lexpr doman env.Env.env env.Env.cond s vars exprs None)
  ;;

  let forget_list ?(doman = doman ()) env s vars =
    let doman, _ = doman in
    convexify doman env (Bddapron.Domain0.forget_list doman env.Env.env s vars)
  ;;
  ...
  let substitute_lexpr ?(doman = doman ()) env s equs =
    ...
    convexify
      doman
      env
      (Bddapron.Domain0.substitute_lexpr doman env.Env.env env.Env.cond s vars exprs None)
  ;;
  ...
  let of_boolnumlist ?(doman = doman ()) env bnlist =
    ...
    | (bexpr, apron) :: [] ->
      Bddapron.Domain0.meet_condition
        ...
        bexpr
    | _ -> convexify doman env (Bddapron.Domain0.of_bddapron doman env.Env.env bnlist)
  ;;
  ...
  let of_boolexpr ?(doman = doman ()) env bexpr =
    let doman, _ = doman in
    Bddapron.Domain0.meet_condition doman env.Env.env env.Env.cond (top env) bexpr
  ;;
  ...
  let meet_to_boolexpr ?(doman = doman ()) env s bexpr =
    Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond bexpr (to_boolexpr ~doman env s)
  ;;

  let meetbool_to_boolexpr ?(doman = doman ()) env s bexpr =
    Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond bexpr (to_boolexprbool ~doman env s)
  ;;
end
