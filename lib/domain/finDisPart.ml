...
let compare_apron apronman s1 s2 =
  ...
  let hash = Apron.Abstract1.hash apronman s1 - Apron.Abstract1.hash apronman s2 in
  let res =
    if hash <> 0 then hash else if Apron.Abstract1.is_eq apronman s1 s2 then 0 else -1
  in
...
let is_indep_in_guard env g vars1 vars2 =
  ...
  let gvars =
    Bddapron.Expr0.support env.Env.env env.Env.cond (Bddapron.Expr0.Bool.to_expr g)
  in
  let res =
    PSette.is_empty (PSette.inter (Util.list2psette compare vars1) gvars)
...
let is_finitely_generating env (pvars, ivars) (v, expr) =
  ...
  let check_equ (v, ee) =
    ...
    let rec check_coeffs j (sv, invars) =
      ...
      then sv, invars
        ...
        let pmone_coeff =
          Apron.Coeff.equal_int (Apron.Linexpr0.get_coeff e j) 1
          || Apron.Coeff.equal_int (Apron.Linexpr0.get_coeff e j) (-1)
        in
  ...
  Log.debug3_o
    logger
    (BddapronUtil.print_equation env.Env.env env.Env.cond)
    "equ: "
    (v, expr);
  match expr with
  | `Apron expr ->
    ...
    let rec check j =
      ...
      then true
        ...
        let apronequ =
          ( BddapronUtil.var_to_apronvar env.Env.env v
          , BddapronUtil.apronaction_to_linexpr env.Env.env env.Env.cond (snd gl.(j)) )
        in
        Log.debug3_o logger ApronUtil.print_equation "apronequ: " apronequ;
        match check_equ apronequ with
        (* coeffs=0 -> true *)
        | None, [] -> check (j + 1) (* svar coeffs=0 -> check indep inputs ivars *)
        | None, invars ->
          let invars = List.map (BddapronUtil.apronvar_to_var env.Env.env) invars in
          if is_indep_in_guard env (fst gl.(j)) ivars invars then check (j + 1) else false
          (* 1svar coeff=+-1, cst=0, sv in pvars -> check indep v sv *)
        | Some sv, [] ->
          Log.debug3 logger "+-x";
          let sv = BddapronUtil.apronvar_to_var env.Env.env sv in
          if (not (List.mem sv ivars)) && is_indep_in_guard env (fst gl.(j)) ivars [ sv ]
          then check (j + 1)
          else false
        | _, invars -> false)
    in
...
let get_tuple_vars (env : Env.t) apronman (pp, i) =
  ...
  if Apron.Abstract1.is_bottom apronman i && PSette.is_empty pp
  then vars, [||]
  else (
...
let change_env_vars (env : Env.t) apronman s vars =
  ...
  let res =
    if newvars = vars
    then Apron.Abstract1.top apronman (ApronUtil.env_of_vars env.Env.apronenv vars)
    else
      Apron.Abstract1.change_environment
        apronman
        s
        (ApronUtil.env_of_vars env.Env.apronenv vars)
        false
  in
...
let change_env_tuple_vars (env : Env.t) apronman (pp : 'a Apron.Abstract1.t PSette.t) vars
  : 'a Apron.Abstract1.t PSette.t
  =
  Util.psette_map
    (fun p -> change_env_vars env apronman p vars)
    (compare_apron apronman)
    pp
;;
...
let convexify_tuple env apronman ppapronenv pp =
  PSette.fold
    (fun p pi -> Apron.Abstract1.join apronman p pi)
    pp
    (Apron.Abstract1.bottom apronman ppapronenv)
;;
...
let bddapron_to_apron (env : Env.t) doman apronenv s =
  ...
  let bnlist =
    Bddapron.Domain0.to_bddapron
      doman
      (BddapronUtil.abstract_convexify env.Env.env env.Env.cond doman s)
  in
  if List.length bnlist < 1
  then Apron.Abstract1.bottom apronman apronenv
  else ApronUtil.abstract1_of_abstract0 apronenv (snd (List.hd bnlist))
;;
...
module FdpProd (Man : Domain.BDDAPRON_MAN_T) = struct
  ...
  let top ?(doman = doman ()) env =
    ...
    , ( PSette.empty (compare_apron apronman)
      , Apron.Abstract1.top apronman env.Env.apronenv ) )
  ;;
  ...
  let meet_condition ?(doman = doman ()) env (b, (pp, i)) e =
    ...
    let bb =
      Cudd.Bdd.exist
        ...
        (Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond b e)
    in
    let res =
      if Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond bb
      then bottom env
      else (
        ...
        let g =
          Bddapron.Expr0.Bool.dand
            env.Env.env
            env.Env.cond
            (BddapronUtil.boolexpr_forget_vars
               env.Env.env
               env.Env.cond
               doman
               (BddapronUtil.apronvars_to_vars env.Env.env varsp)
               e)
            (BddapronUtil.boolexpr_forget_vars
               env.Env.env
               env.Env.cond
               doman
               (List.append
                  (BddapronUtil.apronvars_to_vars env.Env.env varsi)
                  env.Env.ni_vars)
               e)
        in
        Log.debug3_o logger (Env.print_boolexpr env) "relaxed g = " g;
        let ppi =
          if PSette.is_empty pp
          then
            PSette.singleton
              (compare_apron apronman)
              (bddapron_to_apron
            ...
            Util.psette_map
              (fun p ->
                 ...
                 Log.debug3_o
                   logger
                   (BddapronUtil.print_abstract env.Env.env doman)
                   "pie = "
                   pie;
        ...
        in
        let bot =
          Apron.Abstract1.bottom apronman (ApronUtil.env_of_vars env.Env.apronenv varsi)
        in
        (*  Log.debug3_o logger (print_with_env) "ppi = " (ppi,bot); *)
        let ppp =
          if PSette.is_empty pp
          then PSette.empty (compare_apron apronman)
  ...
  let is_bottom ?(doman = doman ()) env (b, (pp, i)) =
    ...
    && Apron.Abstract1.is_bottom apronman i
    && PSette.is_empty pp
  ;;
  ...
  let is_leq ?(doman = doman ()) env (b1, (pp1, i1)) (b2, (pp2, i2)) =
    ...
    Log.debug3_o logger (print_with_env env) "eq s2 = " (b2, (pp2, i2));
    let res =
      if not (Bddapron.Expr0.Bool.is_eq env.Env.env env.Env.cond b1 b2)
      then false
      else (
        let apronman = Bddapron.Domain0.man_get_apron doman in
  ...
  let join ?(doman = doman ()) env (b1, (pp1, i1)) (b2, (pp2, i2)) =
    ...
    let p2i1 =
      Apron.Abstract1.join
        ...
        (change_env_vars env apronman i1 varsp2i1)
    in
    let p12 =
      if Array.length varsp12 = 0
      then PSette.empty (compare_apron apronman)
      else
        PSette.union
    ...
    let res =
      ...
      , (p12, concat_abstract env apronman i12 (concat_abstract env apronman p1i2 p2i1)) )
    in
    Log.debug3_o logger (print_with_env env) "res = " res;
    res
  ;;
  ...
  let meet ?(doman = doman ()) env (b1, (pp1, i1)) (b2, (pp2, i2)) =
    ...
    let res =
      if Bddapron.Expr0.Bool.is_false env.Env.env env.Env.cond b
      then bottom env
      else (
        let apronman = Bddapron.Domain0.man_get_apron doman in
        ( b
              ...
              (fun p1 pp ->
                 PSette.fold
                   (fun p2 pp ->
                      ...
                      if Apron.Abstract1.is_bottom apronman p then pp else PSette.add p pp)
                   pp2
                   pp)
              pp1
              (PSette.empty (compare_apron apronman))
  ...
  let widening ?(doman = doman ()) env (b1, (pp1, i1)) (b2, (pp2, i2)) =
    ...
    let p1i2 =
      convexify_tuple env apronman (ApronUtil.env_of_vars env.Env.apronenv varsp1i2) p1i2
    in
  ...
  let assign_lexpr ?(doman = doman ()) env ((b, (pp, i)) : t) equs : t =
    ...
    let pequs, iequs =
      List.partition
        (is_finitely_generating env (varsp, varsi))
        (Env.get_num_equs env equs)
    in
    Log.debug3_o logger (Env.print_equations env) "fin. gen. equs = " pequs;
    let pvars = BddapronUtil.vars_to_apronvars env.Env.env (fst (Env.split_equs pequs)) in
    let ivars = BddapronUtil.vars_to_apronvars env.Env.env (fst (Env.split_equs iequs)) in
    let vars, exprs = Env.split_equs equs in
    let res =
      if Array.length varsp <> 0
      then
        PSette.fold
          (fun p s ->
             let sp =
               Bddapron.Domain0.of_bddapron
                 ...
                 (to_boolnumlist
                    ~doman
                    env
                    (b, (PSette.singleton (compare_apron apronman) p, i)))
             in
             Log.debug3_o
               logger
               (BddapronUtil.print_abstract env.Env.env doman)
               ...
               sp;
             let spost =
               BddapronUtil.abstract_convexify
                 env.Env.env
                 env.Env.cond
                 doman
             ...
             Log.debug3_o
               ...
               "spost = "
               spost;
             if Bddapron.Domain0.is_bottom doman spost
             then s
             else (
               let b, n = List.hd (Bddapron.Domain0.to_bddapron doman spost) in
               let p =
                 if Array.length pvars > 0
                 then
                   PSette.singleton
                     (compare_apron apronman)
                     (change_env_vars
        ...
        let sp =
          Bddapron.Domain0.of_bddapron
            ...
            [ ( b
              , Apron.Abstract1.abstract0
                  (Apron.Abstract1.change_environment apronman i env.Env.apronenv false) )
            ]
        in
        Log.debug3_o logger (BddapronUtil.print_abstract env.Env.env doman) "sp = " sp;
        let spost =
          BddapronUtil.abstract_convexify
            env.Env.env
            env.Env.cond
  ...
  let forget_list ?(doman = doman ()) env (b, (pp, i)) vars =
    ...
    let bsupp =
      BddapronUtil.supp_of_vars env.Env.env (Util.list_inter vars env.Env.b_vars)
    in
    ( Cudd.Bdd.exist bsupp b
    , ( Util.psette_map
          (fun p -> Apron.Abstract1.forget_array apronman p papronvars false)
          (compare_apron apronman)
          pp
  ...
  let flow ?(doman = doman ()) env (b, (pp, i)) equs staycond =
    let equs = BddapronUtil.simplify_equs env.Env.env env.Env.cond equs staycond in
    Log.debug3_o logger (print_with_env env) "flow to s = " (b, (pp, i));
    Log.debug3_o logger (Env.print_boolexpr env) "with staycond = " staycond;
    let apronman = Bddapron.Domain0.man_get_apron doman in
    let varsp, varsi = get_tuple_vars env apronman (pp, i) in
    let contvars = BddapronUtil.vars_to_apronvars env.Env.env (fst (List.split equs)) in
    let varsp, varsi = Util.array_diff varsp contvars, Util.array_union varsi contvars in
    let _, (_, ii) = top env in
    let ppp =
      if Array.length varsp = 0
      then PSette.empty (compare_apron apronman)
      else change_env_tuple_vars env apronman pp varsp
    in
  ...
  let to_boollinconsslist ?(doman = doman ()) env (b, (pp, i)) =
    ...
    :: List.map
         (fun p -> b, Apron.Abstract1.to_lincons_array apronman p)
         (PSette.elements pp)
  ;;
      ...
      (PSette.fold
         (fun p bacc ->
            Bddapron.Expr0.Bool.dor
              ...
              env.Env.cond
              bacc
              (BddapronUtil.apron_to_boolexpr
                 env.Env.env
                 env.Env.cond
                 apronman
                 (concat_abstract env apronman p i)))
