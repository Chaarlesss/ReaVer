...
let linexpr_depends_on_some_var e vars =
  ...
  let rec check_coeffs j =
    if j >= dims
    then false
    else if Apron.Coeff.is_zero (Apron.Linexpr0.get_coeff e j)
    then check_coeffs (j + 1)
    else true
  in
  check_coeffs 0
;;
...
let make_zero_linexpr env =
  ...
  { Apron.Linexpr1.linexpr0 = linexpr; Apron.Linexpr1.env }
;;

(******************************************************************************)

(** creates equations (v,0) for the given variables *)
let get_zero_equations_for env vars = Array.map (fun v -> v, make_zero_linexpr env) vars
...
let bottom0 man env =
  let dim = Apron.Environment.dimension env in
  Apron.Abstract0.bottom man dim.Apron.Dim.intd dim.Apron.Dim.reald
;;

(******************************************************************************)
let abstract1_of_abstract0 env s = { Apron.Abstract1.abstract0 = s; Apron.Abstract1.env }

(******************************************************************************)
let lincons1_of_lincons0 env c = { Apron.Lincons1.lincons0 = c; Apron.Lincons1.env }
...
let rename_primed_to_unprimed get_primed_var abstract1 vars =
  ...
  { Apron.Abstract1.abstract0 =
      Apron.Abstract0.of_lincons_array
        man
        dim.Apron.Dim.intd
        dim.Apron.Dim.reald
        earr.Apron.Lincons1.lincons0_array
...
let elapse s d =
  ...
  for i = 0 to Apron.Generator1.array_length dgen - 1 do
    ...
    if Apron.Generator1.get_typ g1 == Apron.Generator0.VERTEX
    then Apron.Generator1.set_typ g1 Apron.Generator0.RAY
  done;
...
let project_to_zero s zero_vars =
  ...
  Array.iteri
    (fun i lc ->
       Apron.Lincons1.array_set linconsarr i (Apron.Lincons1.make lc Apron.Lincons1.EQ))
    (Array.of_list lexpr0);
  let s0 = Apron.Abstract1.of_lincons_array man env linconsarr in
  Apron.Abstract1.meet man (Apron.Abstract1.forget_array man s zero_vars false) s0
;;
...
let get_translator man eqs g ivars =
  ...
  Array.iteri
    (fun j eq ->
       ...
       Apron.Lincons1.array_set dt j (Apron.Lincons1.make newexpr Apron.Lincons1.EQ))
    eqs;
  (* J xi <= k /\ d = T xi + u *)
  let dxi =
    Apron.Abstract1.meet_lincons_array man (Apron.Abstract1.of_lincons_array man env g) dt
  in
...
let lincons0_is_eq c1 c2 =
  ...
  && Apron.Linexpr0.cmp c1.Apron.Lincons0.linexpr0 c2.Apron.Lincons0.linexpr0 = 0
;;
...
let linconss_is_unsat linconss =
  ...
  && Apron.Lincons1.is_unsat
       (lincons1_of_lincons0
          linconss.Apron.Lincons1.array_env
...
let lincons_is_true c =
  lincons0_is_eq
    (Apron.Lincons1.get_lincons0 c)
    (Apron.Lincons0.make (Apron.Linexpr0.make None) Apron.Lincons0.SUPEQ)
;;
...
let linconss_false env =
  { Apron.Lincons1.lincons0_array =
      [| Apron.Lincons1.get_lincons0 (Apron.Lincons1.make_unsat env) |]
...
let linconss_append l1 l2 =
  { Apron.Lincons1.lincons0_array =
      Array.append l1.Apron.Lincons1.lincons0_array l2.Apron.Lincons1.lincons0_array
