...
let acc_transreset_forward g gs eqs ivars reset_vars d s =
  ...
  let tauo =
    if Apron.Abstract1.is_bottom man sg
    then taus
    else (
      (* (tau(S)/\G elapse D^t)+D^r *)
      let sgd =
        Apron.Abstract1.meet
          man
          (ApronUtil.project_forget (ApronUtil.elapse_with_domain sg dt) reset_vars)
          dr
      in
      Log.debug3_o logger ApronUtil.print_abstract "(tau(S)/\\G elapse D^t)+D^r = " sgd;
      (* tau(((tau(S)/\\G elapse D^t)+D^r)) *)
      let sgdgd = post g eqs ivars sgd in
      Log.debug3_o
        logger
        ApronUtil.print_abstract
        "tau(((tau(S)/\\G elapse D^t)+D^r)) = "
        sgdgd;
      sgdgd)
  in
...
let acc_transreset_backward g gs eqs ivars reset_vars d s =
  ...
  let sp = pre g eqs ivars s in
  Log.debug3_o logger ApronUtil.print_abstract "pre(s) = " sp;
  if Apron.Abstract1.is_bottom man sp
  then s
  else (
    ...
    Log.debug3_o logger ApronUtil.print_abstract "dr = " dr;
    (* pre(s)|t elapse Dt *)
    let spd = ApronUtil.elapse_with_domain (ApronUtil.project_forget sp reset_vars) ddt in
    Log.debug3_o logger ApronUtil.print_abstract "pre(s)|t elapse Dt = " spd;
    (* (pre(s)|t elapse Dt)/\\gs\\dr *)
    let spdg =
      Apron.Abstract1.meet_lincons_array
        man
        (Apron.Abstract1.meet
           man
           (ApronUtil.elapse_with_domain (ApronUtil.project_forget sp reset_vars) ddt)
           dr)
        gs
    in
    Log.debug3_o logger ApronUtil.print_abstract "(pre(s)|t elapse Dt)/\\g/\\dr = " spdg;
...
let get_weakest_guard trans_man svars ivars g =
  ...
  ( Apron.Abstract1.to_lincons_array trans_man (ApronUtil.project_forget g ivars)
  , Apron.Abstract1.to_lincons_array trans_man (ApronUtil.project_forget g svars) )
;;
...
let is_id_equ (v, e) =
  ...
  let rec check_coeff j =
    ...
    then true
      ...
      if (vdim <> j && Apron.Coeff.is_zero c) || (vdim = j && Apron.Coeff.equal_int c 1)
      then check_coeff (j + 1)
      else false)
  in
  Apron.Coeff.is_zero (Apron.Linexpr0.get_cst e) && check_coeff 0
;;
...
let get_id_vars eqs =
  Array.of_list
    (Array.fold_left
       (fun idvars (v, e) -> if is_id_equ (v, e) then v :: idvars else idvars)
       []
       eqs)
;;
...
let get_reset_vars idvars eqs =
  Array.of_list
    (Array.fold_left
       (fun rvars (v, expr) ->
          if is_reset_equ idvars (v, expr) then v :: rvars else rvars)
       []
       eqs)
;;
...
let acc ?(dir = `Forward) ~trans_man g eqs ivars s =
  ...
  let d =
    ...
    then (
      ...
      let gg =
        Apron.Abstract1.to_lincons_array
          man
          (Apron.Abstract1.meet_lincons_array man sid gi)
      in
      (* compute translator d *)
      let nonid_eqs =
        Array.of_list
          (List.filter (fun (v, _) -> Util.array_mem v nonid_vars) (Array.to_list eqs))
      in
      let d =
        ApronUtil.get_translator trans_man nonid_eqs gg (Array.append ivars id_vars)
      in
    ...
    else ApronUtil.get_translator trans_man eqs gi ivars
  in
  Log.debug3_o logger ApronUtil.print_abstract "translator d = " d;
  (* accelerate *)
  let res =
    if Array.length reset_vars = 0
    then (
      match dir with
...
(******************************************************************************)
let rec check_coeffs env e vdim dims ivars j (is_nonacc, is_state_id, has_inputs) =
  if j >= dims
  then is_nonacc, is_state_id, has_inputs
  else (
    let zero_coeff = Apron.Coeff.is_zero (Apron.Linexpr0.get_coeff e j) in
    let one_coeff = Apron.Coeff.equal_int (Apron.Linexpr0.get_coeff e j) 1 in
    ...
    | false, false, _, true ->
      check_coeffs env e vdim dims ivars (j + 1) (is_nonacc, is_state_id, true)
    | false, true, _, _ ->
      check_coeffs env e vdim dims ivars (j + 1) (is_nonacc, is_state_id, has_inputs)
    | true, false, false, _ -> true, is_state_id, has_inputs
    | true, true, false, _ ->
      check_coeffs env e vdim dims ivars (j + 1) (is_nonacc, false, has_inputs)
    | true, _, true, _ ->
      check_coeffs env e vdim dims ivars (j + 1) (is_nonacc, is_state_id, has_inputs))
;;
...
let get_transition_type_equ (v, e1) ivars =
  ...
  let dims = ApronUtil.dims_of_env env in
  let vdim = Apron.Environment.dim_of_var env v in
  let has_nonzero_cst = not (Apron.Coeff.is_zero (Apron.Linexpr0.get_cst e)) in
  match check_coeffs env e vdim dims ivars 0 (false, true, false), has_nonzero_cst with
  | (true, _, _), _ -> NonAcc
...
let get_transition_type equs ivars =
  ...
  let rec check_equ i (has_reset, has_trans) =
    ...
    then (
      ...
      let has_nonzero_cst = not (Apron.Coeff.is_zero (Apron.Linexpr0.get_cst e)) in
      match
        check_coeffs env e vdim dims idivars 0 (false, true, false), has_nonzero_cst
      with
      | (true, _, _), _ -> NonAcc
