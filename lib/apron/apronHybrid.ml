...
let has_noncst_dynamics env ivars (v, e) =
  ...
  let rec check_coeff j =
    ...
    then false
      ...
      Apron.Coeff.is_zero (Apron.Linexpr0.get_coeff e j)
...
let has_nonzero_dynamics env ivars (v, e) =
  ...
  in
  (not (Apron.Coeff.is_zero (Apron.Linexpr0.get_cst e))) || check_coeff 0
;;
...
let get_nonzero_dynamics_vars env eqs ivars =
  Array.of_list
    (fst (List.split (List.filter (has_nonzero_dynamics env ivars) (Array.to_list eqs))))
;;
...
let get_noncst_dynamics_vars env eqs ivars =
  Array.of_list
    (fst (List.split (List.filter (has_noncst_dynamics env ivars) (Array.to_list eqs))))
;;
...
let cont_elapse2 get_primed_var trans_man staycond eqs ivars s =
  ...
  let discvars = Util.array_diff vars (Array.concat [ contvars; ivars; primed_vars ]) in
  ...
  let eqs = Array.append eqs (ApronUtil.get_zero_equations_for env discvars) in
  ...
  let s_rel =
    Apron.Abstract1.meet_lincons_array
      trans_man
      (Apron.Abstract1.meet trans_man s_vd0 wstaycond)
      fexpr
  in
...
let cont_elapse1 trans_man staycond eqs ivars s =
  ...
  let noncontvars =
    Util.array_diff
      vars
      (Array.append
         (Array.append contvars ivars)
         (Array.map ApronUtil.get_primed_var vars))
  in
  ...
  let istaycond =
    Apron.Abstract1.to_lincons_array
      ...
      (ApronUtil.project_forget
         (Apron.Abstract1.of_lincons_array trans_man env staycond)
         svars)
  in
  ...
  let eqs = Array.append eqs (ApronUtil.get_zero_equations_for env noncontvars) in
  (* compute translator *)
...
let flow
  ...
  (* compute time elapse *)
  let selapse =
    cont_elapse trans_man staycond eqs ivars (ApronUtil.change_domain s trans_man)
  in
