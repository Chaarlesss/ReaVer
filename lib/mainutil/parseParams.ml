...
let get_option options defaults key converter =
  try converter (Mappe.find key options) with
  | Not_found -> converter (Mappe.find key defaults)
;;
    ...
    (List.filter
       (fun expr ->
          ...
          | ex ->
            ...
            false)
       (List.map
          (fun str -> Bddapron.Parser.expr0_of_string env.Env.env env.Env.cond str)
          strlist))
;;
