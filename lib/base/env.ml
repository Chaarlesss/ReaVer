...
let logger =
  { Log.fmt = Format.std_formatter; Log.module_name = "Base.Env"; Log.level = Log.Debug3 }
;;
...
let print_vars env fmt vars = Util.list_print Format.pp_print_string fmt vars
let print_boolexpr env fmt bexpr = BddapronUtil.print_boolexpr env.env env.cond fmt bexpr
let print_equations env fmt equs = BddapronUtil.print_equations env.env env.cond fmt equs
...
let compute_careset env = Bdd.Cond.compute_careset env.cond ~normalized:true

(* start variable reordering of CUDD *)
let cudd_reorder env = Cudd.Man.reduce_heap env.cuddman Cudd.Man.REORDER_SIFT 0

let get_s_varslist env =
  List.fold_left (fun l v -> (v, Bddapron.Env.typ_of_var env.env v) :: l) [] env.s_vars
;;
...
let cudd_group env s_vars =
  ...
  List.iter
    (fun (var, typ) ->
       ...
       | #Bdd.Env.typ ->
         ...
         Cudd.Man.group env.Bdd.Env.cudd tid.(0) (3 * Array.length tid) Cudd.Man.MTR_FIXED
       | _ -> ())
    s_vars;
  ...
  Cudd.Man.group env.Bdd.Env.cudd 0 !bool_size Cudd.Man.MTR_DEFAULT;
  (* make numerical constraints reorderable *)
  Cudd.Man.group env.Bdd.Env.cudd !bool_size !cond_size Cudd.Man.MTR_DEFAULT;
  (* keep numerical constraints below the finite type varibles *)
  Cudd.Man.group env.Bdd.Env.cudd 0 (!bool_size + !cond_size + 1) Cudd.Man.MTR_FIXED
;;
...
let cudd_make_varmap env s_vars =
  ...
  List.iter
    (fun (var, typ) ->
       ...
       | #Bdd.Env.typ ->
         let tid = PMappe.find var env.Bdd.Env.vartid in
         let length = Array.length tid in
         (* size of type *)
         Array.iter
           (fun id ->
              tab.(id) <- id + length;
...
let get_q_var zvar is_continuous = "_q" ^ zvar ^ if is_continuous then "c" else "d"

(* returns a the ith q bool input variable for the given zero-crossing id *)
let get_q_bool_i_var zvar i = get_q_var zvar true ^ "_i" ^ string_of_int i
...
let create_newinputs s_vars = List.map (fun (v, t) -> v ^ "''", t) s_vars
...
let number_of_boolvars_varstyp env vars =
  Mappe.fold (fun _ t size -> size + size_of_booltype env t) vars 0
;;

let number_of_boolvars (env : t) (vars : vars_t) =
  List.fold_left
    (fun size v -> size + size_of_booltype env.env (Bddapron.Env.typ_of_var env.env v))
    0
    vars
;;
...
let compute_sizes cuddman typedefs s_vars i_vars =
  let tempenv =
    Bddapron.Env.make cuddman ~symbol ~relational:false ~bddindex0:0 ~bddsize:1
  in
    ...
    (fun fmt td ->
       PMappe.iter
         (fun a b ->
            ...
            Bdd.Env.print_typdef Format.pp_print_string fmt b)
         td)
    "typedefs:"
    typedefs;
...
let var_of_cuddid env i =
  let s =
    PMappe.filter
      (fun v iarr -> Array.fold_right (fun ii accu -> accu || ii = i) iarr false)
...
let rec make_bool_vars prefix n =
  if n = 0 then [] else (prefix ^ string_of_int n, `Bool) :: make_bool_vars prefix (n - 1)
;;
...
let make ?(num_factor = 1) typedefs s_varstyp i_varstyp =
  ...
  let bool_state_size, bool_input_size, num_size =
    compute_sizes cuddman typedefs s_varstyp i_varstyp
  in
  ...
  (* create environment *)
  let env =
    Bddapron.Env.make cuddman ~symbol ~relational:false ~bddindex0:0 ~bddsize:!bool_size
  in
  let cond =
    Bddapron.Cond.make cuddman ~symbol ~bddindex0:!bool_size ~bddsize:!cond_size
  in
  (* add types *)
  ...
  let primed_varstyp =
    List.map (fun (v, t) -> BddapronUtil.get_primed_var env v, t) s_varstyp
  in
  ...
  let iintvars, irealvars =
    ( BddapronUtil.vars_to_apronvars env iintvars
    , BddapronUtil.vars_to_apronvars env irealvars )
  in
  let sintvars, srealvars = List.partition is_int ns_vars in
  let pintvars, prealvars =
    ( BddapronUtil.vars_to_apronvars
        env
  ...
  in
  let sintvars, srealvars =
    ( BddapronUtil.vars_to_apronvars env sintvars
    , BddapronUtil.vars_to_apronvars env srealvars )
  in
  ...
  let apronenv =
    Apron.Environment.make
      ...
      (Array.concat [ srealvars; prealvars; irealvars ])
  in
...
let get_newzero_var env =
  ...
  if l >= !zeros_size then raise Bdd.Env.Bddindex;
