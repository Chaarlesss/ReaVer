...
module type BDDAPRON_MAN_T = sig
  type apronman_t
...
(** Bddapron t: B^p -> A *)
module Pow (Man : BDDAPRON_MAN_T) = struct
  ...
  let meetbool_to_boolexpr ?(doman = doman ()) env s bexpr =
    Bddapron.Expr0.Bool.dand env.Env.env env.Env.cond bexpr (to_boolexprbool env s)
  ;;
end
...
module Prod (Man : BDDAPRON_MAN_T) = struct
  type t = Env.var_t Bddapron.Domain0.t
  type numdomain_t = Man.apronman_t Apron.Abstract0.t
  type doman_t = (Env.var_t, Env.var_t Bddapron.Domain0.t) Man.man_t
  type doman_param_t = unit
  ...
  let meet_condition ?(doman = doman ()) env s e =
    convexify env (Bddapron.Domain0.meet_condition doman env.Env.env env.Env.cond s e)
  ;;
  ...
  let join ?(doman = doman ()) env s1 s2 =
    convexify env (Bddapron.Domain0.join doman s1 s2)
  ;;

  let meet ?(doman = doman ()) env s1 s2 =
    convexify env (Bddapron.Domain0.meet doman s1 s2)
  ;;
  ...
  let forget_list ?(doman = doman ()) env s vars =
    convexify env (Bddapron.Domain0.forget_list doman env.Env.env s vars)
  ;;
