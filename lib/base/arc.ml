...
let logger =
  { Log.fmt = Format.std_formatter; Log.module_name = "Arc"; Log.level = Log.Debug3 }
;;
...
let print env fmt arc =
  ...
  | BoolNacc (a, f)
  | Nonacc (a, f) ->
    if not (Bddapron.Expr0.Bool.is_true env.Env.env env.Env.cond a)
    then (
...
let simplify env arc phi =
  match arc with
  | Normal (a, f) ->
    let f = BddapronUtil.simplify_equs env.Env.env env.Env.cond f phi in
    Normal (a, f)
  | Loop (a, f) ->
    let f = BddapronUtil.simplify_equs env.Env.env env.Env.cond f phi in
    Loop (a, f)
