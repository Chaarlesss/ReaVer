...
let print_matgen fmt (typ, mat) =
  fprintf fmt "%s: %a" (Apron.Generator0.string_of_typ typ) Matrix.Mpqf.print mat
;;
...
let product_lmu_ls lmu ls =
  ...
  List.iter
    (fun mu ->
       List.iter (fun s -> res := Bound.{ mu1 = mu; mu2 = 1. -. mu; s } :: !res) ls)
    lmu;
  !res
;;
...
let lweight n = product_lmu_ls (lmu n) [ -1.; 1. ]
...
let lbinary_of_lweight ~nbcoeffs (lweight : Bound.weight list) =
  ...
  for i2 = nbcoeffs - 2 downto 1 do
    for i1 = i2 - 1 downto 0 do
      List.iter (fun t -> res := `Binary (t, i1, i2) :: !res) lweight
    done
  done;
  !res
;;
...
let lincons0_of_exprbound expr typ bound =
  Apron.(
    Lincons0.(
      ...
      in
      match typ with
      | `E -> { linexpr0 = linexpr0_of_expr_l expr bound; typ = EQ }
      | `L -> { linexpr0 = linexpr0_of_expr_l expr bound; typ = SUPEQ }
      | `U -> { linexpr0 = linexpr0_of_expr_u expr bound; typ = SUPEQ }))
;;
...
let tlincons0_of_lexprbound lexprbound =
  ...
  List.iter
    (fun (expr, (l, u)) ->
       if l = u
       then llincons0 := lincons0_of_exprbound expr `E l :: !llincons0
       else (
         let l, u = Bound.Int.round (l, u) in
         if l <> Bound.neginf
         then llincons0 := lincons0_of_exprbound expr `L l :: !llincons0;
         if u <> Bound.inf then llincons0 := lincons0_of_exprbound expr `U u :: !llincons0))
    lexprbound;
...
let matlincons_of_poly_guardtrans ~guardtrans ~apron ~poly : Matrix.Mpqf2.t =
  Apron.(
    ...
    let polyx =
      Abstract0.add_dimensions
        apron
        poly
        Dim.{ dim = [| nbdims - 1 |]; intdim = 0; realdim = 1 }
        true
    in
    if false
    then
      printf
        "polyx=%a@ "
        (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimx)
        polyx;
    ...
    then
      printf
        "polyy=%a@ "
        (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimy)
        polyy;
    let boxy = Abstract0.to_box apron polyy in
    let boxy = Array.map Matrix.FMpqf2.of_interval boxy in
    if false then printf "boxy=%a@ " (Print.array Matrix.FMpqf2.print_float) boxy;
    ...
    if false then printf "J.minmaxY=%a@ " Matrix.Mpqf2.print_float mat;
    (* 2. We multiply with uguardy *)
    if false then printf "guardy=%a@ " Matrix.Mpqf2.print_float guardy;
    let mat = if guardy <> [||] then Matrix.Mpqf2.gemm guardy mat else [||] in
    if false then printf "mat=guardy.J.minmaxY=%a@ " Matrix.Mpqf2.print_float mat;
    mat)
;;

let polyjordan_meet_with_matlincons ~apron ~polyjordan ~mat =
  Apron.(
    ...
    then
      printf
        ...
        (Print.array (Lincons0.print Apronaux.string_of_dimj))
        tlincons0;
    ...
    then
      printf
        "npolyjordan=%a@ "
        (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimj)
        npolyjordan;
    npolyjordan)
;;
  ...
  Apron.(
    ...
    let rec parcours res dim polyjordan2 mat =
      ...
      then (
        (* terminal case *)
        let npolyjordan =
          polyjordan_meet_with_matlincons ~apron ~polyjordan:polyjordan2 ~mat
        in
        if false
        then
          printf
            ...
            (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimj)
            npolyjordan;
        if Abstract0.is_bottom apron npolyjordan
        then res
        else if Abstract0.is_eq apron npolyjordan polyjordan
        then raise Identity (* identity := true; res *)
        else npolyjordan :: res)
      else (
        i := 0;
        halt := false;
        ...
        if not !halt
        then parcours res (dim + 1) polyjordan2 mat
        else (
          let res =
            let polyjordanp =
              Abstract0.meet_lincons_array
                apron
                polyjordan2
                [| lincons0_of_dim_sign dim true |]
            in
            if Abstract0.is_bottom apron polyjordanp
            then res
            else (
              let matp =
                Matrix.mapij
                  (fun i1 dim1 ((x, y) as coeff) ->
                     if dim1 = dim && i1 >= !i && Matrix.FMpqf.to_float y < 1. /. 0.
                     then y, y
                     else coeff)
                  mat
              in
          ...
          let res =
            let polyjordann =
              Abstract0.meet_lincons_array
                apron
                polyjordan2
                [| lincons0_of_dim_sign dim false |]
            in
            if Abstract0.is_bottom apron polyjordann
            then res
            else (
              let matn =
                Matrix.mapij
                  (fun i1 dim1 ((x, y) as coeff) ->
                     if dim1 = dim && i1 >= !i && Matrix.FMpqf.to_float x > -1. /. 0.
                     then x, x
                     else coeff)
                  mat
              in
...
let polyjordan_meet_with_poly_guardtrans ~guardtrans ~apron ~polyjordan ~poly
  ...
  let res =
    if !Option.quadrant
    then polyjordan_meet_with_matlincons_quadrant ~apron ~polyjordan ~mat
    else (
      let res = polyjordan_meet_with_matlincons ~apron ~polyjordan ~mat in
      if Apron.Abstract0.is_bottom apron res
      then None
      else if Apron.Abstract0.is_eq apron polyjordan res
      then Some []
      else Some [ res ])
  in
  if false
  then
    printf
      "res=%a@ "
      (Print.option
         (Print.list (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimj)))
      res;
  res
;;
...
let matgen_of_lingeny ~nbdims ~map_dim_lsij lingen0 : matgen =
  Apron.(
    Generator0.(
      ...
      then
        printf
          "matgen_of_lingeny nbdims=%i lingen0=%a@ "
          nbdims
          (Apronaux.print_generator0 ~string_of_dim:Apronaux.string_of_dimj)
          lingen0;
      let mat =
      ...
      Linexpr0.iter
        (fun coeff dim ->
           ...
           List.iter
             ...
             lsij)
        lingen0.linexpr0;
      if false then printf "matgen=%a@ " Matrix.Mpqf.print mat;
      lingen0.typ, mat))
;;

(* We assume M=S.jordan.Sinv, or jordan=Sinv.M.S *)
let assigngenx_of_polyjordany ~(trans : Trans.trans) ~apron poly : Trans.assigngen =
  ...
  then
    printf
      ...
      (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimj)
      poly
      (Apronaux.print_abstract0_gen ~string_of_dim:Apronaux.string_of_dimj)
      poly;
  ...
  if false then printf "assigngen = %a@ " Trans.print_assigngen res;
  res
;;
...
let abstract ~(apron : 'a Apron.Manager.t) ~(trans : trans) ~nmin ~nmax ltemplate
  : 'a Apron.Abstract0.t
  =
...
let abstrans_of_ltemplate ~guardtrans ~apron ltemplate =
  let polyjordan =
    abstract ~apron ~trans:guardtrans.trans ~nmin:0 ~nmax:max_int ltemplate
  in
  let assigngen =
...
let refine_abstrans_with_poly_guard ~apron ~abstrans ~poly : unit =
  if false
  then
    printf
      "Template.refine_with_poly_guard <= poly=%a@."
      (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimj)
      poly;
  let guardtrans = abstrans.guardtrans in
  if guardtrans.tlinconsx = [||]
  then abstrans.cassigngen <- abstrans.assigngen
  else (
    let olpolyjordan2 =
      polyjordan_meet_with_poly_guardtrans
        ~guardtrans
        ~apron
        ~polyjordan:abstrans.polyjordan
        ~poly
    in
    match olpolyjordan2 with
    | None -> abstrans.cassigngen <- assigngen_bottom
    | Some [] -> abstrans.cassigngen <- abstrans.assigngen
    | Some lpolyjordan2 ->
      if false then printf "computing n@.";
      let n =
        match lpolyjordan2 with
        | [ polyjordan2 ] ->
          Counting.main
            ~apron
            ~trans:guardtrans.trans
            ~polyjordan:polyjordan2
            ~first:0
            abstrans.ltemplate
        | _ ->
          ...
          while !first < !n && !n < max_int do
            ...
            if !map_ipolyjordan2 = Mappe.empty
            then n := max_int
            else (
              let n2, i2 =
                Mappe.fold
                  (fun i polyjordan2 (n2, i2) ->
                     if false
                     then
                       printf
                         "count i=%i polyjordan2=%a@."
                         i
                         (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimj)
                         polyjordan2;
                     let npoly =
                     ...
                     if npoly > n2 then npoly, i else n2, i2)
                  !map_ipolyjordan2
                  (-1, -1)
              in
              n := n2;
          ...
          !n
      in
      if false then printf "done, nfinal=%i@." n;
      if n = max_int
      then abstrans.cassigngen <- abstrans.assigngen
      else if n = 0
      then abstrans.cassigngen <- assigngen_bottom
      else (
        assert (
          if n >= 1
          then true
        ...
        let upolyjordan2 =
          Apron.Abstract0.join_array apron (Array.of_list lpolyjordan2)
        in
        if false
        then
          printf
            "done => %a@.meet polyjordan upolyjordan2 @?"
            (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimj)
            upolyjordan2;
