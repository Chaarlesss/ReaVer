...
let print_list ?first ?sep ?last print fmt list =
  ...
  if l >= 6 then pp_print_int fmt l else Print.list ?first ?sep ?last print fmt list
;;
...
let print_trans
  ...
  Format.fprintf
    fmt
    "{@   @[<v>jordan = %a@ s_xy = %a@ sinv_yx = %a@ dmap_coeff_dim = %a@ \
     map_dim_lsij=%a@ map_ij_sdim=%a tdim=%a@ tlinexpr0=%a@]@ }"
    Jordan.printr
    jordan
    Matrix.Mpqf.print
    s_xy
    ...
    (Mappe.print
       pp_print_int
       (Print.list (fun fmt (s, i, j) -> fprintf fmt "(%b,%i,%i)" s i j)))
    map_dim_lsij
    (Mappe.print
    ...
    tdim
    (Print.array (Apron.Linexpr0.print string_of_dim))
    tlinexpr0
;;

let print_guard fmt guard = Matrix.Mpqf2.print fmt guard
let print_uguard fmt uguard = Print.list ~sep:"@ or " print_guard fmt uguard

let print_guardtrans fmt { trans; tlinconsx; guardy } =
  let string_of_dim i = sprintf "x%i" i in
  Format.fprintf
    fmt
    ...
    print_trans
    trans
    (Print.array (Apron.Lincons0.print string_of_dim))
    tlinconsx
    print_guard
    guardy
;;
...
let print_assigngen fmt { lvertex; lray; lline } =
  Format.fprintf
    ...
    lvertex
    (print_list Matrix.Mpqf.print_float)
    lray
    (print_list Matrix.Mpqf.print_float)
    lline
;;
...
let guardtrans_of_trans_tlincons0 (trans : trans) (tlinconsx : Apron.Lincons0.t array)
  : guardtrans
  =
...
let uguard_negate (uguard : uguard) : uguard =
  let cucons = List.rev_map guard_negate uguard in
  List.fold_left uguard_or (List.hd cucons) (List.tl cucons)
;;

let assigngen_apply ~(trans : trans) man poly (assigngen : assigngen) =
  if false
  then
    printf
      "assigngen_apply poly:%a %a@ @?"
      (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimx)
      poly
      print_assigngen
      assigngen;
  if assigngen = assigngen_bottom
  then
    Apron.(
    ...
    then
      printf
        "limages=%a@ @?"
        (print_list (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimx))
        limages;
    ...
    then (
      ...
      then
        printf
          "poly generators=%a@ @?"
          (Print.array (Apron.Generator0.print (fun i -> sprintf "x%i" i)))
          tlingen0;
      ...
      Apron.Generator0.(
        Array.iter
          (fun lingen0 ->
             ...
             if false then printf "row=%a@ @?" (Print.array Matrix.FMpqf.print) row;
             List.iter
               (fun mat ->
                  ...
                  then
                    printf
                      "line mat=%a@ nrow=%a@ @?"
                      Matrix.Mpqf.print_float
                      mat
                      (Print.array Matrix.FMpqf.print)
                      nrow;
    ...
    respoly
    := Apron.Abstract0.join_array man (Array.of_list (!respoly :: List.tl limages));
    if false
    then
      printf
        "done => %a@."
    ...
    then
      printf
        ...
        (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimx)
        !respoly;
    if false then printf "assigngen_apply => @.";
    !respoly)
;;

let guardassigngen_apply ~(trans : trans) man poly ~tlincons ~assigngen =
  let poly2 = Apron.Abstract0.meet_lincons_array man poly tlincons in
  let res = assigngen_apply ~trans man poly2 assigngen in
  res
;;

let abstrans_apply man poly (abstrans : 'a abstrans) =
...
*)
  let res =
    guardassigngen_apply ~trans man poly ~tlincons ~assigngen:abstrans.cassigngen
  in
  if false
  then
    printf
  ...
  then
    printf
      "assigngen_apply inter guard => %a, %a@ @?"
      (Apronaux.print_abstract0 ~string_of_dim:Apronaux.string_of_dimx)
      res
      (Apronaux.print_abstract0_gen ~string_of_dim:Apronaux.string_of_dimx)
      res;
  let res2 =
    Apron.Abstract0.assign_linexpr_array man res2 trans.tdim trans.tlinexpr0 None
  in
  if false
  then
    printf
