...
module FMpqf = struct
  ...
  let of_float x =
    if x = 1. /. 0. then inf else if x = -1. /. 0. then neginf else of_float x
  ;;
...
module FMpqf2 = struct
  ...
  let of_int x =
    let y = FMpqf.of_int x in
    y, y
  ;;

  let of_float x =
    let y = FMpqf.of_float x in
    y, y
  ;;

  let of_mpqf x =
    let y = FMpqf.of_mpqf x in
    y, y
  ;;

  let of_mpfrf x =
    let y = FMpqf.of_mpfrf x in
    y, y
  ;;

  let of_scalar x =
    let y = FMpqf.of_scalar x in
    y, y
  ;;
  ...
  let to_coeff (x1, x2) =
    Apron.(
      Coeff.(
        Interval.(
          if Mpqf.cmp x1 x2 = 0
          then Scalar (Scalar.Mpqf x1)
          else Interval { inf = Scalar.Mpqf x1; sup = Scalar.Mpqf x2 })))
  ;;

  let sign (x1, x2) = if Mpqf.sgn x2 < 0 then -1 else if Mpqf.sgn x1 >= 0 then 1 else 0
  let neg (y1, y2) = FMpqf.neg y2, FMpqf.neg y1

  let inv ((x1, x2) as x) =
    let s = sign x in
    if s <> 0 then FMpqf.inv x2, FMpqf.inv x1 else failwith "FMpqf2.inv"
  ;;

  let myadd y z =
    if FMpqf.is_inf y
    then
      if FMpqf.is_inf z then if FMpqf.sgn y = FMpqf.sgn z then y else failwith "3" else y
    else if FMpqf.is_inf z
    then z
    else FMpqf.add y z
  ;;

  let add (y1, y2) (z1, z2) = myadd y1 z1, myadd y2 z2

  let mysub y z =
    if FMpqf.is_inf y
    then
      if FMpqf.is_inf z then if FMpqf.sgn y <> FMpqf.sgn z then y else failwith "4" else y
    else if FMpqf.is_inf z
    then z
    else FMpqf.sub y z
  ;;

  let sub (y1, y2) (z1, z2) = mysub y1 z2, myadd y2 z1

  let mymul y z =
    if FMpqf.is_inf y || FMpqf.is_inf z
    then (
  ...
  let scale y (z1, z2) =
    if Mpqf.sgn y < 0 then mymul y z2, mymul y z1 else mymul y z1, mymul y z2
  ;;

  let mycmp y z =
    if FMpqf.is_inf y
    then if FMpqf.is_inf z then FMpqf.sgn y - FMpqf.sgn z else FMpqf.sgn y
    else if FMpqf.is_inf z
    then -FMpqf.sgn z
    else FMpqf.cmp y z
  ;;
  ...
  let join (y1, y2) (z1, z2) =
    (if mycmp y1 z1 <= 0 then y1 else z1), if mycmp y2 z2 >= 0 then y2 else z2
  ;;
  ...
  let mul ((y1, y2) as y) ((z1, z2) as z) =
    ...
    and sz = sign z in
    if sy = 0
    then join (scale y1 z) (scale y2 z)
    else if sz = 0
    then join (scale z1 y) (scale z2 y)
    else if sy > 0
    then
      if sz > 0
      then mymul y1 z1, mymul y2 z2
      else (
        assert (sz < 0);
...
module FC = struct
  ...
  let print fmt el =
    let s =
      if el.im = 0.0
      then sprintf "%.3G" el.re
      else if el.re = 0.0
      then sprintf "%.3Gi" el.im
      else if el.im > 0.
      then sprintf "%.3G+%.3Gi" el.re el.im
      else sprintf "%.3G-%.3Gi" el.re (-.el.im)
    in
  ...
  let of_scalar x = { re = FF.of_scalar x; im = 0. }
...
let mapij f t = Array.mapi (fun i row -> Array.mapi (fun j el -> f i j el) row) t
let map f t = Array.map (fun row -> Array.map f row) t
...
let interval_of_scalar mat = map (fun x -> x, x) mat
...
module Make (F : Field) = struct
  ...
  let make0 x y =
    ...
    Array.init x (fun i -> Array.make y el)
  ;;
  ...
  let diag lmat =
    let glength = List.fold_left (fun res mat -> res + dim1 mat) 0 lmat in
...
*)
  let print_float fmt mat =
    Print.array
      ~first:"[@[<v>"
      ~last:"@]]"
      (Print.array ~first:"[@[<hov 8>" ~last:"@]]" F.print_float)
      fmt
      mat
  ;;
  ...
  let print_F fmt mat =
    Print.array
      ~first:"[@[<v>"
      ~last:"@]]"
      (Print.array ~first:"[@[<hov 8>" ~last:"@]]" F.print)
      fmt
      mat
  ;;
    ...
    for i = 0 to pred dimx do
      for k = 0 to pred dimz do
        ...
        for j = 0 to pred dimy do
          ...
          if (not (F.is_zero elxy)) && not (F.is_zero elyz)
          then res := F.add !res (F.mul elxy elyz)
        done;
        matxz.(i).(k) <- !res
      done
    done;
    ...
    Array.map
      (fun rowxy ->
         ...
         for i = 0 to pred dimy do
           if (not (F.is_zero rowxy.(i))) && not (F.is_zero vecy.(i))
           then res := F.add !res (F.mul rowxy.(i) vecy.(i))
         done;
         !res)
      matxy
  ;;
  ...
  let vgemm vecx matxy =
    ...
    for j = 0 to dimy - 1 do
      for i = 0 to dimx - 1 do
        if (not (F.is_zero vecx.(i))) && not (F.is_zero matxy.(i).(j))
        then resy.(j) <- F.add resy.(j) (F.mul vecx.(i) matxy.(i).(j))
      done
    done;
    resy
  ;;
  ...
  let row_set_linexpr_affine row linexpr0 =
    ...
    then
      printf
        "nbdims=%i linexpr0=%a row=%a@ "
        nbdims
        (Apronaux.print_linexpr0 ?string_of_dim:None)
        linexpr0
        (Print.array F.print)
        row;
    ()
  ...
  let of_tlincons_affine ~nbdims tlincons0 =
    Apron.(
      Lincons0.(
        let length = ref 0 in
        Array.iter
          (fun lincons -> length := !length + if lincons.typ = EQ then 2 else 1)
          tlincons0;
  ...
  let row_to_linexpr_affine row =
    ...
    for dim = nbdims - 2 downto 0 do
      let el = row.(dim) in
      if not (F.is_zero el) then Apron.Linexpr0.set_coeff linexpr0 dim (F.to_coeff el)
    done;
    if false
    then
      printf
        "nbdims=%i row=%a linexpr0=%a@ "
        nbdims
        (Print.array F.print)
        row
        (Apronaux.print_linexpr0 ?string_of_dim:None)
  ...
  let to_tlinexpr_affine mat = Array.map row_to_linexpr_affine mat
  let to_tlinexpr_linear mat = Array.map row_to_linexpr_linear mat
