...
let newton ~epsilon ~t0 ~(f : float -> float) ~(fp : float -> float) ~(v : float) =
  ...
  let aeps =
    ...
    if av <= ldexp 1. (-40) then epsilon else epsilon *. av
  in
  if false
  then printf "newton epsilon=%.5G, t=%.5G, ft=%.5G, fpt=%.5G@." epsilon !t !ft (fp t0);
...
let eval_jordan_ith ~trans i =
  ...
  DMappe.iter
    (fun coeff dim ->
       if false then printf "coeff=%a dim=%i @?" Jordan.print_coeff coeff dim;
       if dim < nbdims
       then (
         let c, k = coeff in
         let v =
           match c with
           | `Real lambda -> Bound.Real.single_int ~lk:(lambda, k) i
           | `Sin (lambda, theta, r) ->
  ...
  then
    printf
      "@.Counting.eval_jordan_ith %i => %a@."
      i
      (Print.array (fun fmt x -> fprintf fmt "%.5G" x))
      vertex;
  vertex
;;
...
let counting_jordanpoly_preamble ~apron ~first ~start ~trans ~polyjordan lexpr =
  ...
  let res =
    try
      while !n < start do
        ...
        if not (Apron.Abstract0.is_leq apron poly polyjordan) then raise Exit
      done;
      max_int
    with
    | Exit ->
      ...
      then
        List.iter
          (fun e ->
             let intv =
               Bound.bound_expr ~dmap_coeff_dim:trans.dmap_coeff_dim ~nmin:!n ~nmax:!n e
             in
             let refintv =
               Bound.bound_expr
                 ~dmap_coeff_dim:trans.dmap_coeff_dim
                 ~nmin:0
                 ~nmax:max_int
                 e
             in
             if not (Bound.Int.is_leq ~eps:(ldexp 1. 16) intv refintv)
             then
               printf
                 "expr=%a intv=%a refintv=%a@."
                 Bound.print_expr
                 e
                 Bound.Int.print
                 intv
      ...
      !n
  in
  if false then printf "res=%i@." res;
  res
;;
    ...
    (fun start expr ->
       let starte =
         ...
         | `Unary dim ->
           ...
           (match coeff with
            | `Real lambda -> Bound.Real.n_single ~lk:(lambda, k)
            | _ -> 0)
         | `Binary (t, dim1, dim2) ->
...
let binary_of_linexpr0 linexpr0 =
  ...
  then
    printf
      "binary_of_linexpr0 %a@.%a@."
      (Apronaux.print_linexpr0 ~string_of_dim:Apronaux.string_of_dimj)
      linexpr0
      (Apron.Linexpr0.print Apronaux.string_of_dimj)
      linexpr0;
  try
    ...
    then
      printf
        "mudim=%a@."
        (Print.array (fun fmt (coeff, dim) -> fprintf fmt "(%.5G,%i)" coeff dim))
        mudim;
    if !nbcoeff = 2
    then (
      let mudim =
        if fst mudim.(0) < 0. then Array.map (fun (mu, dim) -> -.mu, dim) mudim else mudim
      in
      ...
      let norm =
        if norm = 1. /. 0.
        then
          if abs_float mu1 >= abs_float mu2
          then mu1 *. sqrt (1. +. sqr (mu2 /. mu1))
          else abs_float mu2 *. sqrt (1. +. sqr (mu1 /. mu2))
        else norm
      in
...
let select_leqn_for_counting_jordanpoly ~apron ~trans ~polyjordan lexpr =
  ...
  let lexpr =
    Array.fold_left
      ...
      tlincons0
  in
  if false then printf "lexpr=%a@." (Print.list Bound.print_expr) lexpr;
  List.fold_left
    (fun res expr ->
       match expr with
       | `Unary dim ->
         ...
         (match coeff with
          | `Real lambda when lambda > 0. && (lambda, k) <> (1., 0) ->
            ...
            if lambda >= 1. && u < infinity
            then `Unary (lambda, k, u) :: res
            else if lambda < 1. && l > 0.
            then `Unary (lambda, k, l) :: res
            else res
       ...
       | `Binary (t, dim1, dim2) ->
         ...
         (match coeff1, coeff2 with
          | `Real lambda1, `Real lambda2
            when (lambda1, k1) > (1., 0) || (lambda2, k2) > (1., 0) ->
            let linexpr0 = Apron.Linexpr0.make None in
            Apron.Linexpr0.set_list
              linexpr0
            ...
            let l, u = Bound.Int.round (Apronaux.interval_to_float2 interval) in
            if u < infinity && (t.s = 1. || (lambda1, k1) >= (lambda2, k2))
            then `Binary (t.mu1, lambda1, k1, t.s *. t.mu2, lambda2, k2, u) :: res
            else if l > neg_infinity && t.s = -1. && (lambda2, k2) >= (lambda1, k1)
            then `Binary (t.mu2, lambda2, k2, t.s *. t.mu1, lambda1, k1, -.l) :: res
            else res
          | `Sin (lambda1, theta1, r1), `Sin (lambda2, theta2, r2)
            when k1 = 0 && k1 = k2 && lambda1 = lambda2 && theta1 = theta2 ->
            assert (r1 = 1 - r2);
            let linexpr0 = Apron.Linexpr0.make None in
            Apron.Linexpr0.set_list
              linexpr0
            ...
            then
              printf
                "w=%a, lambda1=%.5G, theta1=%.5G, r1=%i, [l,u]=[%.5G,%.5G], mu=%.5G, \
                 varphi=%.5G@."
                Bound.print_weight
                t
                lambda1
...
let print_eqn fmt = function
  | `Unary (lambda, k, sup) -> fprintf fmt "Unary(lambda=%.5G,k=%i,sup=%.5G)" lambda k sup
  | `Binary (mu1, lambda1, k1, mu2, lambda2, k2, sup) ->
    fprintf
      fmt
      "Binary(mu1=%2G,lambda1=%.5G,k1=%i,mu2=%2G,lambda2=%.5G,k2=%i,sup=%.5G)"
      mu1
      lambda1
      k1
      mu2
  ...
  | `SinUpper (mu, lambda, theta, varphi, u) ->
    fprintf
      fmt
      "SinUpper(mu=%.5G,lambda=%.5G,theta=%.5G,varphi=%.5G,sup=%.5G)"
      mu
      lambda
      theta
      varphi
  ...
  | `SinLower (mu, lambda, theta, varphi, l) ->
    fprintf
      fmt
      "SinLower(mu=%.5G,lambda=%.5G,theta=%.5G,varphi=%.5G,inf=%.5G)"
      mu
      lambda
      theta
      varphi
...
let solve_for_counting_jordanpoly_one ~start eqn =
  ...
  if false then printf "solve_for_counting <= %i %a@." start print_eqn eqn;
  let t =
    match eqn with
    | `Unary (lambda, k, sup) ->
    ...
    | `Binary (mu1, lambda1, k1, mu2, lambda2, k2, sup) ->
      ...
      let fp t =
        (mu1
         *. Bound.binom_float t k1
         *. (lambda1 ** (t -. fk1))
         *. Bound.add_ratio_dpoly_poly ~gamma:gamma1 t k1)
        +. (mu2
            *. Bound.binom_float t k2
            *. (lambda2 ** (t -. fk2))
    ...
    | `SinUpper (mu, lambda, theta, varphi, u) ->
      ...
      let fp t =
        mu
        *. (lambda ** t)
        *. ((gamma *. sin ((theta *. t) +. varphi))
            +. (theta *. cos ((theta *. t) +. varphi)))
      in
      ...
      then (
        ...
        then neg_infinity
          ...
          let t = newton ~epsilon ~t0 ~f ~fp ~v:u in
          assert (t >= tinf && t <= t0);
          let ct = ceil t in
        ...
        try
		 ...
		 (overapproximation in case (f tenveloppe)=u) *)
            ...
            for i = 0 to 10 do
              ...
              let ct = ceil t in
              let ct1 = if t = ct then ct +. 1. else ct in
              if ct1 <= t0 +. (Bound.pi2 /. theta) && f ct1 > u then raise (Val ct1)
            done;
            infinity)
          else (
            (* lambda<=1. *)
    ...
    | `SinLower (mu, lambda, theta, varphi, l) ->
      ...
      then (
        ...
        then neg_infinity
          ...
          let tinf = t0 -. (Bound.pi2 /. theta) in
          let t = newton ~epsilon ~t0 ~f ~fp ~v:l in
          assert (t >= tinf && t <= t0);
          let ct = ceil t in
          let ct1 = if t = ct then ct +. 1. else ct in
        ...
        try
		 ...
		 (overapproximation in case (f tenveloppe)=u) *)
            let tmax = max tenveloppe tstart in
            let fk0 =
              ceil (((theta *. tmax) +. varphi -. Bound.pi) /. (2. *. Bound.pi))
            in
            for i = 0 to 10 do
              let fk = fk0 +. float_of_int i in
              ...
              let ct = ceil t in
              let ct1 = if t = ct then ct +. 1. else ct in
              if ct1 <= t0 +. (Bound.pi2 /. theta) && f ct1 < l then raise (Val ct1)
            done;
            infinity)
          else (
            (* lambda<=1. *)
  ...
  in
  let res =
    if t >= float_of_int (max_int - 1)
    then max_int
    else if t = neg_infinity
    then min_int
    else int_of_float t
  in
  if false then printf "solve_for_counting => res=%i@." res;
  res
;;
...
let main ~apron ~trans ~polyjordan ~first (lexpr : Bound.expr list) =
  ...
  then (
    ...
    let leqn = select_leqn_for_counting_jordanpoly ~apron ~trans ~polyjordan lexpr in
