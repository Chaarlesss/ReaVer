...
let apron_interval_compare tab itv1 itv2 =
  Apron.(
    Interval.(
      if is_bottom itv1 && not (is_bottom itv2)
      then tab.(2) <- tab.(2) - 0
      else if is_bottom itv2 && not (is_bottom itv1)
      then tab.(2) <- tab.(2) + 0
      else (
        if Scalar.is_infty itv1.inf <> 0
        then (if not (Scalar.is_infty itv2.inf <> 0) then tab.(0) <- tab.(0) + 1)
        else if Scalar.is_infty itv2.inf <> 0
        then tab.(1) <- tab.(1) - 1
        else (
          ...
          then tab.(3) <- tab.(3) - 1);
        if Scalar.is_infty itv1.sup <> 0
        then (if not (Scalar.is_infty itv2.sup <> 0) then tab.(0) <- tab.(0) + 1)
        else if Scalar.is_infty itv2.sup <> 0
        then tab.(1) <- tab.(1) - 1
        else (
...
let parse () =
  ...
  <- { lexbuf.Lexing.lex_curr_p with Lexing.pos_fname = !inputfilename };
  let prog =
    try Parser.auto Lexer.token lexbuf with
    | Parsing.Parse_error ->
    ...
    | Syntax.Error str ->
      ...
      Format.fprintf
        err_formatter
        "Lexical error: %s, line %i, character %i-%i: '%s': %s@."
        startp.Lexing.pos_fname
        startp.Lexing.pos_lnum
        (startp.Lexing.pos_cnum - startp.Lexing.pos_bol)
        (endp.Lexing.pos_cnum - endp.Lexing.pos_bol)
    ...
    | _ as exn ->
      ...
      Format.fprintf
        err_formatter
        "Problem here: %s, line %i, character %i-%i: '%s'@."
        startp.Lexing.pos_fname
        startp.Lexing.pos_lnum
        (startp.Lexing.pos_cnum - startp.Lexing.pos_bol)
        (endp.Lexing.pos_cnum - endp.Lexing.pos_bol)
...
let _ =
  ...
  then (
    ...
    if !Option.debug >= 1
    then (
      let stab = Array.make 4 0
      and dtab = Array.make 4 0 in
      let scount = Array.make 2 0
      and dcount = Array.make 2 0 in
      PSHGraph.iter_vertex auto (fun vertex _ ~pred ~succ ->
        if vertex <> "linit"
        then (
