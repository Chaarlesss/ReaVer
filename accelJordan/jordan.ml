...
type 'a t =
  { lblock : 'a block list
  ; dim : int (** should be the sum of the size of the blocks *)
  }
...
let print_block printa fmt { eigenvalue; lk; _ } =
  fprintf fmt "{ eigen=%a; lk=%a }" printa eigenvalue (Print.list pp_print_int) lk
;;

let print printa fmt jordan = Print.list (print_block printa) fmt jordan.lblock

let print_cblock fmt x =
  print_block (fun fmt c -> fprintf fmt "(%G,%Gi)" c.Complex.re c.Complex.im) fmt x
;;

let printc fmt x =
  print (fun fmt c -> fprintf fmt "(%G,%Gi)" c.Complex.re c.Complex.im) fmt x
;;
...
let mat_C_of_F matD = Matrix.map (fun r -> { Complex.re = r; Complex.im = 0. }) matD
       ...
       List.iter
         (fun size ->
            for j = 0 to size - 1 do
              ...
              if j < size - 1 then mat.(!i).(!i + 1) <- Complex.one;
              incr i
            done)
         block.lk)
    jordan.lblock;
...
let block_of_cblock2 cblock1 cblock2 =
  ...
  let cblock1, cblock2, s =
    if cblock1.eigenvalue.im < 0. then cblock1, cblock2, 1. else cblock2, cblock1, -1.
  in
  let norm = Complex.norm cblock2.eigenvalue in
  let angle = Complex.arg cblock2.eigenvalue in
  let block = { cblock2 with eigenvalue = `Complex (cblock2.eigenvalue, norm, angle) } in
...
let conjugate2 mat =
  Matrix.maptranspose (fun x -> Complex.{ re = x.re /. 2.; im = -.x.im /. 2. }) mat
;;

let real_of_complex ~(sinv_yx : Matrix.C.t) ~(jordany : Complex.t t) ~(s_xy : Matrix.C.t)
  ...
  let rec parcours (ltinv_yx, lblock, lt_xy) = function
    ...
    | cblock1 :: (cblock2 :: l2 as l1) ->
      if cblock1.eigenvalue.Complex.im <> 0.
      then
        if cblock1.eigenvalue = Complex.conj cblock2.eigenvalue
        then (
  ...
  if true
  then (
    printf "sinv_yx=%a@ s_xy=%a@ " Matrix.C.print sinv_yx Matrix.C.print s_xy;
    printf
      "jordany.lblock=%a@ lblock=%a@ lt_xy=%a@ ltinv_yx=%a@ "
      (Print.list print_cblock)
      jordany.lblock
      (Print.list print_rblock)
      lblock
      (Print.list Matrix.C.print)
      lt_xy
      (Print.list Matrix.C.print)
      ltinv_yx);
  let njordany = { jordany with lblock } in
  let tinv_yx = Matrix.C.diag ltinv_yx in
  let t_xy = Matrix.C.diag lt_xy in
  if true then printf "tinv_yx=%a@ t_xy=%a@ " Matrix.C.print tinv_yx Matrix.C.print t_xy;
  ...
  then (
    ...
    printf "j=%a@ " Matrix.C.print j;
    printf
      "(sinv_yx j s_xy)=%a@ "
      Matrix.C.print
      (Matrix.C.gemm sinv_yx (Matrix.C.gemm j s_xy));
    printf
      "jr:%a@ = (t_xy j tinv_yx):%a@ "
      Matrix.C.print
      jr
      Matrix.C.print
      (Matrix.C.gemm t_xy (Matrix.C.gemm j tinv_yx));
    printf
      "(sinv_yx tinv_yx jr t_xy s_xy)=%a@ "
      Matrix.C.print
      (Matrix.C.gemm
         (Matrix.C.gemm sinv_yx tinv_yx)
         (Matrix.C.gemm jr (Matrix.C.gemm t_xy s_xy)));
    printf "nsinv_yx=%a@ ns_xy=%a@ " Matrix.C.print nsinv_yx Matrix.C.print ns_xy;
    printf
      "(nsinv_yx jr s_xy)=%a@ "
      Matrix.C.print
      (Matrix.C.gemm nsinv_yx (Matrix.C.gemm jr ns_xy)));
...
let block_of_cblock2_sage cblock1 cblock2 =
  ...
  let norm = Complex.norm cblock2.eigenvalue in
  let angle = Complex.arg cblock2.eigenvalue in
  { cblock2 with eigenvalue = `Complex (cblock2.eigenvalue, norm, angle) }
;;
...
let real_of_complexsage
  ...
  if true
  then (
    printf "sinv_yx=%a@ s_xy=%a@ " Matrix.F.print sinv_yx Matrix.F.print s_xy;
    printf
      "jordany.lblock=%a@ lblock=%a@ "
      (Print.list print_cblock)
      jordany.lblock
      (Print.list print_rblock)
      lblock);
...
let dmap_coeff_dim (jordan : real t) : (coeff, int) DMappe.t =
  ...
  List.iter
    ...
    jordan.lblock;
  if false then printf "dmap_coeff_dim=%a@ " (DMappe.print print_coeff pp_print_int) !dmap;
  !dmap
;;
             ...
             (fun order ->
                ...
                then
                  for i = 0 to order - 1 - k do
                    ...
                    map_ij_sdim
                    := Mappe.add (row, row + (2 * k)) (true, dimcos) !map_ij_sdim;
                    map_ij_sdim
                    := Mappe.add (row + 1, row + (2 * k) + 1) (true, dimcos) !map_ij_sdim;
                    map_ij_sdim
                    := Mappe.add (row, row + (2 * k) + 1) (false, dimsin) !map_ij_sdim;
                    map_ij_sdim
                    := Mappe.add (row + 1, row + (2 * k)) (true, dimsin) !map_ij_sdim
                  done;
