...
let print fmt { tlinconsx; tdim; tlinexpr0; tlingen0 } =
  ...
  fprintf
    ...
    tlinexpr0
    (Print.array (Apron.Generator0.print string_of_dim))
    tlingen0
;;
...
let of_jordan ~apron abstrans =
  ...
  let tlingen0 =
    Apron.(
      ...
      let mat =
        Array.map (Matrix.Mpqf.row_of_linexpr_affine ~nbdims:(nbdims + 1)) tlinexpr0
      in
      Array.iteri
        (fun i row -> row.(i) <- Matrix.FMpqf.sub row.(i) (Matrix.FMpqf.of_int 1))
        mat;
