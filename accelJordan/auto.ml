...
let linexpr0_of_linexpr ~env linexpr =
  ...
  then
    printf
      "linexpr0_of_linexpr@.%a@."
      (Print.list (fun fmt (name, coeff) ->
         fprintf fmt "(%a,%s)" Apron.Coeff.print coeff name))
      linexpr;
  let linexpr0 = Linexpr0.make None in
  List.iter
    (fun (var, coeff) ->
       if var = ""
       then Linexpr0.set_cst linexpr0 coeff
       else (
...
let arc_of_syntax
      ...
      (t : Syntax.guardedtransformation)
  : arc
  =
  ...
  let ldim, llinexpr0 =
    List.fold_left
      (fun (ldim, llinexpr0) (dim, linexpr0) -> dim :: ldim, linexpr0 :: llinexpr0)
      ([], [])
      ldimlinexpr0
  in
  let tdim = Array.of_list ldim
  and tlinexpr0 = Array.of_list llinexpr0 in
  let ntrans =
    match jordan with
    | `None -> `Assign (tlincons0, tdim, tlinexpr0)
    | `Jordan (sinv_yx, cjordany, s_xy) ->
      let sinv_yx, rjordany, s_xy =
        Jordan.real_of_complex ~sinv_yx ~jordany:cjordany ~s_xy
      in
      let sinv_yx = Matrix.mpqf_of_d sinv_yx in
      let s_xy = Matrix.mpqf_of_d s_xy in
      let trans =
        Trans.trans_of_jordan ~tdim ~tlinexpr0 ~s_xy ~sinv_yx ~jordan:rjordany
      in
      let guardtrans = Trans.guardtrans_of_trans_tlincons0 trans tlincons0 in
      let ltemplate =
        Template.ltemplate ~nbcoeffs:(Jordan.nbcoeffs rjordany) logtemplate
      in
...
let graph_of_syntax
  ...
  List.iter
    (fun (org, dest, transf) ->
       if not (PSHGraph.is_vertex graph org) then PSHGraph.add_vertex graph org ();
...
let of_syntax ~logtemplate ~(apron : Polka.loose Polka.t Apron.Manager.t) (sys : Syntax.t)
  : arc t
  =
...
let accgraph_of_graph (map_jordan : jordan -> [< allarc ]) (graph : arc t) =
  ...
  (* We copy vertices *)
  PSHGraph.iter_vertex graph (fun vertex attr ~pred ~succ ->
    PSHGraph.add_vertex ngraph vertex attr);
  (* We iterate on hedges, with the exception of self-loops *)
  let id = ref min_int in
  PSHGraph.iter_hedge graph (fun hedge arc ~pred ~succ ->
...
let jgraph_of_graph graph : [ `Assign of assign | `Jordan of jordan ] t =
  accgraph_of_graph (fun x -> `Jordan (Trans.abstrans_copy x)) graph
;;

let dgraph_of_graph ~apron graph : [ `Assign of assign | `Derivative of Derivative.t ] t =
...
let string_of_dim env dim = Var.to_string (Environment.var_of_dim env dim)
  ...
  Print.array
    ...
    (fun fmt (dim, linexpr) ->
       fprintf
         ...
         (Linexpr0.print (string_of_dim env))
         linexpr)
    fmt
    tab
;;
...
let make_fixpoint
      ~apron
      (graph :
        ([< `Assign of assign | `Jordan of jordan | `Derivative of Derivative.t ] as 'a) t)
  =
  let info = PSHGraph.info graph in
  FixpointType.
    { bottom = (fun vertex -> Abstract0.bottom apron 0 info.nbdims)
    ; canonical = (fun vertex abs -> ())
    ; is_bottom = (fun vertex abs -> Abstract0.is_bottom apron abs)
    ; is_leq = (fun vertex abs1 abs2 -> Abstract0.is_leq apron abs1 abs2)
    ; join = (fun vertex abs1 abs2 -> Abstract0.join apron abs1 abs2)
    ; join_list = (fun vertex labs -> Abstract0.join_array apron (Array.of_list labs))
    ; widening = (fun vertex abs1 abs2 -> Abstract0.widening apron abs1 abs2)
    ; odiff = None
    ; (* Some begin fun vertex abs1 abs2 ->
      ...
      *)
      abstract_init = (fun vertex -> Abstract0.top apron 0 info.nbdims)
    ; arc_init = (fun hedge -> ())
    ; apply =
        (fun hedge tabs ->
          ...
          let res =
            ...
            | `Assign (assign : assign) ->
              ...
              then (
                ...
                printf
                  "Assign @[<v>abs:%a@ tlincons0:%a@ tdim:%a@ tlinexpr0:%a@ "
                  (Apronaux.print_abstract0 ~string_of_dim)
                  abs
                  (Print.array (Apronaux.print_lincons0 ~string_of_dim))
                  tlincons0
                  (Print.array pp_print_int)
                  tdim
                  (Print.array (Apronaux.print_linexpr0 ~string_of_dim))
                  tlinexpr0);
              if false
              then (
                ...
                let string_of_dim = Apronaux.string_of_dimx in
                printf "abs2=%a@ " (Apronaux.print_abstract0 ~string_of_dim) abs;
                let res =
                  if tlincons0 <> [||]
            ...
            | `Jordan (abstrans : jordan) ->
              Template.refine_abstrans_with_poly_guard ~apron ~abstrans ~poly:abs;
              Trans.abstrans_apply apron abs abstrans
            | `Derivative (t : Derivative.t) -> Derivative.apply apron abs t
          in
          (), res)
    ...
    ; print_arc = (fun fmt _ -> pp_print_string fmt "()")
    ; accumulate = true
            ...
            then
              fprintf
                ...
                (Print.array
                   ...
                   ~sep:" and@ "
                   ~last:"@]"
                   (Lincons0.print (string_of_dim info.env)))
                tlincons0;
            fprintf fmt "%a@]" (print_tdimtlinexpr0 info.env) (tdim, tlinexpr0)
          | `Jordan (abstrans : jordan) ->
            let tlincons0, tdimtlinexpr0 =
              Trans.(
...
let analysis ~apron graph =
  ...
  let time = ref 0.0 in
  let output =
    Time.wrap_duration time (fun () ->
      if !Option.iteration_guided
      then
        Fixpoint.analysis_guided fixpoint graph sinit (fun filter ->
