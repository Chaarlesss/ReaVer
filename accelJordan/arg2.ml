...
let rec print_entry
  ...
  print_offset fmt offset;
  (match spec with
   | Toggle _ -> fprintf fmt "(+/-)%s %s" key arg
   | Symbol (l, _) ->
     fprintf
       fmt
       "-%s %a"
       ...
       l
   | _ -> fprintf fmt "-%s %s" key arg);
  (match tab with
   | Some tab ->
     let str = String.make tab ' ' in
     pp_print_string fmt str;
     pp_set_tab fmt ()
   | None -> pp_print_tab fmt ());
  fprintf fmt doc;
  pp_print_space fmt ();
  (match spec with
   | Recursive t ->
...
let rec parse_argv ?(current = current) (argv : string array) (t : t) : unit =
  ...
  let stop (error : error) : unit =
    ...
    (match error with
     | Unknown "-help" -> ()
     | Unknown "--help" -> ()
     | Unknown s -> fprintf Format.err_formatter "%s: unknown option `%s'.@." progname s
     | Missing s ->
     ...
     | Wrong (opt, arg, expected) ->
       fprintf
         ...
         opt
         expected
     | Message s -> fprintf Format.err_formatter "%s: %s.@." progname s);
    exit 2
  in
  let rec treat_action (s : string) (spec : spec) : unit =
    ...
    | Bool f when !current + 1 < l ->
      ...
      (try f (bool_of_string arg) with
       | Invalid_argument "bool_of_string" -> raise (Stop (Wrong (s, arg, "a boolean"))));
      incr current
    | Set_bool r when !current + 1 < l ->
      let arg = argv.(!current + 1) in
      (try r := bool_of_string arg with
       | Failure "bool_of_string" -> raise (Stop (Wrong (s, arg, "a Boolean"))));
      incr current
    | Toggle r ->
      (match String.get argv.(!current) 0 with
       | '+' -> r := true
       | '-' -> r := false
       | _ -> failwith "")
        ...
        raise
          (Stop
             (Wrong
                ...
                , Print.sprintf
                    "one of: %a"
                    (Print.list ~first:"@[" ~sep:"@ " pp_print_string)
                    symb )))
    | Int f when !current + 1 < l ->
      let arg = argv.(!current + 1) in
      (try f (int_of_string arg) with
       | Failure "int_of_string" -> raise (Stop (Wrong (s, arg, "an integer"))));
      incr current
    | Set_int r when !current + 1 < l ->
      let arg = argv.(!current + 1) in
      (try r := int_of_string arg with
       | Failure "int_of_string" -> raise (Stop (Wrong (s, arg, "an integer"))));
      incr current
    | Float f when !current + 1 < l ->
      let arg = argv.(!current + 1) in
      (try f (float_of_string arg) with
       | Failure "float_of_string" -> raise (Stop (Wrong (s, arg, "a float"))));
      incr current
    | Set_float r when !current + 1 < l ->
      ...
      (try r := float_of_string arg with
       | Failure "float_of_string" -> raise (Stop (Wrong (s, arg, "a float"))));
      incr current
    | Tuple specs -> List.iter (treat_action s) specs
    | Rest f ->
...
let parse (t : t) : unit = parse_argv Sys.argv t
  ...
  let rec print_aux (prefix : string) (t : t) =
    ...
    List.iter
      (fun (key, spec, _, _) ->
         let is_print =
           match spec with
           | Toggle _ | Set_bool _ | Set_int _ | Set_float _ | Set_string _ -> true
           | _ -> false
         in
         if is_print
         then (
