...
let scalar_to_float x =
  Scalar.(
    match x with
    | Mpqf x ->
      if Mpzf.sgn (Mpqf.get_den x) = 0
      then float_of_int (Mpqf.sgn x) *. inf
      else Mpqf.to_float x
...
let interval_to_float2 x = Interval.(scalar_to_float x.inf, scalar_to_float x.sup)
let print_scalar fmt x = fprintf fmt "%.4G" (scalar_to_float x)

let print_interval fmt itv =
  Interval.(
    Format.fprintf fmt "[@[<hv>%a;@ %a@]]" print_scalar itv.inf print_scalar itv.sup)
;;
...
let row_normalize row =
  ...
  Array.iteri (fun i el -> row.(i) <- el /. !min) row;
  ()
;;

let linexpr0_normalize linexpr0 =
  let maxdim = ref 0 in
  Linexpr0.iter (fun coeff dim -> maxdim := max !maxdim dim) linexpr0;
  ...
  for i = nbdims - 2 downto 0 do
    if row.(i) <> 0. then Linexpr0.set_coeff linexpr0 i (Coeff.s_of_float row.(i))
  done;
    ...
    iter
      (fun coeff dim ->
         ...
         then (
           ...
           (match coeff with
            | Coeff.Scalar scalar ->
              if Scalar.equal_int scalar (-1)
              then Format.pp_print_string fmt "-"
              else if not (Scalar.equal_int scalar 1)
              then print_scalar fmt scalar
            | Coeff.Interval i -> print_interval fmt i);
           Format.pp_print_string fmt (string_of_dim dim);
           first := false))
      expr;
    (match get_cst expr with
     | Coeff.Scalar scalar ->
       ...
       then Format.pp_print_char fmt '0'
     | Coeff.Interval i ->
       if not (Interval.is_zero i)
       then (
...
let print_lincons0 ?(string_of_dim = string_of_dim_default) fmt cons =
  Lincons0.(
    print_linexpr0
      ~string_of_dim
      fmt
...
let print_generator0 ?(string_of_dim = string_of_dim_default) fmt gen =
  Generator0.(
...
let print_abstract0 ?(string_of_dim = string_of_dim_default) fmt poly =
  ...
  if Apron.Abstract0.is_bottom man poly
  then pp_print_string fmt "bottom"
  else if Apron.Abstract0.is_top man poly
  then pp_print_string fmt "top"
  else (
    ...
    if false
    then
      if Array.length tlingen0 <= 48
      then (
        first := false;
        fprintf fmt "%a@ " (Print.array (print_generator0 ~string_of_dim)) tlingen0);
    (match !Option.print with
     | Option.Print_poly | Option.Print_boxpoly ->
       if Array.length tlincons0 <= 48
       then (
     ...
     | Option.Print_box | Option.Print_boxpoly ->
       ...
       Print.array
         (fun fmt (dim, (l, u)) ->
            ...
            if l = u
            then fprintf fmt "%s=%.4G" name l
            else if not (l = -1. /. 0. && u = 1. /. 0.)
            then fprintf fmt "%s in [%.4G , %.4G]" name l u)
         fmt
         box
     | _ -> ());
    if not !first then fprintf fmt "@ " else first := false;
    fprintf
      fmt
      "%i constraints, %i generators"
      (Array.length tlincons0)
      (Array.length tlingen0);
    fprintf fmt "@]")
;;
...
let print_abstract0_gen ?(string_of_dim = string_of_dim_default) fmt poly =
  ...
  then pp_print_string fmt "top"
    ...
    if Array.length tlingen0 <= 48
    then fprintf fmt "%a@ " (Print.array (print_generator0 ~string_of_dim)) tlingen0;
